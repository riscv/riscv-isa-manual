...
* [#norm:ext:cmo:coherent-alloc-dealloc-rw]#A coherent cache is permitted to allocate and deallocate copies of a cache
  block and perform read and write transfers# as described in <<#memory-caches>>

* [#norm:ext:cmo:coherent-write-mem]#A coherent cache is permitted to perform a write transfer to memory provided
...
* [#norm:ext:cmo:coherent-write-mem-perm]#At least one coherent cache is responsible for performing a write transfer to
...
* [#norm:ext:cmo:coherent-write-mem2]#A coherent cache is required to perform a write transfer to memory if a store
...
[#norm:ext:cmo:noncoherent-coherent-cbo]#A non-coherent agent may initiate a cache-block operation that operates on the
...
overlapping address rule. Specifically, [#norm:ext:cmo:PPO-overlap]#if _a_ precedes _b_ in program order,
...
. [#norm:ext:cmo:invalidate-load-val1]#If no clean or flush operations on _x_ precede _i_ in the global memory order,
  either the initial value of _x_ or the value of any store to _x_ that precedes
  _i_#

. [#norm:ext:cmo:invalidate-load-val2]#If no store to _x_ precedes a clean or flush operation on _x_ in the global
...
. [#norm:ext:cmo:invalidate-load-val3]#If a store to _x_ precedes a clean or flush operation on _x_ in the global
  memory order and if the clean or flush operation on _x_ precedes _i_ in the
...
. [#norm:ext:cmo:invalidate-load-val4]#The value of any store to _x_ by a non-coherent agent regardless of the above
  conditions#
...
* [#norm:ext:cmo:PMP-same]#The PMP access control bits shall be the same for _all_ physical addresses in
  the cache block, and if write permission is granted by the PMP access control
  bits, read permission shall also be granted#

* [#norm:ext:cmo:PMA-same]#The PMAs shall be the same for _all_ physical addresses in the cache block,
...
[#norm:ext:cmo:cbm-access]#A cache-block management instruction is permitted to access the specified cache
...
management instruction is permitted to access the cache block is UNSPECIFIED. [#norm:ext:cmo:cbm-unperm-fault]#If
...
otherwise.# [#norm:ext:cmo:cbm-unperm-translate]#During address translation, the instruction also checks the accessed
...
[#norm:ext:cmo:cbz-access]#A cache-block zero instruction is permitted to access the specified cache block
...
supported access type.# [#norm:ext:cmo:cbz-unperm-fault]#If access to the cache block is not permitted, a
...
access-fault exception otherwise.# [#norm:ext:cmo:cbz-unperm-translate]#During address translation, the instruction
also checks the accessed and dirty bits and may either raise an exception or set
the bits as required.#

[#norm:ext:cmo:cbp-access]#A cache-block prefetch instruction is permitted to access the specified cache
block whenever a load instruction, store instruction, or instruction fetch is
permitted to access the corresponding physical addresses.# [#norm:ext:cmo:cbp-unperm-noexcep]#If access to the cache
block is not permitted, a cache-block prefetch instruction does not raise any
exceptions and shall not access any caches or memory.# [#norm:ext:cmo:cbp-unperm-translate]#During address
...
[#norm:ext:cmo:fault-excep-csr]#When a page-fault, guest-page-fault, or access-fault exception is taken, the
...
[[norm:ext:cmo:no-addr-misaligned-excep]]
...
[#norm:ext:cmo:cache-block-zero-specified-block]#These instructions operate on the cache block, or the memory locations
...
[#norm:ext:cmo:cache-block-prefetch-operating-block]#These instructions operate on the cache block whose effective address is the sum
...
[#norm:ext:cmo:cbo_clean]#A *cbo.clean* instruction performs a clean operation on the cache block whose
effective address is the base address specified in _rs1_.# [#norm:ext:cmo:cbo_clean-offset]#The offset operand may
...
[#norm:ext:cmo:cbo_flush]#A *cbo.flush* instruction performs a flush operation on the cache block whose
that contains the address specified in _rs1_.# [#norm:ext:cmo:cbo_flush-unaligned]#It is not required that _rs1_ is
...
[#norm:ext:cmo:cbo_inval]#A *cbo.inval* instruction performs an invalidate operation on the cache block
that contains the address specified in _rs1_.# [#norm:ext:cmo:cbo_inval-unaligned]#It is not required that _rs1_ is
aligned to the size of a cache block.# On faults, the faulting virtual address
is considered to be the value in rs1, rather than the base address of the cache
block. The instruction operates on the set of coherent caches accessed by the
agent executing the instruction.
...
[#norm:ext:cmo:cbo_zero]#A *cbo.zero* instruction performs stores of zeros to the full set of bytes
corresponding to the cache block that contains the address specified in _rs1_.#
[#norm:ext:cmo:cbo_zero-unaligned]#It is not required that _rs1_ is aligned to the size of a cache block.# On
