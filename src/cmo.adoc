...
* [#norm:cmo-coherent-alloc-dealloc-rw]#A coherent cache is permitted to allocate and deallocate copies of a cache
  block and perform read and write transfers# as described in <<#memory-caches>>

* [#norm:cmo-coherent-write-mem]#A coherent cache is permitted to perform a write transfer to memory provided
...
* [#norm:cmo-coherent-write-mem-perm]#At least one coherent cache is responsible for performing a write transfer to
...
* [#norm:cmo-coherent-write-mem2]#A coherent cache is required to perform a write transfer to memory if a store
...
[#norm:cmo-noncoherent-coherent-cbo]#A non-coherent agent may initiate a cache-block operation that operates on the
...
overlapping address rule. Specifically, [#norm:cmo-PPO-overlap]#if _a_ precedes _b_ in program order,
...
. [#norm:cmo-invalidate-load-val1]#If no clean or flush operations on _x_ precede _i_ in the global memory order,
  either the initial value of _x_ or the value of any store to _x_ that precedes
  _i_#

. [#norm:cmo-invalidate-load-val2]#If no store to _x_ precedes a clean or flush operation on _x_ in the global
...
. [#norm:cmo-invalidate-load-val3]#If a store to _x_ precedes a clean or flush operation on _x_ in the global
  memory order and if the clean or flush operation on _x_ precedes _i_ in the
...
. [#norm:cmo-invalidate-load-val4]#The value of any store to _x_ by a non-coherent agent regardless of the above
  conditions#
...
* [#norm:cmo-PMP-same]#The PMP access control bits shall be the same for _all_ physical addresses in
  the cache block, and if write permission is granted by the PMP access control
  bits, read permission shall also be granted#

* [#norm:cmo-PMA-same]#The PMAs shall be the same for _all_ physical addresses in the cache block,
...
[#norm:cmo-cbm-access]#A cache-block management instruction is permitted to access the specified cache
...
management instruction is permitted to access the cache block is UNSPECIFIED. [#norm:cmo-cbm-unperm-fault]#If
...
otherwise.# [#norm:cmo-cbm-unperm-translate]#During address translation, the instruction also checks the accessed
...
[#norm:cmo-cbz-access]#A cache-block zero instruction is permitted to access the specified cache block
...
supported access type.# [#norm:cmo-cbz-unperm-fault]#If access to the cache block is not permitted, a
...
access-fault exception otherwise.# [#norm:cmo-cbz-unperm-translate]#During address translation, the instruction
also checks the accessed and dirty bits and may either raise an exception or set
the bits as required.#

[#norm:cmo-cbp-access]#A cache-block prefetch instruction is permitted to access the specified cache
block whenever a load instruction, store instruction, or instruction fetch is
permitted to access the corresponding physical addresses.# [#norm:cmo-cbp-unperm-noexcep]#If access to the cache
block is not permitted, a cache-block prefetch instruction does not raise any
exceptions and shall not access any caches or memory.# [#norm:cmo-cbp-unperm-translate]#During address
...
[#norm:cmo-fault-excep-csr]#When a page-fault, guest-page-fault, or access-fault exception is taken, the
...
[[norm:cmo-no-addr-misaligned-excep]]
...
[#norm:cbo-zero-specified-block]#These instructions operate on the cache block, or the memory locations
...
[#norm:prefetch-operating-block]#These instructions operate on the cache block whose effective address is the sum
...
[#norm:cbo-clean-op]#A *cbo.clean* instruction performs a clean operation on the cache block whose
effective address is the base address specified in _rs1_.# [#norm:cbo-clean-offset]#The offset operand may
...
[#norm:cbo-flush-op]#A *cbo.flush* instruction performs a flush operation on the cache block whose
that contains the address specified in _rs1_.# [#norm:cbo-flush-unaligned]#It is not required that _rs1_ is
...
[#norm:cbo-inval-op]#A *cbo.inval* instruction performs an invalidate operation on the cache block
that contains the address specified in _rs1_.# [#norm:cbo-inval-unaligned]#It is not required that _rs1_ is
aligned to the size of a cache block.# On faults, the faulting virtual address
is considered to be the value in rs1, rather than the base address of the cache
block. The instruction operates on the set of coherent caches accessed by the
agent executing the instruction.
...
[#norm:cbo-zero-op]#A *cbo.zero* instruction performs stores of zeros to the full set of bytes
corresponding to the cache block that contains the address specified in _rs1_.#
[#norm:cbo-zero-unaligned]#It is not required that _rs1_ is aligned to the size of a cache block.# On
