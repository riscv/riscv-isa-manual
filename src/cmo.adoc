...
[#norm:noncoherent-coherent-cbo]#A non-coherent agent may initiate a cache-block operation that operates on the
...
overlapping address rule. Specifically, [#norm:PPO-overlap]#if _a_ precedes _b_ in program order,
...
. [#norm:invalidate-load-val1]#If no clean or flush operations on _x_ precede _i_ in the global memory order,
  either the initial value of _x_ or the value of any store to _x_ that precedes
  _i_#

. [#norm:invalidate-load-val2]#If no store to _x_ precedes a clean or flush operation on _x_ in the global
...
. [#norm:invalidate-load-val3]#If a store to _x_ precedes a clean or flush operation on _x_ in the global
  memory order and if the clean or flush operation on _x_ precedes _i_ in the
...
. [#norm:invalidate-load-val4]#The value of any store to _x_ by a non-coherent agent regardless of the above
  conditions#
...
* [#norm:PMP-same]#The PMP access control bits shall be the same for _all_ physical addresses in
  the cache block, and if write permission is granted by the PMP access control
  bits, read permission shall also be granted#

* [#norm:PMA-same]#The PMAs shall be the same for _all_ physical addresses in the cache block,
  and if write permission is granted by the supported access type PMAs, read
  permission shall also be granted#

[#norm:cbo-rsv]#If the above constraints are not met, the behavior of a CBO instruction is
UNSPECIFIED.#
...
[#norm:cbm-access]#A cache-block management instruction is permitted to access the specified cache
...
management instruction is permitted to access the cache block is UNSPECIFIED. [#norm:cbm-unperm-fault]#If
...
otherwise.# [#norm:cbm-unperm-translate]#During address translation, the instruction also checks the accessed
...
[#norm:cbz-access]#A cache-block zero instruction is permitted to access the specified cache block
...
supported access type.# [#norm:cbz-unperm-fault]#If access to the cache block is not permitted, a
...
access-fault exception otherwise.# [#norm:cbz-unperm-translate]#During address translation, the instruction
also checks the accessed and dirty bits and may either raise an exception or set
the bits as required.#

[#norm:cbp-access]#A cache-block prefetch instruction is permitted to access the specified cache
block whenever a load instruction, store instruction, or instruction fetch is
permitted to access the corresponding physical addresses.# [#norm:cbp-unperm-noexcep]#If access to the cache
block is not permitted, a cache-block prefetch instruction does not raise any
exceptions and shall not access any caches or memory.# [#norm:cbp-unperm-translate]#During address
...
[#norm:fault-excep-csr]#When a page-fault, guest-page-fault, or access-fault exception is taken, the
...
[[norm:cmo-menvcfg-csr]][[norm:cmo-senvcfg-csr]][[norm:cmo-henvcfg-csr]]
...
[[norm:menvcfg-cbie-op]]
Enables the execution of the cache block invalidate instruction, `CBO.INVAL`, in
a lower privilege mode:

* [#norm:menvcfg-cbie-00]#`00`: The instruction raises an illegal-instruction or virtual-instruction#
  exception
* [#norm:menvcfg-cbie-01]#`01`: The instruction is executed and performs a flush operation#
* `10`: _Reserved_
* [#norm:menvcfg-cbie-11]#`11`: The instruction is executed and performs an invalidate operation#
