...
[[hedeleg-bits]]
.Bits of `hedeleg` that must be writable or muse be read-only zero.
...
[[hgeiereg]]
.Hypervisor guest external interrupt-enable register (`hgie`).
...
address, and if operand __rs2__latexmath:[$\neq$]`x0`, it specifies a
...
in <<sv39x4va>>. This partitioning is identical to
that for an Sv39 virtual address as depicted in
<<sv39va>>, except with 2 more bits at the
...
assigned cause 10, whereas those from HS-mode or S-mode use cause9 as
usual.
