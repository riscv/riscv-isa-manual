...
Otherwise, the implementation shall select the largest supported nonzero
lambda value for the resulting configuration.
The set of supported nonzero lambda values is never empty for a legal
configuration (see <<integrated-matrix-lambda>>), so this selection
always yields a nonzero value.
...
For any legal (`VLEN`, `SEW`) configuration the set of supported nonzero
lambda values is non-empty. The minimum supported lambda is `1` when
`VLEN ≤ 16 × SEW`, and a larger power of two otherwise, because
`EMUL_C` = `VLEN` ÷ (`SEW` × λ²) shall not exceed `16`. Consequently
`lambda[2:0]` always holds a supported nonzero value in a legal
configuration, and `000` is never produced as a selected value by a
...
* Otherwise, the implementation shall select the largest supported
  nonzero lambda value for the resulting configuration.

Because the supported set is non-empty, a `000` write always results in
a supported nonzero selected value.
...
If no supported value is less than or equal to the requested value, the
implementation shall select the smallest supported nonzero lambda value.

Because the supported set is non-empty, a nonzero write always results
in a supported nonzero selected value.

A configuration instruction on a legal (`VLEN`, `SEW`) configuration
never selects `lambda[2:0] = 000`; the encoding can be observed only in
an as-yet-unconfigured `vtype`. Base Vector instructions may execute
...
Because the supported set is non-empty for any legal (`VLEN`, `SEW`)
configuration, the established value is always a supported nonzero value.
