...
if vm == 0 & vtype[bs] == 0b1 & (8 * LMUL > EEW_C) then return Illegal_Instruction();
...
foreach (j from 0 to (N - 1)) {
  foreach (i from 0 to (M - 1)) {
    ...
    foreach (s from 0 to (S - 1)) {
      ...
      // Accumulate sub-dot-products within this block
      let k_lo : int = s * block_size;
      let k_hi : int = min(k_lo + block_size, K_eff) - 1;

      var blk_acc : bits(2 * EEW_C) = zeros(2 * EEW_C);
      foreach (k from k_lo to k_hi) {
        let a_flat : int = mat_A_idx(i, k, K_eff, LMUL, lambda, epr_A);
        let b_flat : int = mat_B_idx(k, j, K_eff, LMUL, lambda, epr_A);
        let a_fp   : bits(EEW_C) = fp_mul_to(
            read_single_element(EEW_A, a_flat, vs1), EEW_A, fmt_A, EEW_C, fmt_C, rm);
        let b_fp   : bits(EEW_C) = fp_mul_to(
            read_single_element(EEW_A, b_flat, vs2), EEW_A, fmt_B, EEW_C, fmt_C, rm);
        blk_acc = fp_add(blk_acc, fp_mul(a_fp, b_fp, EEW_C, fmt_C, rm),
                         2 * EEW_C, fmt_C, rm)
      };

      // Scale and accumulate into C
      let scaled : bits(EEW_C) = fp_mul(truncate(blk_acc, EEW_C), blk_scale,
                                        EEW_C, fmt_C, rm);
      acc = fp_add(acc, scaled, EEW_C, fmt_C, rm)
    };

    if nan_out then acc = fp_canonical_NaN(EEW_C, fmt_C);
...
// BS=16 LMUL restriction: W × LMUL ≤ SEW
if vtype[bs] == 0b1 & (8 * LMUL > EEW_C) then return Illegal_Instruction();
// Microscaling legality: SEW × λ ≥ 16
if EEW_C * lambda < 16 then return Illegal_Instruction();
