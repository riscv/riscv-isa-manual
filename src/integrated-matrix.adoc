...
accumulator format — i.e., when p~A~ + p~B~ ≤ p~C~, where p~A~,
...
+
The encoding SEW=8 (EEW=4, Int4 inputs into OFP8 accumulator) is _reserved_.
...
The `bs` field in `vtype` selects the block size (32 or 16 elements).
+
The encodings SEW=8 (EEW=2) and altfmt=1 with SEW=32 are _reserved_.
