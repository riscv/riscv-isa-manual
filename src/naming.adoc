...
RISC-V ISA strings begin with either RV32I, RV32E, RV64I, or RV64E,
...
NOTE: The Zhinx extension is an exception to this rule, as the extension is not
related to the H extension but the Zfh extension.

If multiple "Z" extensions are named, they should be ordered first by category,
then alphabetically within a category—for example, "Zicsr_Zifencei_Ztso".
...
Components in ISA name string must appear in the following order:

. Base ISA,
. One-letter extensions, IMAFDQCBPVH, in this order,
...
For instances, RV32IMACV is legal, whereas RV32IMAVC is not.
...
|Hypervisor-level extension "jkl" |Shjkl |

3+^|*Standard Machine-Level Extensions*

|Machine-level extension "mno" |Smmno |

3+^|*Non-Standard Extensions*

|Non-standard extension "pqr" |Xpqr |
|===
