...
<<epcreg>>. The low bit of `sepc` (`sepc[0]`) is
...
spaces.
* If __rs1__=`x0` and __rs2__latexmath:[$\neq$]`x0`, the fence orders all
...
* If __rs1__latexmath:[$\neq$]`x0` and __rs2__=`x0`, the fence orders only
...
latexmath:[${\tt a+va}.vpn\[i\] \times \textrm{PTESIZE}$]. (For Sv32, PTESIZE=4.)
...
supports 2 MiB _megapages_ and 1 GiB _gigapages_, each of which must be virtually and physically aligned to a boundary equal to its size. A page-fault
...
__pte.ppn[i][pte.napot_bits__-1:0] is replaced by
__vpn[i][pte.napot_bits__-1:0]. If the encoding in
latexmath:[$pte$] is reserved according to
<<ptenapot>>, then a page-fault exception must be
raised.
...
__j >> napot_bits__ = __vpn[i] >> napot_bits__, all for the
...
update the PTE(s), as described in <<sfence.vma>>,
