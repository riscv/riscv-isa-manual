...
This chapter defines RISC-V's memory consistency model, named RISC-V Weak
Memory Ordering (RVWMO).
As its name implies, RVWMO is a relaxed memory consistency model.

Informally, there exists a total order of all harts' memory accesses,
but the order in which a hart's memory accesses appear in that total order
might not correspond to their program order.
Any pair of memory accesses from a hart might be reordered unless:

\begin{itemize}
  \item their addresses overlap, or
  \item they are ordered by a FENCE instruction or an AMO instruction's acquire or release annotation, or
  \item the younger access has a data dependency on the older access, possibly carried through other instructions, or
  \item the younger access is a store, and a branch or indirect jump that is older than the store has a data dependency on the older memory access.
\end{itemize}

RVWMO is described in full by the following operational model, which has
...
\section{Introduction to the RVWMO Operational Model}

This section contains an informal introduction of the operational model's states and transitions.
...
When such instruction is finished, any untaken alternative paths are discarded.
...
\section{RVWMO Operational Model}\label{sec:omm:pseudocode_exec}

This section formally defines the RVWMO operational model.
...
RVWMO allows store operations from the same hart as the {\tt sc} to intervene between the store operation of the {\tt sc} and the store operations the paired {\tt lr} read from.
...
\begin{commentary}
RVWMO allows misaligned memory accesses to be decomposed at any granularity.
This model decomposes them to one-byte accesses as this granularity subsumes all others.
\end{commentary}
