...
[#norm:senvcfg_ukte_writable]#If the Svukte extension is implemented, the csr:senvcfg[UKTE] field is writable#.
[#norm:hstatus_hukte_writable]#If the hypervisor extension is additionally implemented, the csr:hstatus[HUKTE] field
is also writable#.
See <<sec:senvcfg>> and <<sec:hstatus>> for the definitions of those fields.

[[norm:Svukte_depends_Sv39]]
...
[#norm:Svukte_qualification]#When csr:senvcfg[UKTE]=1 and the active csr:satp[] register's csr::[MODE] field is not Bare,
...
[#norm:Svukte_qualified_fault]#For any Svukte-qualified memory access, virtual addresses {ge} 2^SXLEN-1^ are
...
[#norm:Svukte_timing_independence]#The timing of an instruction that raises an exception for this reason must be
...
[[norm:Svukte_hypervisor_qualification]]
