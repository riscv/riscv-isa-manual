...
Programs compiled with the insnlink:lpad[] instruction continue to function correctly,
...
completion of insnlink:jalr[]/insnlink:c.jalr[]/insnlink:c.jr[], but before the instruction at the target
...
insnlink:lpad[] instruction when `ELP` is `LP_EXPECTED` or a software-check
exception caused by the insnlink:lpad[] instruction itself leads to a trap being
...
An insnlink:mret[] or insnlink:sret[] instruction is used to return from a trap in M-mode or
...
See also <<rnmi>> for semantics added to the RNMI trap and the insnlink:mnret[] instruction
...
need not be synchronized with insnlink:sfence.vma[], insnlink:hfence.gvma[], or
insnlink:hfence.vvma[] instructions.
...
by an insnlink:ssamoswap.w[] or insnlink:ssamoswap.d[] instruction results in a store/AMO access-fault exception.#

[#norm:ssmp_ss_page_access_fault]#Memory mapped as an SS page cannot be written to by instructions other than
insnlink:ssamoswap.w[], insnlink:ssamoswap.d[], insn:sspush[], and insn:c.sspush[]. Attempts will raise a store/AMO
...
accessed memory to be idempotent. [#norm:ssmp_ss_idempotent_memory]#If the memory referenced by
insn:sspush[], insn:c.sspush[], insn:sspopchk[], insn:c.sspopchk[], insnlink:ssamoswap.w[] and insnlink:ssamoswap.d[] instructions is not
...
The insnlink:ssamoswap.w[] and insnlink:ssamoswap.d[] instructions require the PMA of the accessed memory range to
