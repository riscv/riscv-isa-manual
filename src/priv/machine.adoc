...
hart.# [#norm:m_mode_first_at_reset]#M-mode is used for low-level access to a hardware platform and
...
[#norm:misa_acc]#The csr:misa[] CSR is a *WARL* read-write register# reporting the ISA supported by the hart.
[#norm:misa_always_rd]#This register must be readable in any implementation#,
but [#norm:misa_csr_implemented]#a value of zero can be returned to indicate this feature has not been implemented#, requiring that CPU capabilities be determined through a separate non-standard mechanism.

[[norm:misa_enc_img]]
.Machine ISA register (csr:misa[])
include::images/bytefield/misareg.edn[]

[#norm:misa_mxl_op_isa]#The csr::[mxl] (Machine XLEN) field encodes the native base integer ISA width as
shown in <<norm:misa_mxl_enc>>.#
[#norm:misa_mxl_acc]#The csr::[mxl] field is read-only.#
[#norm:misa_mxl_op_nz]#If csr:misa[] is nonzero, the
csr::[mxl] field indicates the effective XLEN in M-mode, a constant termed _MXLEN_.#
[#norm:xlen_le_mxlen]#XLEN is never greater than MXLEN, but XLEN might be smaller than MXLEN in
less-privileged modes.#

[[norm:misa_mxl_enc]]
.Encoding of csr::[mxl] field in csr:misa[]
...
[#norm:misa_sz]#The csr:misa[] CSR is MXLEN bits wide.#

[NOTE]
====
The base width can be quickly ascertained using branches on the sign of
the returned csr:misa[] value, and possibly a shift left by one and a second
...
The base width can also be found if csr:misa[] is zero, by placing the
...
extension ext:a[], bit 1 encodes presence of extension ext:b[], through to
bit 25 which encodes ext:z[]).#
[#norm:misa_i_op]#The csr::[i] bit will be set for the RV32I and RV64I base ISAs#,
and [#norm:misa_e_op]#the csr::[e] bit will be set for RV32E and RV64E.#
...
[#norm:misa_extensions_reset_maximal]#At reset, the Extensions field shall contain the maximal set of supported extensions,
and csr::[i] shall be selected over csr::[e] if both are available.#

[[norm:misa_extensions_disabling]]
When a standard extension is disabled by clearing its bit in csr:misa[], the
...
example, the ext:f[] extension is said to be implemented for an execution
environment if and only if the instructions that the RISC-V Unprivileged
ISA defines for ext:f[] execute as specified.

[[norm:misa_extensions_disabling_def]]
With this definition of _implemented_, disabling an extension by
clearing its bit in csr:misa[] results in the extension being considered
_not implemented_ in M-mode. For example, setting csr:misa[f]=0 results in
the ext:f[] extension being not implemented for M-mode, because the
ext:f[] extension's instructions will not act as the Unprivileged ISA requires
...
.Encoding of Extensions field in csr:misa[]
[%autowidth,float="center",align="center",cols=">,>,<",options="header"]
|===
|Bit |Character |Description
|0  |csr::[a] |Atomic extension
|1  |csr::[b] |B extension
|2  |csr::[c] |Compressed extension
|3  |csr::[d] |Double-precision floating-point extension
|4  |csr::[e] |RV32E/64E base ISA
|5  |csr::[f] |Single-precision floating-point extension
|6  |csr::[g] |_Reserved_
|7  |csr::[h] |Hypervisor extension
|8  |csr::[i] |RV32I/64I base ISA
|9  |csr::[j] |_Reserved_
|10 |csr::[k] |_Reserved_
|11 |csr::[l] |_Reserved_
|12 |csr::[m] |Integer Multiply/Divide extension
|13 |csr::[n] |_Tentatively reserved for User-Level Interrupts extension_
|14 |csr::[o] |_Reserved_
|15 |csr::[p] |_Tentatively reserved for Packed-SIMD extension_
|16 |csr::[q] |Quad-precision floating-point extension
|17 |csr::[r] |_Reserved_
|18 |csr::[s] |Supervisor mode implemented
|19 |csr::[t] |_Reserved_
|20 |csr::[u] |User mode implemented
|21 |csr::[v] |Vector extension
|22 |csr::[w] |_Reserved_
|23 |csr::[x] |Non-standard extensions implemented
|24 |csr::[y] |_Reserved_
|25 |csr::[z] |_Reserved_
|===

[#norm:misa_x_op]#The csr::[x] bit will be set if there are any non-standard extensions.#

[#norm:misa_B_op]#When the csr::[b] bit is 1, the implementation supports the instructions provided by the
ext:zba[], ext:zbb[], and ext:zbs[] extensions.# When the csr::[b] bit is 0, it indicates that the
implementation might not support one or more of the ext:zba[], ext:zbb[], or ext:zbs[] extensions.

[#norm:misa_m_op]#When the csr::[m] bit is 1, the implementation supports all multiply and
division instructions defined by the ext:m[] extension.# When the csr::[m] bit
is 0, it indicates that the implementation might not support those
instructions. However [#norm:Zmmul_misa_m]#if the ext:zmmul[] extension is supported then
the multiply instructions it specifies are supported irrespective
of the value of the csr::[m] bit.#

[#norm:misa_s_op]#When the csr::[s] bit is 1, the implementation supports supervisor mode.#
When the csr::[s] bit is 0, the implementation might not support supervisor mode.

[#norm:misa_u_op]#When the csr::[u] bit is 1, the implementation supports user mode.#
When the csr::[u] bit is 0, the implementation might not support user mode.

[NOTE]
====
The csr:misa[] CSR exposes a rudimentary catalog of CPU features to
...
[#norm:misa_e_acc]#The csr::[e] bit is read-only.#
[#norm:misa_e_not_i]#Unless csr:misa[] is all read-only zero, the
csr::[e] bit always reads as the complement of the csr::[i] bit.#
If an execution environment supports both RV32E and RV32I, software can select
RV32E by clearing the csr::[i] bit.
...
being disabled.#
For example, setting csr::[f]=0 and csr::[d]=1 results in both csr::[f] and csr::[d] being cleared.
Similarly, setting csr::[u]=0 and csr::[s]=1 results in both csr::[u] and csr::[s] being cleared.

[#norm:misa_extensions_collective_warl]#An implementation may impose additional constraints on the collective
setting of two or more csr:misa[] fields, in which case they function
...
[#norm:misa_inc_ialign]#Writing csr:misa[] may increase IALIGN, e.g., by disabling the
ext:c[] extension. If an instruction that would write csr:misa[] increases IALIGN,
and the subsequent instruction's address is not IALIGN-bit aligned, the
write to csr:misa[] is suppressed, leaving csr:misa[] unchanged.#

[NOTE]
====
[#norm:misa_C_set_line]#csr:misa[c] must be clear if any of the following holds:#

[[norm:misa_C_set_list]]
* ext:zca[] is not implemented
* MXLEN=32, ext:f[] is implemented, and ext:zcf[] is not implemented
* ext:d[] is implemented, and ext:zcd[] is not implemented
====

[#norm:misa_extensions_reenable_unspec]#When software enables an extension that was previously disabled, then
all state uniquely associated with that extension is UNSPECIFIED, unless otherwise specified by that extension.#

NOTE: Although one of the bits 25{endash}0 in csr:misa[] being set to 1 implies that
...
[#norm:mvendorid_sz_acc_op]#The csr:mvendorid[] CSR is a 32-bit read-only register providing the JEDEC
...
.Vendor ID register (csr:mvendorid[])
...
csr:mvendorid[] CSR as `0x60a`.

[NOTE]
====
In JEDEC's parlance, the bank number is one greater than the number of
continuation codes; hence, the csr:mvendorid[] Bank field encodes a value
...
[#norm:marchid_sz_acc_op]#The csr:marchid[] CSR is an MXLEN-bit read-only register encoding the base
...
The combination of csr:mvendorid[] and csr:marchid[] should uniquely identify the type of hart microarchitecture that is implemented.
...
organization. The csr:misa[] register also helps distinguish different
variants of a design.
====

==== Machine Implementation ID (csr:mimpid[]) Register

[#norm:mimpid_op]#The csr:mimpid[] CSR provides a unique encoding of the version of the
...
[#norm:mhartid_sz_acc_op]#The csr:mhartid[] CSR is an MXLEN-bit read-only register containing the
...
[#norm:mstatus_sz_acc]#The csr:mstatus[] register is an MXLEN-bit read/write register formatted as
shown in <<mstatusreg-rv32>> for RV32 and <<mstatusreg>> for RV64. csr:mstatus[] must be implemented.#
The csr:mstatus[] register keeps track of and controls the hart’s current operating state. A
restricted view of csr:mstatus[] appears as the csr:sstatus[] register in the S-level ISA.

[[mstatusreg-rv32]]
.Machine-mode status (csr:mstatus[]) register for RV32
include::images/wavedrom/mstatusreg-rv321.edn[]

[[mstatusreg]]
.Machine-mode status (csr:mstatus[]) register for RV64
include::images/wavedrom/mstatusreg.edn[]

[#norm:mstatush_sz_acc]#For RV32 only, csr:mstatush[] is a 32-bit read/write register formatted as shown in <<mstatushreg>>.#
[#norm:mstatush_enc]#Bits 30:4 of csr:mstatush[] generally contain the same fields found in bits 62:36 of csr:mstatus[] for RV64. Fields csr::[sd], csr::[sxl], and csr::[uxl] do not exist in `mstatush`.#

[[mstatushreg]]
.Additional machine-mode status (csr:mstatush[]) register  for RV32
include::images/wavedrom/mstatushreg.edn[]

[[privstack]]
===== Privilege and Global Interrupt-Enable Stack in csr:mstatus[] register

[#norm:mstatus_mie_sie_exist]#Global interrupt-enable bits, csr::[mie] and csr::[sie], are provided for M-mode and
...
[NOTE]
====
The global __x__IE bits are located in the low-order bits of csr:mstatus[],
allowing them to be atomically set or cleared with a single CSR
instruction.
====

[#norm:mstatus_mie_sie_op]#When a hart is executing in privilege mode _x_, interrupts are globally
...
[#norm:mstatus_sie_spie_rdonly0]#csr::[mie] and csr::[mpie] must be writable.
If supervisor mode is not implemented, then csr::[sie] and csr::[spie] are read-only 0;
...
[#norm:mstatus_mpp_sz]#csr::[mpp] is two bits wide# and [#norm:mstatus_spp_sz]#csr::[spp] is one bit wide.#
...
An insn:mret[] or insn:sret[] instruction is used to return from a trap in M-mode or S-mode respectively.
...
__y__{ne}M, __x__RET also sets csr::[mprv]=0.#
...
writing that mode to csr::[mpp] then reading it back.

If the machine provides only U and M modes, then only a single hardware
storage bit is required to represent either 00 or 11 in csr::[mpp].
====
...
[#norm:mstatus_mdt_sz_warl]#The M-mode-disable-trap (csr::[mdt]) bit is a WARL field introduced by the ext:smdbltrp[] extension.#
[#norm:mstatus_mdt_rst]#Upon reset, the csr::[mdt] field is set to 1.#
[#norm:mstatus_mie_clr_by_mdt]#When the csr::[mdt] bit is set to 1 by an explicit CSR write,
the csr::[mie] (Machine Interrupt Enable) bit is cleared to 0.#
[#norm:mstatus_mie_clr_by_mdt_rv64]#For RV64, this clearing occurs regardless of the value written, if any, to the csr::[mie] bit by the same write.#
[#norm:mstatus_mie_set_mdt_0]#The csr::[mie] bit can only be set to 1 by an
...
(For RV32, the csr::[mdt] bit is in csr:mstatush[] and the csr::[mie] bit in csr:mstatus[] register).

[#norm:trap_exp]#When a trap is to be taken into M-mode, if the csr::[mdt] bit is currently 0, it is then set to 1, and the trap is delivered as expected.#
[#norm:trap_unexp_mdt_1]#However, if csr::[mdt] is already set to 1, then this is an _unexpected trap_.#
[#norm:trap_unexp_rnmi]#When the ext:smrnmi[] extension
is implemented, a trap caused by an RNMI is not considered an _unexpected trap_
irrespective of the state of the csr::[mdt] bit.#
[#norm:mstatus_mdt_not_set_rnmi]#A trap caused by an RNMI does not set the csr::[mdt] bit.# However,
[#norm:trap_unexp_mnstatus_nmie_0]#a trap that occurs when executing in M-mode with
csr:mnstatus[nmie] set to 0 is an _unexpected trap_.#

[#norm:trap_unexp_hndl_lead_in]#In the event of a _unexpected trap_, the handling is as follows:#
...
* [#norm:trap_unexp_hndl_rnmi]#When the Smrnmi extension is implemented and csr:mnstatus[nmie] is 1, the hart
  ...
  M-mode and its csr::[nmie] field is set to 0.#
...
csr:mtval[] and the csr:mtval2[] registers. This information, if needed, can be obtained
by the RNMI handler by decoding the instruction at the address in csr:mnepc[] and
examining its source register contents.
====

* [#norm:trap_unexp_hndl_no_rnmi]#When the ext:smrnmi[] extension is not implemented, or if the ext:smrnmi[] extension is
  implemented and csr:mnstatus[nmie] is 0, the hart enters a critical-error state
  ...
  asserting a "`critical-error`" signal to the platform.# Whether performance
  counters and timers are updated in the critical-error state is UNSPECIFIED.
...
When the ext:smdbltrp[] extension is implemented, executing an insn:mret[] instruction, or executing
an insn:sret[] instruction while the current privilege mode is M, has the following additional
effects.
[#norm:mstatus_mdt_clr_mret_sret]#The csr::[mdt] bit is set to 0.#
[#norm:sstatus_sdt_clr_mret_sret]#If the Ssdbltrp extension is also implemented, and the new privilege mode is U, VS, or VU, then csr:sstatus[sdt] is also set to 0.#
[#norm:vsstatus_sdt_clr_mret_sret]#Additionally, if it is VU, then csr:vsstatus[sdt] is also set to 0.#

[#norm:mstatus_mdt_clr_mnret]#When the ext:smdbltrp[] extension is implemented, the insn:mnret[] instruction, provided by the ext:smrnmi[] extension, sets the csr::[mdt] bit to 0 if the new privilege mode is not M.#
[#norm:sstatus_sdt_clr_mnret]#If the ext:ssdbltrp[] extension is also implemented, and the new privilege mode is U, VS, or VU, then csr:sstatus[sdt] is also set to 0.#
[#norm:vsstatus_sdt_clr_mnret]#Additionally, if it is VU, then csr:vsstatus[sdt] is also set to 0.#

[[xlen-control]]
===== Base ISA Control in csr:mstatus[] Register

[#norm:mstatus_sxl_uxl_warl_op]#For RV64 harts, the csr::[sxl] and csr::[uxl] fields are *WARL* fields that control the
value of XLEN for S-mode and U-mode, respectively.#
[#norm:mstatus_sxl_uxl_enc]#The encoding of these fields is the same as the csr::[mxl] field of csr:misa[], shown in <<norm:misa_mxl_enc>>.#
...
When MXLEN=32, the csr::[sxl] and csr::[uxl] fields do not exist, and SXLEN=32 and UXLEN=32.

[#norm:mstatus_sxl_acc_mxlen64]#When MXLEN=64, if S-mode is not supported, then csr::[sxl] is read-only zero.
Otherwise, it is a *WARL* field that encodes the current value of SXLEN.#
In particular, [#norm:mstatus_sxl_rdonly_mxlen64]#an implementation may make csr::[sxl] be a read-only field whose
value always ensures that SXLEN=MXLEN.#

[#norm:mstatus_uxl_acc_mxlen64]#When MXLEN=64, if U-mode is not supported, then csr::[uxl] is read-only zero.
Otherwise, it is a *WARL* field that encodes the current value of UXLEN.#
In particular, [#norm:mstatus_uxl_rdonly_mxlen64]#an implementation may make csr::[uxl] be a read-only field whose
value always ensures that UXLEN=MXLEN or UXLEN=SXLEN.#

[[norm:mstatus_uxl_legal_vals_smode]]
If S-mode is implemented, the set of legal values that the csr::[uxl] field may
...
The csr::[mprv] (Modify PRiVilege) bit modifies the _effective privilege mode_,
i.e., the privilege level at which explicit memory accesses execute.
[#norm:mstatus_mprv_ldst_op]#When csr::[mprv]=0, explicit memory accesses
...
setting of csr::[mprv].#
[#norm:mstatus_mprv_rdonly0_no_umode]#csr::[mprv] is read-only 0 if U-mode is not supported; otherwise, it must be writable.#

[#norm:mstatus_mprv_clr_mret_sret_less_priv]#An insn:mret[] or insn:sret[] instruction that changes the privilege mode to a mode
less privileged than M also sets csr::[mprv]=0.#
...
[NOTE]
====
The csr::[mprv] and csr::[mxr] mechanisms were conceived to improve the efficiency of
M-mode routines that emulate missing hardware features, e.g., misaligned
loads and stores. csr::[mprv] obviates the need to perform address translation
in software. csr::[mxr] allows instruction words to be loaded from pages marked
execute-only.

The current privilege mode and the privilege mode specified by csr::[mpp] might
have different XLEN settings. When csr::[mprv]=1, load and store memory
addresses are treated as though the current XLEN were set to MPP’s XLEN,
following the rules in <<xlen-control>>.
====
The csr::[sum] (permit Supervisor User Memory access) bit modifies the
privilege with which S-mode loads and stores access virtual memory.
[#norm:mstatus_sum_op]#When csr::[sum]=0, S-mode memory accesses to pages that are accessible by U-mode
(U=1 in <<sv32pte>>) will fault. When csr::[sum]=1, these accesses are permitted.#
[#norm:mstatus_sum_op_no_vm]#csr::[sum] has no effect when page-based virtual memory is not in effect.#
Note that, [#norm:mstatus_sum_op_mprv_mpp]#while csr::[sum] is ordinarily ignored when not
executing in S-mode, it _is_ in effect when csr::[mprv]=1 and csr::[mpp]=S.#
[#norm:mstatus_sum_rdonly0]#csr::[sum] is read-only 0 if S-mode is not supported or if csr:satp[mode] is read-only 0; otherwise, it must be writable.#

[[norm:mstatus_mxr_sum_op_acc_fault]]
The csr::[mxr] and csr::[sum] mechanisms only affect the interpretation of permissions
...
[#norm:mstatus_mstatush_xbe_warl]#The csr::[mbe], csr::[sbe], and csr::[ube] bits in csr:mstatus[] and csr:mstatush[] are *WARL* fields that
...
[[norm:mstatus_mbe_op]]
csr::[mbe] controls whether non-instruction-fetch memory accesses made from
M-mode (assuming csr:mstatus[mprv]=0) are little-endian (csr::[mbe]=0) or
big-endian (csr::[mbe]=1).

[[norm:mstatus_sbe_op]]
If S-mode is not supported, csr::[sbe] is read-only 0. Otherwise, csr::[sbe] controls
whether explicit load and store memory accesses made from S-mode are
little-endian (csr::[sbe]=0) or big-endian (csr::[sbe]=1).

[[norm:mstatus_ube_op]]
If U-mode is not supported, csr::[ube] is read-only 0. Otherwise, UBE controls
whether explicit load and store memory accesses made from U-mode are
little-endian (csr::[ube]=0) or big-endian (csr::[ube]=1).

[#norm:mstatus_sbe_implicit]#For _implicit_ accesses to supervisor-level memory management data
structures, such as page tables, endianness is always controlled by csr::[sbe].#
[#norm:mstatus_sbe_change_fence]#Since changing csr::[sbe] alters the implementation’s interpretation of these
data structures, if any such data structures remain in use across a
change to csr::[sbe], M-mode software must follow such a change to csr::[sbe] by
executing an insn:sfence.vma[] instruction with _rs1_=`x0` and _rs2_=`x0`.#
...
practice, csr::[sbe] will only be changed at runtime on world switches, in
...
insn:sfence.vma[] is necessary, beyond what would ordinarily be required for a
world switch.
====

[#norm:mstatus_sbe_rocopy]#If S-mode is supported, an implementation may make csr::[sbe] be a read-only
copy of csr::[mbe].#
[#norm:mstatus_ube_rocopy]#If U-mode is supported, an implementation may make csr::[ube] be a
read-only copy of either csr::[mbe] or csr::[sbe].#

[NOTE]
====
An implementation supports only little-endian memory accesses if fields
csr::[mbe], csr::[sbe], and csr::[ube] are all read-only 0. An implementation supports only
big-endian memory accesses (aside from instruction fetches) if csr::[mbe] is
read-only 1 and csr::[sbe] and csr::[ube] are each read-only 1 when S-mode and U-mode
are supported.

'''

<<sec:intro-memory>> defines a hart's address space as a circular sequence of
...
[#norm:mstatus_tvm_warl]#The csr::[tvm] (Trap Virtual Memory) bit is a *WARL* field that supports intercepting
supervisor virtual-memory management operations.# [#norm:mstatus_tvm_op]#When csr::[tvm]=1, attempts to
read or write the csr:satp[] CSR or execute an insn:sfence.vma[] or insn:sinval.vma[]
instruction while executing in S-mode will raise an illegal-instruction
exception. When csr::[tvm]=0, these operations are permitted in S-mode.# [#norm:mstatus_tvm_acc]#csr::[tvm] is
read-only 0 when S-mode is not supported or if csr:satp[mode] is read-only 0;
otherwise, it must be writable.#

[NOTE]
====
The csr::[tvm] mechanism improves virtualization efficiency by permitting guest
...
Trapping csr:satp[] accesses and the insn:sfence.vma[] and insn:sinval.vma[] instructions
provides the hooks necessary to lazily populate shadow page tables.
====

[#norm:mstatus_tw_warl]#The csr::[tw] (Timeout Wait) bit is a *WARL* field that supports intercepting the
insn:wfi[] instruction (see <<wfi>>).#
[#norm:mstatus_tw_op]#When csr::[tw]=0, the insn:wfi[] instruction may execute in modes less privileged than M when not prevented for
some other reason. When csr::[tw]=1, then if insn:wfi[] is executed in any
less-privileged mode, and it does not complete within an
implementation-specific, bounded time limit, the insn:wfi[] instruction causes
an illegal-instruction exception.#
[#norm:mstatus_tw_always_illegal]#An implementation may have insn:wfi[] always
raise an illegal-instruction exception in modes less privileged than M when
csr::[tw]=1, even if there are pending globally-disabled interrupts when the
instruction is executed.#
[#norm:mstatus_tw_acc]#csr::[tw] is read-only 0 when there are no modes less privileged than M;
otherwise, it must be writable.#

[NOTE]
====
Trapping the insn:wfi[] instruction can trigger a world switch to another guest
OS, rather than wastefully idling in the current guest.
====

[#norm:mstatus_tw_umode_op]#When S-mode is implemented, then executing insn:wfi[] in U-mode causes an
illegal-instruction exception, regardless of the value of the csr::[tw] bit, unless the
instruction completes within an implementation-specific, bounded time limit.#

[#norm:mstatus_tsr_warl]#The csr::[tsr] (Trap insn:sret[]) bit is a *WARL* field that supports intercepting the
supervisor exception return instruction, insn:sret[].#
[#norm:mstatus_tsr_op]#When csr::[tsr]=1, attempts to
execute insn:sret[] while executing in S-mode will raise an illegal-instruction
exception. When csr::[tsr]=0, this operation is permitted in S-mode.#
[#norm:mstatus_tsr_acc]#csr::[tsr] is read-only 0 when S-mode is not supported;
otherwise, it must be writable.#

[NOTE]
====
Trapping insn:sret[] is necessary to emulate the hypervisor extension (see
...
[NOTE]
====
To date, the ext:v[] extension is the only standard extension that defines
additional state beyond the floating-point CSR and data registers.
====

[#norm:mstatus_fs_vs_warl]#The csr::[fs][1:0] and csr::[vs][1:0] *WARL* fields
and the csr::[xs][1:0] read-only field are used
...
[#norm:mstatus_fs_op]#The csr::[fs] field encodes the status of the
floating-point unit state, including the floating-point registers
`f0`{endash}``f31`` and the CSRs csr:fcsr[], csr:frm[], and csr:fflags[].#
[#norm:mstatus_vs_op]#The csr::[vs] field encodes the status of the
vector extension state, including the vector
registers `v0`{endash}``v31`` and the CSRs csr:vcsr[], csr:vxrm[], csr:vxsat[], csr:vstart[],
csr:vl[], csr:vtype[], and csr:vlenb[].#
[#norm:mstatus_xs_encodes_op]#The csr::[xs] field encodes the status of
...
The csr::[fs], csr::[vs], and csr::[xs] fields use the same status encoding as shown in
...
If the ext:f[] extension is implemented, the csr::[fs] field shall not be read-only zero.

[#norm:mstatus_fs_acc2]#If neither the ext:f[] extension nor S-mode is implemented, then csr::[fs] is read-only zero.#
[#norm:mstatus_fs_rdonly0_s_no_F]#If S-mode is implemented but the ext:f[] extension is not,
csr::[fs] may optionally be read-only zero.#

[NOTE]
====
Implementations with S-mode but without the ext:f[] extension are permitted,
but not required, to make the csr::[fs] field be read-only zero. Some such
implementations will choose _not_ to have the csr::[fs] field be read-only
zero, so as to enable emulation of the ext:f[] extension for both S-mode and
U-mode via invisible traps into M-mode.
====

[[norm:mstatus_vs_acc1]]
If the `v` registers are implemented, the csr::[vs] field shall not be read-only zero.

[#norm:mstatus_vs_acc2]#If neither the `v` registers nor S-mode is implemented, then csr::[vs] is
read-only zero.#
...
csr::[xs] field is read-only zero.#
[#norm:mstatus_xs_equiv]#Every additional extension with state
provides a CSR field that encodes the equivalent of the csr::[xs] states.#
[#norm:mstatus_xs_summary_op]#The csr::[xs] field represents a summary of all extensions' status as shown in
<<norm:mstatus_fs_vs_xs_enc>>.#

[NOTE]
====
The csr::[xs] field effectively reports the maximum status value across all
user-extension status fields, though individual extensions can use a
different encoding than csr::[xs].
====

[#norm:mstatus_sd_acc]#The csr::[sd] bit is a read-only bit# that
[#norm:mstatus_sd_summary_op]#summarizes whether either the csr::[fs], csr::[vs],
or csr::[xs] fields signal the presence of some dirty state that will require
saving extended user context to memory.#
[#norm:mstatus_sd_rdonly0]#If csr::[fs], csr::[vs], and csr::[xs] are all read-only zero, then csr::[sd] is also always zero.#
...
The csr::[fs], csr::[vs], and csr::[xs] fields are read by the privileged code
before saving the context.
The csr::[fs] and csr::[vs] fields are set directly by privileged code when
resuming a user context, while the csr::[xs] field is set indirectly by
writing to the status register of the individual extensions.
[#norm:mstatus_fs_vs_xs_update_indep_priv]#The status fields are
...
[#norm:mstatus_fs_wr]#Changing the setting of csr::[fs] has no effect on the contents of the
floating-point register state. In particular, setting csr::[fs]=Off does not
destroy the state, nor does setting csr::[fs]=Initial clear the contents.# Similarly,
...
are Off and Dirty, and an attempt to set csr::[fs] to Initial or Clean causes
it to be set to Dirty.#

[NOTE]
====
This definition of csr::[fs] does not disallow setting csr::[fs] to Dirty as a result
of errant speculation. Some platforms may choose to disallow
speculatively writing csr::[fs] to close a potential side channel.
====

[#norm:mstatus_fs_no_change_dirty]#If an instruction explicitly or implicitly writes a floating-point
register or the csr:fcsr[] but does not alter its contents, and csr::[fs]=Initial
or csr::[fs]=Clean, it is implementation-defined whether FS transitions to
Dirty.#

[#norm:mstatus_vs_imprecise]#Implementations may choose to track the dirtiness of the vector register
state in an analogous imprecise fashion, including possibly setting
csr::[vs] to Dirty when software attempts to set csr::[vs]=Initial or csr::[vs]=Clean.#
[#norm:mstatus_vs_no_change_dirty]#When csr::[vs]=Initial or csr::[vs]=Clean, it is implementation-defined whether an
instruction that writes a vector register or vector CSR but does not
alter its contents causes csr::[vs] to transition to Dirty.#

<<fsxsstates>> shows all the possible state
transitions for the csr::[fs], csr::[vs], or csr::[xs] status bits. Note that the standard
...
[[fsxsstates]]
.csr::[fs], csr::[vs], and csr::[xs] state transitions
...
The csr::[xs] field provides a summary of all added extension state, but
...
[#norm:mstatus_sd_formula]#The csr::[sd] bit is read-only and is set when either the csr::[fs], csr::[vs], or csr::[xs] bits
encode a Dirty state (i.e., csr::[sd]=(csr::[fs]==0b11 OR csr::[xs]==0b11 OR csr::[vs]==0b11)).# This
...
using standard instructions (ext:f[], ext:d[], and/or ext:q[]), and privileged code must
be aware of FLEN to determine the appropriate space to reserve for each
`f` register.

Machine and Supervisor modes share a single copy of the csr::[fs], csr::[vs], and csr::[xs]
bits. Supervisor-level software normally uses the csr::[fs], csr::[vs], and csr::[xs] bits
...
[#norm:mstatus_spelp_mpelp_op]#The ext:zicfilp[] extension adds the csr::[spelp] and csr::[mpelp] fields that hold the previous
csr::[elp], and are updated as specified in <<ZICFILP_FORWARD_TRAPS>>.#
[#norm:mstatus_spelp_mpelp_enc_lead_in]#The __x__`PELP` fields are encoded as follows:#

[[norm:mstatus_spelp_mpelp_enc_list]]
* 0 - `NO_LP_EXPECTED` - no landing pad instruction expected.
* 1 - `LP_EXPECTED` - a landing pad instruction is expected.

If supervisor mode is not implemented, csr::[spelp] is read-only 0.

==== Machine Trap-Vector Base-Address (csr:mtvec[]) Register

[#norm:mtvec_sz_warl_acc]#The csr:mtvec[] register is an MXLEN-bit *WARL* read/write register that holds
trap vector configuration, consisting of a vector base address (csr::[base])
and a vector mode (csr::[mode]).#

[[norm:mtvec_enc]]
.Encoding of csr:mtvec[mode] field
include::images/bytefield/mtvec.edn[]

[#norm:mtvec_mandatory]#The csr:mtvec[] register must always be implemented#, but
[#norm:mtvec_rdonly]#can contain a read-only value.#
If csr:mtvec[] is writable, the set of values the register
may hold can vary by implementation.
[#norm:mtvec_base_align_4b]#The value in the csr::[base] field must
always be aligned on a 4-byte boundary#, and
[#norm:mtvec_base_align_func_mode]#the csr::[mode] setting may impose
additional alignment constraints on the value in the csr::[base] field.#
Note that the CSR contains only bits XLEN-1 through 2 of the address csr::[base].
...
|0 |Direct |All traps set `pc` to csr::[base].
|1 |Vectored |Asynchronous interrupts set `pc` to csr::[base]+4{times}cause.
|{ge}2 |-- |_Reserved_
|===

The encoding of the csr::[mode] field is shown in <<norm:mtvec_mode_enc>>.
[#norm:mtvec_mode_direct_op]#When csr::[mode]=Direct, all traps into
machine mode cause the `pc` to be set to the address in the csr::[base] field.#
[#norm:mtvec_mode_vectored_op]#When csr::[mode]=Vectored, all synchronous exceptions into machine mode cause
the `pc` to be set to the address in the csr::[base] field, whereas interrupts
cause the `pc` to be set to the address in the csr::[base] field plus four
times the interrupt cause number.#
For example, a machine-mode timer interrupt (see <<norm:mcause_exccode_enc_img>>) causes the `pc` to be set to csr::[base]+`0x1c`.

An implementation may have different alignment constraints for different
modes. In particular, csr::[mode]=Vectored may have stricter alignment
constraints than csr::[mode]=Direct.
...
[#norm:trap_def_m_mode]#By default, all traps at any privilege level are handled in machine
mode#, though a machine-mode handler can redirect traps back to the
appropriate level with the insn:mret[] instruction (<<otherpriv>>). To increase performance,
[#norm:medeleg_mideleg_per_bit_rw]#implementations can provide individual read/write bits within
csr:medeleg[] and csr:mideleg[] to indicate that certain exceptions and interrupts should
be processed directly by a lower privilege level.#
[#norm:medeleg_sz_acc]#The machine exception delegation register (csr:medeleg[]) is a 64-bit read/write register.#
[#norm:mideleg_sz_acc]#The machine interrupt delegation (csr:mideleg[]) register is an MXLEN-bit
read/write register.#

[#norm:medeleg_mideleg_mandatory_s_mode]#In harts with S-mode, the `medeleg` and `mideleg` registers must
exist#, and [#norm:medeleg_mideleg_delegate_op]#setting a bit in csr:medeleg[] or csr:mideleg[] will delegate the
corresponding trap, when occurring in S-mode or U-mode, to the S-mode
trap handler.#
[#norm:medeleg_mideleg_omit_wo_s_mode]#In harts without S-mode, the csr:medeleg[] and csr:mideleg[] registers should not exist.#
...
[#norm:trap_del_s_mode_op]#When a trap is delegated to S-mode,
the csr:scause[] register is written
with the trap cause; the csr:sepc[] register is written with the virtual
address of the instruction that took the trap; the csr:stval[] register is
written with an exception-specific datum; the csr::[spp] field of csr:mstatus[] is
written with the active privilege mode at the time of the trap; the
csr::[spie] field of csr:mstatus[] is written with the value of the csr::[sie] field at the
time of the trap; and the csr::[sie] field of csr:mstatus[] is cleared.#
[#norm:trap_del_s_mode_no_m_mode]#The csr:mcause[], csr:mepc[], and csr:mtval[] registers and the csr::[mpp] and csr::[mpie] fields of
csr:mstatus[] are not written.#
...
then reading back the value in csr:medeleg[] or csr:mideleg[] to see which bit
positions hold a one.#

[#norm:medeleg_no_rd1]#An implementation shall not have any bits of csr:medeleg[] be read-only one#,
...
any bits of csr:mideleg[] corresponding to machine-level interrupts#
(but [#norm:mideleg_rd1_lower_level]#may do so for lower-level interrupts#).

[NOTE]
====
Version 1.11 and earlier prohibited having any bits of csr:mideleg[] be
...
interrupt (STI) is delegated to S-mode by setting csr:mideleg[][5], STIs
will not be taken when executing in M-mode. By contrast, if csr:mideleg[][5]
is clear, STIs can be taken in any mode and regardless of current mode
will transfer control to M-mode.#

[[norm:medeleg_enc_img]]
.Machine Exception Delegation (csr:medeleg[]) register
include::images/bytefield/medeleg.edn[]

[#norm:medeleg_enc_txt]#csr:medeleg[] has a bit position allocated for every synchronous exception
shown in <<norm:mcause_exccode_enc_img>>, with the index of the
bit position equal to the value returned in the csr:mcause[] register#
(i.e., setting bit 8 allows user-mode environment calls to be delegated to a
lower-privilege trap handler).

[#norm:medelegh_sz_acc_enc_xlen32]#When XLEN=32, csr:medelegh[] is a 32-bit read/write register
that aliases bits 63:32 of csr:medeleg[].#
[#norm:medelegh_omit_xlen64]#The csr:medelegh[] register does not exist when XLEN=64.#
...
[#norm:mip_sz_acc]#The csr:mip[] register is an MXLEN-bit read/write register containing
information on pending interrupts#, while [#norm:mie_sz_acc]#csr:mie[] is the corresponding
MXLEN-bit read/write register containing interrupt enable bits.#
[#norm:mip_mie_enc_txt]#Interrupt cause number _i_ (as reported in CSR csr:mcause[],
<<mcause>>) corresponds with bit _i_ in both csr:mip[] and csr:mie[].
...
[[norm:mip_enc_img]]
.Machine Interrupt-Pending (csr:mip[]) register
include::images/bytefield/mideleg.edn[]

[[norm:mie_enc_img]]
.Machine Interrupt-Enable (csr:mie[]) register
include::images/bytefield/mideleg.edn[]
...
current privilege mode is M and the csr::[mie] bit in the csr:mstatus[] register is
set, or the current privilege mode has less privilege than M-mode;
(b) bit _i_ is set in both csr:mip[] and csr:mie[]; and (c) if register
csr:mideleg[] exists, bit _i_ is not set in csr:mideleg[].#

[#norm:intr_mip_mie_bounded_time]#These conditions for an interrupt trap to occur must be evaluated in a
bounded amount of time from when an interrupt becomes, or ceases to be,
pending in csr:mip[],# and must also [#norm:intr_mip_mie_xret_csrwr]#be evaluated immediately following the
execution of an __x__RET instruction or an explicit write to a CSR on
which these interrupt trap conditions expressly depend (including csr:mip[],
csr:mie[], csr:mstatus[], and csr:mideleg[]).#

[[norm:intr_m_mode_highest_pri]]
Interrupts to M-mode take priority over any interrupts to lower privilege modes.

[#norm:mip_mandatory]#The csr:mip[] register must always be implemented,
but can contain read-only zeros in any position, indicating that interrupt
can never become pending.#
[#norm:mip_bits_wr_or_rdonly]#Each individual bit in register csr:mip[] may be writable or may be read-only.#
[#norm:mip_bits_wr_op]#When bit _i_ in csr:mip[] is writable, a pending interrupt _i_
can be cleared by writing 0 to this bit.#
[#norm:mip_bits_rdonly_op]#If interrupt _i_ can become pending but bit _i_ in csr:mip[] is read-only, the implementation must
provide some other mechanism for clearing the pending interrupt.#

[#norm:mie_mandatory]#The csr:mie[] register must always be implemented.#
[#norm:mie_bits_wr]#A bit in csr:mie[] must be writable if the corresponding interrupt can ever become pending.#
[#norm:mie_bits_rdonly0]#Bits of csr:mie[] that are not writable must be read-only zero.#

[#norm:mip_mie_std_enc_txt]#The standard portions (bits 15:0) of the csr:mip[] and csr:mie[] registers are
...
The non-maskable interrupt is not made visible via the csr:mip[] register as
its presence is implicitly known when executing the NMI trap handler.
====

[#norm:mip_meip_mie_meie_op]#Bits csr:mip[meip] and csr:mie[meie] are the interrupt-pending and
interrupt-enable bits for machine-level external interrupts.#
[#norm:mip_meip_rdonly]#csr::[meip] is read-only in csr:mip[], and is set and cleared by a platform-specific
interrupt controller.#

[#norm:mip_mtip_mie_mtie_op]#Bits csr:mip[mtip] and csr:mie[mtie] are the interrupt-pending and
interrupt-enable bits for machine timer interrupts.#
[#norm:mip_mtip_rdonly]#csr::[mtip] is read-only in the csr:mip[] register, and is cleared by writing to
the memory-mapped machine-mode timer compare register.#

[#norm:mip_msip_mie_msie_op]#Bits csr:mip[msip] and csr:mie[msie] are the interrupt-pending and
interrupt-enable bits for machine-level software interrupts.#
[#norm:mip_msip_rdonly]#csr::[msip] is read-only in csr:mip[], and is written by accesses to memory-mapped control
registers, which are used to provide machine-level interprocessor interrupts.#

[#norm:msip_sz_acc]#A hart's memory-mapped csr:msip[] register is a 32-bit read/write register#,
where [#norm:msip_enc]#bits 31{endash}1 read as zero and bit 0 contains the csr::[msip] bit.#
[#norm:msip_update_max_time]#When the memory-mapped csr:msip[] register changes, it is guaranteed to be
reflected in csr:mip[msip] eventually, but not necessarily immediately.#
...
[#norm:mip_seip_mie_seie_op]#If supervisor mode is implemented, bits csr:mip[seip] and csr:mie[seie] are
the interrupt-pending and interrupt-enable bits for supervisor-level external interrupts.#
[#norm:mip_seip_acc]#csr::[seip] is writable in csr:mip[]#, and may be written by
...
software-writable csr::[seip] bit and the signal from the external interrupt controller.#
[#norm:mip_seip_rdcsr]#When csr:mip[] is read with a CSR instruction, the value of the
csr::[seip] bit returned in the _rd_ destination register is the logical-OR of
...
calculate the value written to csr::[seip]. Only the software-writable csr::[seip] bit
participates in the read-modify-write sequence of a insn:csrrs[] or insn:csrrc[] instruction.#

[NOTE]
====
For example, if we name the software-writable csr::[seip] bit `B` and the
signal from the external interrupt controller `E`, then if
insn:csrrs[t0,mip,t1] is executed, `t0`[9] is written with `B` || `E`, then
`B` is written with `B` || `t1`[9]. If insn:csrrw[t0,mip,t1] is executed,
then `t0`[9] is written with `B` || `E`, and `B` is simply written with
`t1`[9]. In neither case does `B` depend upon `E`.

The csr::[seip] field behavior is designed to allow a higher privilege layer to
...
[#norm:mip_stip_mie_stie_op]#If supervisor mode is implemented, its csr:mip[stip] and csr:mie[stie] are
the interrupt-pending and interrupt-enable bits for supervisor-level timer interrupts.#
[#norm:mip_stip_no_stimecmp_acc]#If the csr:stimecmp[] register is not implemented, csr::[stip] is writable in
csr:mip[]#, and [#norm:mip_stip_no_stimecmp_mmode_write]#may be written by M-mode software to deliver timer interrupts to
S-mode.# [#norm:mip_stip_stimecmp_acc]#If the csr:stimecmp[] (supervisor-mode timer compare) register is
implemented, csr::[stip] is read-only in csr:mip[]# and [#norm:mip_stip_reflects_stimecmp]#reflects the supervisor-level timer
interrupt signal resulting from csr:stimecmp[].# [#norm:mip_stip_stimecmp_clr]#This timer interrupt signal is
cleared by writing csr:stimecmp[] with a value greater than the current time value.#

[#norm:mip_ssip_mie_ssie_op]#If supervisor mode is implemented, bits csr:mip[ssip] and csr:mie[ssie] are
the interrupt-pending and interrupt-enable bits for supervisor-level software interrupts.#
[#norm:mip_ssip_acc]#csr::[ssip] is writable in csr:mip[]# and [#norm:mip_ssip_intr_ctrl]#may also be set to 1
by a platform-specific interrupt controller.#

[#norm:mip_lcofip_mie_lcofie_op]#If the ext:sscofpmf[] extension is implemented, bits csr:mip[lcofip] and
csr:mie[lcofie] are the interrupt-pending and interrupt-enable bits for local-counter-overflow interrupts.#
[#norm:mip_lcofip_acc]#csr::[lcofip] is read-write in csr:mip[]# and [#norm:mip_lcofip_reflects_overflow]#reflects the occurrence of a local
counter-overflow overflow interrupt request resulting from any of the `mhpmevent__n__.OF` bits being set.#
[#norm:mip_lcofip_mie_lcofie_rdonly0]#If the ext:sscofpmf[] extension is not implemented, csr:mip[lcofip] and csr:mie[lcofie] are
read-only zeros.#

[#norm:intr_m_mode_pri]#Multiple simultaneous interrupts destined for M-mode are handled in the
...
Restricted views of the csr:mip[] and csr:mie[] registers appear as the csr:sip[]
and csr:sie[] registers for supervisor level. If an interrupt is delegated
to S-mode by setting a bit in the csr:mideleg[] register, it becomes visible
in the csr:sip[] register and is maskable using the csr:sie[] register.
Otherwise, the corresponding bits in csr:sip[] and csr:sie[] are read-only zero.

==== Hardware Performance Monitor

M-mode includes a basic hardware performance-monitoring facility.
[#norm:mcycle_op]#The csr:mcycle[] CSR counts the number of clock cycles executed by the processor
core on which the hart is running.#
[#norm:minstret_op]#The csr:minstret[] CSR counts the number of instructions the hart has retired.#
[#norm:mcycle_minstret_sz]#The csr:mcycle[] and csr:minstret[] registers have 64-bit precision on all RV32 and RV64 harts.#
...
[#norm:mcycle_shared]#The csr:mcycle[] CSR may be
shared between harts on the same core, in which case writes to
csr:mcycle[] will be visible to those harts.# The platform should provide a mechanism
to indicate which harts share an csr:mcycle[] CSR.

[#norm:mhpmcounter_num]#The hardware performance monitor includes 29 additional 64-bit event
counters, csr:mhpmcounter3[]{endash}csr:mhpmcounter31[].#
[#norm:mhpmevent_sz_warl_op]#The event selector CSRs,
csr:mhpmevent3[]{endash}csr:mhpmevent31[], are 64-bit *WARL* registers that control which
...
[#norm:mhpmcounter_warl]#The ``mhpmcounter``s are *WARL* registers# that
[#norm:mhpmcounter_sz]#support up to 64 bits of precision on RV32 and RV64.#

When XLEN=32, reads of the csr:mcycle[], csr:minstret[], `mhpmcounter__n__`, and `mhpmevent__n__`
CSRs return bits 31-0 of the corresponding register, and writes change only bits 31-0;
[#norm:mcycleh_minstreth_mhpmh_op]#reads of the csr:mcycleh[], csr:minstreth[], `mhpmcounter__n__h`, and `mhpmevent__n__h`
CSRs return bits 63-32 of the corresponding register, and writes change only bits 63-32.#
[#norm:mhpmeventh_presence]#The `mhpmevent__n__h` CSRs are provided only if the ext:sscofpmf[] extension is implemented.#

[[mcounteren]]
==== Machine Counter-Enable (csr:mcounteren[]) Register

[#norm:mcounteren_sz]#The counter-enable csr:mcounteren[] register is a 32-bit register# that
...
[#norm:mcounteren_clr_ill_inst_exc]#When the csr::[cy], csr::[tm], csr::[ir], or HPM__n__ bit in the csr:mcounteren[] register is
clear, attempts to read the csr:cycle[], csr:time[], csr:instret[], or
`hpmcounter__n__` register while executing in S-mode or U-mode will cause an
illegal-instruction exception.#
...
In addition, [#norm:mcounteren_tm_clr]#when the csr:[tm] bit in the csr:mcounteren[] register is clear, attempts to
access the csr:stimecmp[] or csr:vstimecmp[] register while executing in a mode less
privileged than M will cause an illegal-instruction exception.#
[#norm:mcounteren_tm_set]#When this bit is set, access to the csr:stimecmp[] or csr:vstimecmp[] register is permitted in S-mode
if implemented, and access to the csr:vstimecmp[] register (via csr:stimecmp[]) is
permitted in VS-mode if implemented and not otherwise prevented by the csr::[tm] bit in
csr:hcounteren[].#

[#norm:cycle_instret_hpmcounter_op_rdonly]#The csr:cycle[], csr:instret[], and `hpmcounter__n__` CSRs are read-only shadows of csr:mcycle[], csr:minstret[], and `mhpmcounter__n__`, respectively.#
[#norm:time_op_rdonly]#The csr:time[] CSR is a read-only shadow of the memory-mapped csr:mtime[] register.#
Analogously, [#norm:cycleh_instreth_hpmcounternh_op_rdonly]#when XLEN=32, the csr:cycleh[], csr:instreth[] and `hpmcounter__n__h` CSRs
are read-only shadows of csr:mcycleh[], csr:minstreth[] and `mhpmcounter__n__h`, respectively.#
[#norm:timeh_op_rdonly]#When XLEN=32, the csr:timeh[] CSR is a read-only shadow of the
upper 32 bits of the memory-mapped csr:mtime[] register#, while csr:time[]
shadows only the lower 32 bits of csr:mtime[].

[NOTE]
====
[#norm:time_csr_architectural_availability]#Implementations can convert reads of the csr:time[] and csr:timeh[] CSRs into
loads to the memory-mapped csr:mtime[] register, or emulate this
functionality on behalf of less-privileged modes in M-mode software.#
====

[#norm:mcounteren_flds_mandatory_warl]#In harts with U-mode, the csr:mcounteren[] must be
...
[#norm:mcounteren_presence]#In harts without U-mode, the csr:mcounteren[] register should not exist.#
...
[#norm:mcountinhibit_sz_warl]#The counter-inhibit register csr:mcountinhibit[] is a 32-bit
...
[#norm:mcountinhibit_clear_op]#When the csr::[cy], csr::[ir], or HPM__n__ bit in the csr:mcountinhibit[] register is clear,
the csr:mcycle[], csr:minstret[], or `mhpmcounter__n__` register increments as usual.
When the csr::[cy], csr::[ir], or HPM__n__ bit is set, the corresponding counter does
not increment.#

[#norm:mcountinhibit_cy_shared]#The csr:mcycle[] CSR may be shared between harts on the same core,
in which case the csr:mcountinhibit[cy] field is also shared between those harts,
and so writes to csr:mcountinhibit[cy] will be visible to those harts.#

[#norm:mcountinhibit_not_impl]#If the csr:mcountinhibit[] register is not implemented, the implementation
behaves as though the register were set to zero.#

[NOTE]
====
When the csr:mcycle[] and csr:minstret[] counters are not needed, it is desirable
...
Because the csr:mtime[] counter can be shared between multiple cores, it
cannot be inhibited with the csr:mcountinhibit[] mechanism.
====
...
for use by machine mode.  The `mscratch` register must be implemented.#
Typically, it is used to hold a pointer to a
...
[#norm:mepc_warl]#`mepc` is a *WARL* register that must be implemented and
must be able to hold all valid virtual
...
shown in <<norm:mcause_enc_img>>. [#norm:mcause_mandatory]#The `mcause` register must be implemented.#
...
[#norm:mcause_exccode_priority_order]#If an instruction may raise multiple synchronous exceptions, the
decreasing priority order of <<norm:exc_priority>> indicates which
exception is taken and reported in `mcause`.#
[#norm:custom_exc_priority]#The priority of any custom synchronous exceptions is implementation-defined.#
...
[#norm:mcause_exccode_priority_misaligned]#Load/store/AMO address-misaligned exceptions may have either higher or
...
[#norm:mtval_rdonly0]#The `mtval` register must always be implemented. If the
hardware platform specifies that no exceptions set `mtval`
to a nonzero value, then `mtval` is read-only zero.#

[#norm:mtval_vaddr_wr_faulting_addr]#If `mtval` is written with a nonzero value when a breakpoint,
...
[#norm:mtval_ebreak_op]#On a breakpoint exception raised by an EBREAK or C.EBREAK instruction, `mtval`
...
[#norm:mtval_vaddr_wr_misaligned_addr]#If `mtval` is written with a nonzero value when a misaligned load or
...
[#norm:mtval_instr_bits_lead_in]#The `mtval` register can optionally also be used to return the faulting
...
[#norm:mtval_swchk_lead_in]#On a trap caused by a software-check exception, the `mtval` register holds
...
8{times}__n__, then `mconfigptr`[log~2~n-1:0] must be zero.#
...
[#norm:menvcfg_stce_exists]#The Sstc extension adds the `STCE` (STimecmp Enable) bit to `menvcfg` CSR.#
[#norm:menvcfg_stce_rdonly0]#When the Sstc extension is not implemented, `STCE` is read-only zero.#
[#norm:menvcfg_stce_enable_op]#The `STCE` bit enables `stimecmp` for S-mode when set to one.#
[#norm:menvcfg_stce_zero_op]#When this extension is implemented and `STCE` in `menvcfg` is zero, an attempt
...
implemented.# [#norm:menvcfg_stce_zero_vstip]#Further, if the H extension is implemented, then `hip`.VSTIP also
...
[#norm:menvcfg_cbie_cbo-inval_op_lead_in]#When `CBIE` is set to `0b01`
...
[#norm:menvcfg_lpe_op_lead_in]#The Zicfilp extension adds the `LPE` field in `menvcfg`. When the `LPE` field is
...
[#norm:menvcfg_sse_op_lead_in]#The Zicfiss extension adds the `SSE` field to `menvcfg`.
...
implemented.# [#norm:menvcfg_dte_sdt_rdonly0]#When Ssdbltrp is not implemented `sstatus.SDT`, `vsstatus.SDT`, and
...
[#norm:menvcfg_mandatory_u_mode]#In harts with U-mode, `menvcfg` must be
implemented, but all `WARL` fields may be read-only zero.#
[#norm:menvcfgh_mandatory_s_mode_rv32]#For harts with S-mode and XLEN=32, `menvcfgh` must be similarly implemented.#
[#norm:menvcfg_menvcfgh_no_u_mode]#If U-mode is not supported, then registers `menvcfg` and `menvcfgh` do
not exist.#
...
[#norm:mseccfg_useed_u_mode_op]#When `USEED` is 0, access to the `seed` CSR in U-mode raises an
...
[#norm:mseccfg_sseed_sorhs_mode_op]#When `SSEED` is 0, access to the `seed` CSR from S-/HS-mode raises an
...
[#norm:mseccfg_sseed_vsorvu_mode_op]#When the H extension is also implemented,
...
. [#norm:pmp_rw01_rsv]#If `mseccfg.MML` is not set, the combination of `pmpcfg.RW=01` remains
...
[#norm:mseccfg_mlpe_clr_op_lead_in]#When the `MLPE` field is 0, the Zicfilp extension is not enabled
...
[[norm:ebreak_debugger_transfer_op]]
...
[NOTE]
====
The insnlink:c.ebreak[] instruction performs the same
...
[#norm:xret_clr_lr_resv]#If the Zalrsc extension is supported, the __x__RET instruction is allowed to
...
pending but not locally enabled).# [#norm:wfi_resume_any_priv]#WFI is also required to resume
...
[#norm:m_mode_priv_at_reset]#Upon reset, a hart’s privilege mode is set to M.#
...
[#norm:misa_extensions_reset_ref]#The `misa` register is reset to enable the maximal set of supported
extensions, as described in <<misa>>.#
[#norm:ld_rsv_rst]#For implementations with the Zalrsc standard extension, there is no valid
load reservation.#
...
[#norm:rst_other_state_unspec]#All other hart state is UNSPECIFIED.#
...
the instruction that was interrupted,# and [#norm:nmi_mcause_source]#`mcause` is set to a value
indicating the source of the NMI.# The NMI can thus overwrite state in an
active machine-mode interrupt handler.

[#norm:nmi_mcause_impl_def]#The values written to `mcause` on an NMI are implementation-defined.#
...
described in <<pmp>>, [#norm:pma_indep_exec_context]#PMAs do not vary by execution
...
region, and can be hardwired into the PMA checker.
Where the attributes are run-time configurable,
...
[#norm:pma_precise_recom]#Precisely trapped PMA violations manifest as instruction, load, or store
...
required by the attached devices#, and [#norm:pma_mm_ifetch]#can specify whether instruction fetch is supported.#
...
In some cases, [#norm:pma_mm_mandatory_sz]#the design of a processor or device accessing main memory
...
[#norm:pma_rsrv_levels]#For _LR/SC_, there are three levels of support indicating combinations
...
[[sec:misaligned-atomicity-granule]]
...
NOTE: [#norm:pma_mag_op_rsrv]#LR/SC instructions are unaffected by this PMA and so always raise an
...
the I/O Ordering section in <<app:mm-explanatory>>.
...
are idempotent. [#norm:pma_idp_mm]#Main memory regions are assumed to be idempotent.#
For I/O regions, idempotency on reads and writes can be specified separately
...
[NOTE]
====
While [#norm:pma_idp_no_spec]#hardware should always be designed to avoid speculative or
...
The results of these additional reads
...
[#norm:pmp_load_fault]#Attempting to execute a load or load-reserved instruction that accesses
a physical address within a PMP region without read permissions raises a
load access-fault exception.# [#norm:pmp_store_fault]#Attempting to execute a store,
store-conditional, AMO, cache-block management, or cache-block zero instruction that accesses a physical address
...
[#norm:pmp_locked_until_reset]#Locked PMP entries remain locked until the hart is reset.# [#norm:pmp_l_bit_write_protection]#If PMP entry
