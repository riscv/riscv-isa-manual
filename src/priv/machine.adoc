...
supervisor virtual-memory management operations. When csr::[tvm]=1, attempts to
...
exception. When csr::[tvm]=0, these operations are permitted in S-mode. csr::[tvm] is
read-only 0 when S-mode is not supported, or if csr:satp[mode] is read-only 0
and the `Sspmp` extension is not implemented; otherwise, it must be writable.
...
[#norm:trap_del_S_mode_op]#the csr:scause[] register is written
...
[#norm:mip_bits_wr_or_rdonly]#The csr:mip[] register must always be implemented,
but can contain read-only zeros in any position, indicating that interrupt
can never become pending.
Each individual bit in register csr:mip[] may be writable or may be read-only.#
...
[#norm:mie_bits_wr]#The csr:mie[] register must always be implemented.
A bit in csr:mie[] must be writable if the corresponding interrupt can ever become pending.#
...
[#norm:menvcfg_menvcfgh_no_U_mode]#In harts with U-mode, `menvcfg` must be
implemented, but all `WARL` fields may be read-only zero.
For harts with S-mode and XLEN=32, `menvcfgh` must be similarly implemented.
...
system debugging, [#norm:pma_precise_recom]#we strongly recommend that, where possible, RISC-V
