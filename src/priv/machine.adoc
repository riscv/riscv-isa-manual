...
[#norm:mstatus_mie_sie_op1]#Global interrupt-enable bits, csr::[mie] and csr::[sie], are provided for M-mode and
...
[#norm:mstatus_sum_op_no-vm]#csr::[sum] has no effect when page-based virtual memory is not in effect.#
...
[#norm:mstatus_xs_op1]#The csr::[xs] field encodes the status of
...
[#norm:mstatus_xs_op2]#The csr::[xs] field represents a summary of all extensions' status as shown in
<<norm:mstatus_fs_vs_xs_enc>>.#
...
[#norm:mstatus_sd_op1]#summarizes whether either the csr::[fs], csr::[vs],
...
[#norm:medeleg_mideleg_op1]#implementations can provide individual read/write bits within
...
exist#, and [#norm:medeleg_mideleg_op2]#setting a bit in csr:medeleg[] or csr:mideleg[] will delegate the
...
csr:mip[]#, and [#norm:mip_stip_no_stimecmp_op2]#may be written by M-mode software to deliver timer interrupts to
S-mode.# [#norm:mip_stip_stimecmp_acc]#If the csr:stimecmp[] (supervisor-mode timer compare) register is
implemented, csr::[stip] is read-only in csr:mip[]# and [#norm:mip_stip_stimecmp_op2]#reflects the supervisor-level timer
...
[#norm:mip_lcofip_acc]#csr::[lcofip] is read-write in csr:mip[]# and [#norm:mip_lcofip_op2]#reflects the occurrence of a local
...
[#norm:mcountinhibit_sz_warl_op1]#The counter-inhibit register csr:mcountinhibit[] is a 32-bit
...
[#norm:mcountinhibit_op2]#When the csr::[cy], csr::[ir], or HPM__n__ bit in the csr:mcountinhibit[] register is clear,
