...
An insnlink:mret[] or insnlink:sret[] instruction is used to return from a trap in M-mode or S-mode respectively.
...
When the ext:smdbltrp[] extension is implemented, executing an insnlink:mret[] instruction, or executing
an insnlink:sret[] instruction while the current privilege mode is M, has the following additional
effects.
...
[#norm:mstatus_mdt_clr_mnret]#When the ext:smdbltrp[] extension is implemented, the insnlink:mnret[] instruction, provided by the ext:smrnmi[] extension, sets the csr::[mdt] bit to 0 if the new privilege mode is not M.#
...
[#norm:mstatus_mprv_clr_mret_sret_less_priv]#An insnlink:mret[] or insnlink:sret[] instruction that changes the privilege mode to a mode
...
executing an insnlink:sfence.vma[] instruction with _rs1_=`x0` and _rs2_=`x0`.#
...
insnlink:sfence.vma[] is necessary, beyond what would ordinarily be required for a
world switch.
====
...
trap handler.#
[#norm:medeleg_mideleg_omit_wo_S_mode]#In harts without S-mode, the `medeleg` and `mideleg` registers should not exist.#
...
[#norm:trap_del_S_mode_op]#the `scause` register is written
...
[#norm:trap_del_S_mode_no_M_mode]#The `mcause`, `mepc`, and `mtval` registers and the MPP and MPIE fields of
...
[#norm:mip_stip_no_stimecmp_acc]#If the stimecmp register is not implemented, STIP is writable in
mip#, and [#norm:mip_stip_no_stimecmp_op2]#may be written by M-mode software to deliver timer interrupts to
