...
The csr::[fs] and csr::[xs] fields are read by the privileged code before saving the
context. The csr::[fs] field is set directly by privileged code when resuming a
user context, while the csr::[xs] field is set indirectly by writing to the
