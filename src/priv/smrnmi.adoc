[[rnmi]]
=== ext:smrnmi[] Extension for Resumable Non-Maskable Interrupts
...
NOTE: This requirement applies to the architectural behavior defined by Smrnmi. A custom mechanism that masks RNMIs while it is active is not conformant to Smrnmi. A gate implemented outside the hart is an integration-level platform policy, not a software control defined by Smrnmi.
...
encoded in the same manner as `mstatus`.MPP.# [#norm:mnstatus_mnpv_op]#If the H extension is also
implemented, `mnstatus` also holds a one-bit
...
[#norm:mnstatus_wfi]#For the purposes of the WFI instruction, NMIE is a global
interrupt enable, meaning that the setting of NMIE does not affect which
interrupts are locally enabled.
Furthermore, RNMIs are always considered to be locally enabled; hence, WFI is
required to resume execution if an RNMI becomes pending when NMIE=0,
or to take the interrupt if an RNMI becomes pending when NMIE=1.#
...
[#norm:rnmi_exception_nmie0]#When the Smdbltrp extension is not implemented, if the hart encounters an exception while executing in M-mode with the `mnstatus`.NMIE bit clear, the actions taken are the same as if the exception had occurred while `mnstatus`.NMIE were set, except that the program counter is set to the RNMI exception trap handler address.#
The behavior when the Smdbltrp extension is implemented is described in <<machine-double-trap>>.
