...
[#norm:mnscratch_sz_acc_op]#The `mnscratch` CSR holds an MXLEN-bit read-write register which enables
...
[#norm:mnepc_sz_acc_op]#The `mnepc` CSR is an MXLEN-bit read-write register which on entry to
