...
On RV32 systems, [#norm:spmpaddr_format_rv32]#each `spmpaddr` register encodes a 34-bit physical address from bit 33 down to bit 2, as illustrated in <<spmpaddr-rv32>>.#
On RV64 systems, [#norm:spmpaddr_format_rv64]#each `spmpaddr` register encodes a 56-bit physical address from bit 55 down to bit 2, as shown in <<spmpaddr-rv64>>.#
...
[#norm:addr_match_tor_entry0]#Particularly, if `spmpcfg[0].A` is set to TOR, zero is used for the lower bound.#
...
+
. The encodings [#norm:rwx_010_reserved]#`spmpcfg.RWX=010`#, [#norm:rwx_011_reserved]#`spmpcfg.RWX=011`#, as well as the combination `spmpcfg.SHARED == 1 and spmpcfg.U == 0` are all reserved for future standard use.
...
[#norm:siselect_oob_read_zero]#An access to an out-of-bounds index via `siselect` will return zero on read# and [#norm:siselect_oob_write_ignored]#be ignored on write.#
...
The `Sspmp` extension reuses the existing page fault exception codes for SPMP violations.
...
|0|12|[#norm:spmp_instr_page_fault]#Instruction page fault#
|0|13|[#norm:spmp_load_page_fault]#Load page fault#
|0|15|[#norm:spmp_store_page_fault]#Store/AMO page fault#
...
* [#norm:spmpen_readwrite]#For RV64 architectures, it introduces a 64-bit WARL CSR, `spmpen`.#
...
In an implementation featuring 64 PMP entries with N (`N {le} 64`) delegated to the supervisor level, the bits `spmpen[0..N-1]` control their respective SPMP entries `[0..N-1]`.
Any attempt to write to the upper bits of the register are ignored.
...
For the purposes of this specification, `Smpmpdeleg` assumes an architectural space of 64 PMP entries, although fewer entries may be writable; any non-writable entries behave as read-only zero.
...
. Accesses to out-of-range indices, such as reading `PMP[N]` or writing to `SPMP[64-N]` in this scenario, are handled as follows: reads return zero, and writes are ignored.
...
. As `pmpnum` is reconfigured, SPMP entries with the largest indices change.
