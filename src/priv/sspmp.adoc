[[sspmp]]
== Supervisor-Level PMP Extensions, Version 1.0
...
+
. [#norm:sspmp_includes_sfence_vma]#The `Sspmp` extension includes the `SFENCE.VMA` instruction.#
...
|`satp.MODE=Bare` and `Sspmp` implemented |SPMP only
|`satp.MODE=Bare` and `Sspmp` not implemented |No S-mode protection checks
...
[#norm:sspmp_sfence_vma_ordering]#To enforce ordering for subsequent accesses whose effective privilege mode is
