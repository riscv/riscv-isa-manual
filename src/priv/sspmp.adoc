...
* For RV64 architectures, it introduces a 64-bit WARL CSR, `spmpen`.
* For RV32 architectures, it introduces an additional 32-bit WARL CSR, `spmpenh`, which serves as an alias for the upper 32 bits of the `spmpen` register.
...
The formal condition for this activation is `spmpen[i] & spmpcfg[i].A != 0`.
