[[sspmp]]
== Supervisor-Level PMP Extensions, Version 0.9.1
...
|0|OFF|[#norm:csrfld:spmpcfg:A:match_off]#Null region (disabled)#
|1|TOR|[#norm:csrfld:spmpcfg:A:match_tor]#Top of range#
|2|NA4|[#norm:csrfld:spmpcfg:A:match_na4]#Naturally aligned four-byte region#
|3|NAPOT|[#norm:csrfld:spmpcfg:A:match_napot]#Naturally aligned power-of-two region, ≥8 bytes#
|===
...
+
* [#norm:ext:Sspmp:umode_rule_sum_effect]#With `sstatus.SUM` set, the rule is enforced for Supervisor mode data accesses,# but [#norm:ext:Sspmp:umode_rule_enforceNoX]#execution permission is denied (termed *EnforceNoX* in <<spmpencode>>)#. This prevents the OS from executing code residing in memory assigned to an unprivileged process.
+
* [#norm:ext:Sspmp:umode_rule_sum_denied]#With `sstatus.SUM` clear, the rule is denied for any Supervisor mode access.# This prevents the OS from accessing the memory of an unprivileged process unless a specific code path is followed.
+
...
+
. The encodings [#norm:csrfld:spmpcfg:RWX:rwx_010_reserved]#`spmpcfg.RWX=010`#, [#norm:csrfld:spmpcfg:RWX:rwx_011_reserved]#`spmpcfg.RWX=011`#, as well as the combination `spmpcfg.SHARED == 1 and spmpcfg.U == 0` are all reserved for future standard use.
...
+
. [#norm:ext:Sspmp:no_match_deny]#If the effective privilege mode of the access is S-mode or U-mode and no SPMP entry matches, but at least one SPMP entry is implemented, the access is `denied`.#
+
...
The registers `sireg3` through `sireg6` are reserved (i.e., reads return zero, writes are ignored).

SPMP entries are indexed starting from zero.
For instance, in a system with N SPMP entries (asciimath:[N \le 64]),
`SPMP[0..N-1]` is accessed by writing `0x100+i` to the `siselect` register, where `i` represents the entry index (asciimath:[0 \le i \le N-1]).
...
[#norm:ext:Sspmp:sfence_vma_ordering]#To enforce ordering for subsequent accesses whose effective privilege mode is
...
The `Sspmp` extension co-opts the existing page fault exception codes for SPMP violations, as page fault exceptions are commonly delegated to S-mode.
...
|0|12|[#norm:ext:Sspmp:instr_page_fault]#Instruction page fault#
|0|13|[#norm:ext:Sspmp:load_page_fault]#Load page fault#
|0|15|[#norm:ext:Sspmp:store_page_fault]#Store/AMO page fault#
...
* [#norm:ext:Sspmpen:spmpen_readwrite]#For RV64 architectures, it introduces a 64-bit WARL CSR, `spmpen` (CSR number 0x183).#
* [#norm:ext:Sspmpen:spmpenh_alias]#For RV32 architectures, it introduces an additional 32-bit WARL CSR, `spmpenh` (CSR number 0x193), which serves as an alias for the upper 32 bits of the `spmpen` register.#


The activation of each SPMP entry is governed by its corresponding bit in the `spmpen` register.
[#norm:ext:Sspmpen:activation_condition]#An SPMP entry `i` is considered active only when both the `spmpen[i]` bit is set and the `spmpcfg[i].A` field is non-zero.#
...
In an implementation featuring 64 PMP entries with N (asciimath:[N \le 64]) delegated to the supervisor level, the bits `spmpen[0..N-1]` control their respective SPMP entries `[0..N-1]`.
...
This delegation is managed by an MXLEN-bit M-mode CSR named `mpmpdeleg` (CSR number 0x316), whose layout is detailed in <<mpmpdeleg-format-rv64>>.
...
For example, in an implementation with 64 total entries where `pmpnum` is configured to N (asciimath:[N \le 64]):
...
For instance, raising `pmpnum` from N to N+2 truncates SPMP and spmpen ranges from `SPMP[0..63-N]`, `spmpen[0..63-N]` to `SPMP[0..61-N]`, `spmpen[0..61-N]`.
...
The `mireg3` through `mireg6` are reserved (i.e., reads return zero, writes are ignored).
...
For instance, if N (asciimath:[N \le 64]) out of 64 entries are delegated,
`SPMP[0..N-1]` is accessed by writing `0x100+i` to the corresponding `xiselect` register (`siselect` for S-mode, `siselect` and `miselect` for M-mode), where `i` represents the entry index (asciimath:[0 \le i \le N-1]).
Any access attempt by either mode to an index outside this range (i.e., `asciimath:[i \ge N]`) results in a read of zero, and writes are ignored.
