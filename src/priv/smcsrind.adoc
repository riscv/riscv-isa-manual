[[indirect-csr]]
=== ext:smcsrind[] and ext:sscsrind[] Extensions for Indirect CSR Access
...
extension Smstateen in <<smstateen>>.
...
[NOTE]
====
CSR address space is reserved for a possible future ext:sucsrind[] extension that extends indirect CSR access to user mode.
====
