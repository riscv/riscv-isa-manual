...
The execution environment is responsible for ensuring eventual
forward progress of each of its harts --- until their possible
termination and while not exercising a mechanism that explicitly
waits for an event, such as the WFI instruction waiting for interrupts.
Forward progress includes:
\vspace{-0.2in}
\begin{itemize}
\parskip 0pt
\itemsep 1pt
\item Completing the execution of an instruction.
\item Other events defined by extensions, such as traps.
\end{itemize}
