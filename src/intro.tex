...
forward progress of each of its harts.  Forward progress includes:
\begin{itemize}
\item Completing the execution of an instruction.
\item Other events defined by extensions, such as traps.
\item Mechanisms that explicitly wait for an event, such as the WFI
  instruction waiting for interrupts, or various environment calls.
\end{itemize}
