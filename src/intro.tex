...
provided that the region was reached non-speculatively. Such a region is reached
non-speculatively if a simple instruction-by-instruction execution performs a
non-speculative instruction fetch in that region via sequential code, including
fetching an instruction that spans adjacent regions, or via a branch, jump, or
trap. By implication, an implementation may always cache any data read by an
instruction fetch.
...
power-of-2 region. This region does need not be the same region as the related
