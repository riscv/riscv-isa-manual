...
Except when specified otherwise, an implicit instruction fetch that does not
raise an exception may be performed arbitrarily early and speculatively, i.e.
before the instruction fetch is required by a simple instruction-by-instruction
execution of a program. An instruction fetch accessing main memory may read any
number of bytes and may be performed at any time. For instance, a valid
...
again. An instruction fetch accessing I/O devices, however, may only read the
bytes within an implementation-defined, naturally aligned power-of-2 region
containing the address of the instruction fetch, provided that the region was
reached non-speculatively. Such a region is reached non-speculatively if a
simple instruction-by-instruction execution performs a non-speculative
instruction fetch in that region via sequential code (including fetching an
instruction that spans adjacent regions) or via a branch, jump, or trap. By
implication, an implementation may always cache any data read by an
instruction fetch.

(Fetch a bounded number of NAPOT regions, and continue executing within those
regions)

(The practical restriction is that an implementation that limits speculation 
can fetch from any memory without PMAs, but if an implementation knows where
executable memory is, it can fetch more.)

In addition, except when specified otherwise, other implicit reads, e.g.
reads of an address translation data structure, that do not raise an exception
may be performed arbitrarily early and speculatively. An implicit read
accessing main memory may read any number of bytes and may be performed at any
time. An implicit read accessing I/O devices, however, may only read the bytes
within an implementation-defined, naturally aligned power-of-2 region containing
the address of the implicit read, provided that the instruction fetch for the
corresponding instruction has been performed as described above.
This region does need not be the same region as the related instruction fetch
region, and the size of this region does not need to be the same as the size of
the related instruction fetch region. By implication, an implementation may
always cache any data read by an implicit read.

To ensure that certain implicit reads are ordered only after writes to
