...
Except when specified otherwise, an implicit read for an instruction fetch that
does not raise an exception may
be performed arbitrarily early and speculatively, i.e. before the instruction
fetch is required by a simple instruction-by-instruction execution of a program.
An instruction fetch accessing main memory may
read any number of bytes and may be performed at any time. For instance, a
valid implementation could attempt to read all of main memory at the earliest
...
again. An instruction fetch accessing I/O
devices, however, may only read the bytes within an implementation-defined
naturally-aligned power-of-2 region, provided that the region was reached
non-speculatively. Such a region is reached non-speculatively if a simple
instruction-by-instruction execution performs a non-speculative instruction
fetch in that region via sequential code, including fetching an instruction that
spans adjacent regions, or via a branch, jump, or trap. By implication, an
implementation may cache any data read by an instruction fetch.

In addition, except when specified otherwise, any additional implicit read that
results from an instruction fetch described above and that does not
raise an exception, e.g. a read of an address translation data
structure, may be performed arbitrarily early and speculatively. An implicit
read accessing main memory may read any number
of bytes, whereas an implicit read accessing I/O
devices may only read the bytes within an implementation-defined naturally-aligned
power-of-2 region. This region does need not be the same as the related
instruction fetch region, and the size of this region does not need to be the
same as the size of the related instruction fetch region. By implication, an
implementation may cache any data read by an implicit read.
