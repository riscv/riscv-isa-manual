...
In addition, except when specified otherwise, any additional implicit read that
results from an instruction fetched as described above and that does not raise
an exception, e.g. a read of an address translation data structure, may be
performed arbitrarily early and speculatively. An implicit read accessing main
memory may read any number of bytes, whereas an implicit read accessing I/O
devices may only read the bytes within an implementation-defined,
naturally-aligned power-of-2 region containing the address of the implicit read.
