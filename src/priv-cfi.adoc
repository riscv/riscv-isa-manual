...
[#norm:elp_prior_trap#]#In such cases, the `ELP` prior to the trap, the previous `ELP`, must be
preserved by the trap delivery such that it can be restored on a return from the
trap.# [#norm:elp_mstatus_mpelp#]#To store the previous `ELP` state on trap delivery to M-mode, an `MPELP`
bit is provided in the `mstatus` CSR.# [#norm:elp_mstatus_spelp#]#To store the previous `ELP` state on trap
delivery to S/HS-mode, an `SPELP` bit is provided in the `mstatus` CSR.# The
`SPELP` bit in `mstatus` can be accessed through the `sstatus` CSR. [#norm:elp_vsstatus_spelp#]#To store
the previous `ELP` state on traps to VS-mode, a `SPELP` bit is defined in the
`vsstatus` (VS-modes version of `sstatus`).# [#norm:elp_debug_mode#]#To store the previous `ELP` state on
...
shadow stack instructions will raise a store/AMO access-fault exception.# [#norm:ssmp_ssamoswap#]#When
...
instruction initiated memory accesses. [#norm:ssmp_ss_mxr#]#The state of the `MXR` bit does not
...
extension. [#norm:ssmp_ss_xwr#]#The `xwr == 010b` encoding in the G-stage PTE remains reserved.# [#norm:active_g_stage_pte]#When
