...
This chapter describes the proposed _vector_ _4-element_ _dot_ _product_ extensions for RISC-V.
...
Note:: there is no constraint on XLEN but if XLEN < SEW the `.vx` variants may not be very useful (since the scalar element is zero extended to SEW bits, regardless of the signedness of the operation).

[[ext:zvdot4a8i]]
=== ext:zvdot4a8i[] Extension for Vector 4-Element Dot Product Accumulate of packed 8-bit Integers, Version 0.1
...
[[norm:zvdot4a8i_depends_zve32x]]
ext:zvdot4a8i[] depends on `Zve32x`.
...
image::zvdot4a8i_vdot4a_vx.svg[width=60%,align=center]

.Vector Vector data layout
image::zvdot4a8i_vdot4a_vv.svg[width=60%,align=center]
...
[[vector_dot_product_4a_insns, reftext="Instructions"]]
=== Instructions
...
function clause execute (VDOT4A(vs2, vs1, vd, suffix)) = {
  foreach (i from vstart to vl-1) {
    ...
    let res : bits (SEW) = sign_extend(a0) * sign_extend(b0)
                         + sign_extend(a1) * sign_extend(b1)
                         + sign_extend(a2) * sign_extend(b2)
                         + sign_extend(a3) * sign_extend(b3)
                         + acc;
    set_velem(vd, i, res);
  }
  RETIRE_SUCCESS
...
function clause execute (VDOT4AU(vs2, vs1, vd, suffix)) = {
  foreach (i from vstart to vl-1) {
    ...
    let acc : bits (SEW) = get_velem(vd, i);
    let res : bits (SEW) = zero_extend(a0) * zero_extend(b0)
                         + zero_extend(a1) * zero_extend(b1)
                         + zero_extend(a2) * zero_extend(b2)
                         + zero_extend(a3) * zero_extend(b3)
                         + acc;
    set_velem(vd, i, res);
  }
  RETIRE_SUCCESS
...
function clause execute (VDOT4ASU(vs2, vs1, vd, suffix)) = {
  foreach (i from vstart to vl-1) {
    ...
    let acc : bits (SEW) = get_velem(vd, i);
    let res : bits (SEW) = sign_extend(a0) * zero_extend(b0)
                         + sign_extend(a1) * zero_extend(b1)
                         + sign_extend(a2) * zero_extend(b2)
                         + sign_extend(a3) * zero_extend(b3)
                         + acc;
    set_velem(vd, i, res);
  }
  RETIRE_SUCCESS
...
function clause execute (VDOT4AUS(vs2, vs1, vd)) = {
  foreach (i from vstart to vl-1) {
    ...
    let acc : bits (SEW) = get_velem(vd, i);
    let res : bits (SEW) = zero_extend(a0) * sign_extend(b0)
                         + zero_extend(a1) * sign_extend(b1)
                         + zero_extend(a2) * sign_extend(b2)
                         + zero_extend(a3) * sign_extend(b3)
                         + acc;
    set_velem(vd, i, res);
  }
  RETIRE_SUCCESS
