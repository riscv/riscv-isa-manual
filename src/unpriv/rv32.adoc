...
can emulate almost any other ISA extension (except the ext:a[] extension,
...
[#norm:rv32i_xreg_sz]#For RV32I, the 32 `x` registers are each 32 bits wide, in other words, XLEN=32.#
...
[#norm:imm_always_sex]#Except for the 5-bit immediates used in <<ext:zicsr,CSR instructions>>,
...
===== Unconditional Jumps
The jump and link (insn:jal[]) instruction uses the J-type format, where the
