...
cover the insnlink:ecall[]/insnlink:ebreak[] instructions with a single insn:system[] hardware
instruction that always traps and might be able to implement the insnlink:fence[]
...
insnlink:add[t0,t1,t2]; insnlink:bltu[t0,t1,overflow].

For signed addition, if one operand's sign is known, overflow checking
requires only a single branch after the addition:
insnlink:addi[t0,t1,+imm]; insnlink:blt[t0,t1,overflow]. This covers the common case
...
comparing the results of insnlink:add[] and insnlink:addw[] on the operands.
====
...
[#norm:addi_op]#insnlink:addi[] adds the sign-extended 12-bit immediate to register _rs1_.#
[#norm:addi_overflow]#Arithmetic overflow is ignored and the result is simply the low XLEN bits of the result.#
insnlink:addi[rd,rs1,0] is used to implement the insn:mv[rd,rs1] assembler pseudoinstruction.

[#norm:slti_sltiu_op]#insnlink:slti[] (set less than immediate) places the value 1 in register _rd_ if
register _rs1_ is less than the sign-extended immediate when both are
treated as signed numbers, else 0 is written to _rd_. insnlink:sltiu[] is similar
...
Note, insnlink:sltiu[rd,rs1,1] sets _rd_ to 1 if _rs1_ equals zero, otherwise
sets _rd_ to 0 (assembler pseudoinstruction insn:seqz[rd,rs]).

[#norm:andi_ori_xori_op]#insnlink:andi[], insnlink:ori[], insnlink:xori[] are logical operations that perform bitwise AND, OR, and
XOR on register _rs1_ and the sign-extended 12-bit immediate and place
the result in _rd_.#
Note, insnlink:xori[rd,rs1,-1] performs a bitwise logical inversion of register _rs1_ (assembler pseudoinstruction insn:not[rd,rs]).
...
[#norm:slli_op]#insnlink:slli[] is a logical left shift (zeros are shifted into the lower bits);#
[#norm:srli_op]#insnlink:srli[] is a logical right shift (zeros are shifted into the upper bits);# and
[#norm:srai_op]#insnlink:srai[] is an arithmetic right shift (the original sign bit is copied into the vacated upper bits).#

include::images/wavedrom/int-comp-lui-aiupc.edn[]
[[int-comp-lui-aiupc]]
//.Integer register-immediate, U-immediate

insnlink:lui[] (load upper immediate) is used to build 32-bit constants and uses the U-type format.
[#norm:lui_op]#insnlink:lui[] places the 32-bit U-immediate value into the
destination register _rd_, filling in the lowest 12 bits with zeros.#

insnlink:auipc[] (add upper immediate to `pc`) is used to build `pc`-relative
addresses and uses the U-type format.
[#norm:auipc_op]#insnlink:auipc[] forms a 32-bit offset from
the U-immediate, filling in the lowest 12 bits with zeros, adds this
offset to the address of the insnlink:auipc[] instruction, then places the result
in register _rd_.#

[NOTE]
====
The assembly syntax for insnlink:lui[] and insnlink:auipc[] does not represent the lower
12 bits of the U-immediate, which are always zero.

The insnlink:auipc[] instruction supports two-instruction sequences to access
arbitrary offsets from the `pc` for both control-flow transfers and data
accesses. The combination of an insnlink:auipc[] and the 12-bit immediate in a insnlink:jalr[]
can transfer control to any 32-bit `pc`-relative address, while an insnlink:auipc[]
...
a insnlink:jal[] +4 instruction could also be used to obtain the local `pc` (of the
instruction following the insnlink:jal[]), it might cause pipeline breaks in
...
[#norm:add_op]#insnlink:add[] performs the addition of _rs1_ and _rs2_.#
[#norm:sub_op]#insnlink:sub[] performs the subtraction of _rs2_ from _rs1_.#
[#norm:add_sub_overflow]#Overflows are ignored and the low XLEN bits of results are written to the destination _rd_.#
[#norm:slt_sltu_op]#insnlink:slt[] and insnlink:sltu[] perform signed and unsigned compares respectively, writing 1 to _rd_ if
_rs1_ < _rs2_, 0 otherwise.#
Note, insnlink:sltu[rd,x0,rs2] sets _rd_ to 1 if _rs2_ is not equal to zero,
otherwise sets _rd_ to zero (assembler pseudoinstruction insn:snez[rd,rs]).
[#norm:and_or_xor_op]#insnlink:and[], insnlink:or[], and insnlink:xor[] perform bitwise logical operations.#

[#norm:sll_srl_sra_op]#insnlink:sll[], insnlink:srl[], and insnlink:sra[] perform logical left, logical right, and arithmetic
...
performance counters.# [#norm:nop_enc]#insn:nop[] is encoded as insnlink:addi[x0,x0,0].#
...
insnlink:addi[] was chosen for the insn:nop[] encoding as this is most likely to take
...
Also, an insnlink:addi[] functional unit is more likely to be available in a
superscalar design as adds are the most common operation. In particular,
address-generation functional units can execute insnlink:addi[] using the same
hardware needed for base+offset address calculations, while
register-register insnlink:add[] or logical/shift operations require additional
hardware.
====
...
===== Unconditional Jumps
The insnlink:jal[] (jump and link) instruction uses the J-type format, where the
...
[#norm:jal_op]#insnlink:jal[] stores the address of the instruction
...
a insnlink:jal[] with _rd_=`x0`.

include::images/wavedrom/ct-unconditional.edn[]
[[ct-unconditional]]
//.The unconditional-jump instruction, JAL

The indirect jump instruction insnlink:jalr[] (jump and link register) uses the I-type encoding.
...
encoded as a insnlink:jalr[] with _rd_=`x0`.
Procedure returns in the standard calling convention (assembler
pseudoinstruction insn:ret[]) are encoded as a insnlink:jalr[] with _rd_=`x0`, _rs1_=`x1`, and
_imm_=0.
...
help support position-independent code. The insnlink:jalr[] instruction was defined
to enable a two-instruction sequence to jump anywhere in a 32-bit
absolute address range. A insnlink:lui[] instruction can first load _rs1_ with the
upper 20 bits of a target address, then insnlink:jalr[] can add in the lower bits.
Similarly, insnlink:auipc[] then insnlink:jalr[] can jump anywhere in a 32-bit `pc`-relative
address range.

Note that the insnlink:jalr[] instruction does not treat the 12-bit immediate as
...
insnlink:jalr[] will have either a zero immediate or be paired with a insnlink:lui[] or insnlink:auipc[],
so the slight reduction in range is not significant.

Clearing the least-significant bit when calculating the insnlink:jalr[] target
...
When used with a base _rs1_=`x0`, insnlink:jalr[] can be used to
...
The insnlink:jal[] and insnlink:jalr[] instructions will generate an
...
via the register numbers used. A insnlink:jal[] instruction should push the return
address onto a return-address stack (RAS) only when _rd_ is `x1` or
`x5`. insnlink:jalr[] instructions should push/pop a RAS as shown in <<rashints>>.

[[rashints]]
.Return-address stack prediction hints encoded in the register operands of a insnlink:jalr[] instruction.
...
insnlink:lui[ra,imm20]; insnlink:jalr[ra,imm12(ra)] and insnlink:auipc[ra,imm20]; insnlink:jalr[ra,imm12(ra)].
====
...
[#norm:beq_bne_op]#insnlink:beq[] and insnlink:bne[] take the branch if registers _rs1_ and _rs2_ are equal or unequal respectively.#
[#norm:blt_bltu_op]#insnlink:blt[] and insnlink:bltu[] take the branch if _rs1_ is less than _rs2_, using signed and
unsigned comparison respectively.#
[#norm:bge_bgeu_op]#insnlink:bge[] and insnlink:bgeu[] take the branch if _rs1_ is greater than or equal to _rs2_,
using signed and unsigned comparison respectively.#
Note, insn:bgt[], insn:bgtu[], insn:ble[], and insn:bleu[] can be synthesized by
reversing the operands to insnlink:blt[], insnlink:bltu[], insnlink:bge[], and insnlink:bgeu[], respectively.

[NOTE]
====
Signed array bounds may be checked with a single insnlink:bltu[] instruction, since
...
Unlike some other architectures, the RISC-V jump (insnlink:jal[] with _rd_=`x0`)
...
[#norm:lw_op]#The insnlink:lw[] instruction loads a 32-bit value from memory into _rd_.#
[#norm:lh_op]#insnlink:lh[] loads a 16-bit value from memory, then sign-extends to 32-bits before storing
in _rd_.#
[#norm:lhu_op]#insnlink:lhu[] loads a 16-bit value from memory but then zero extends to
32-bits before storing in _rd_.#
[#norm:lb_lbu_op]#insnlink:lb[] and insnlink:lbu[] are defined analogously for 8-bit values.#
[#norm:sw_sh_sb_op]#The insnlink:sw[], insnlink:sh[], and insnlink:sb[] instructions store 32-bit, 16-bit, and
...
insnlink:fence[] instructions are used to order device I/O and memory accesses as
...
observe any operation in the _successor_ set following a insnlink:fence[] before
any operation in the _predecessor_ set preceding the insnlink:fence[].
<<memorymodel>> provides a precise description
of the RISC-V memory consistency model.

insnlink:fence[] instructions also order memory reads and writes made by the
hart as observed by memory reads and writes made by an external device.
However, insnlink:fence[] instructions do not order observations of events made by an external
...
communication is outside the scope of the insnlink:fence[] ordering mechanism and
hence insnlink:fence[] instructions can provide no guarantee on when a change in
...
|1000 |.TSO |With insnlink:fence[RW,RW]: exclude write-to-read ordering; otherwise: _Reserved for future use._
|_other_|_other_ |_Reserved for future use._
|===

The insnlink:fence[] mode field _fm_ defines the semantics of the insnlink:fence[] instruction.
[#norm:fence_op]#A insnlink:fence[] (with _fm_=`0000`) orders all memory operations in its predecessor set
before all memory operations in its successor set.#

A insnlink:fence.tso[] instruction is encoded as a insnlink:fence[] instruction
with _fm_=`1000`, _predecessor_=`RW`, and _successor_=`RW`.
[#norm:fence-tso_op]#insnlink:fence.tso[] orders
...
Because insnlink:fence[RW,RW] imposes a superset of the orderings that
insnlink:fence.tso[] imposes, it is correct to ignore the _fm_ field and implement insnlink:fence.tso[] as
insnlink:fence[RW,RW].
====

[NOTE]
====
The insnlink:fence.tso[] instruction was incorrectly described as optional in 2019.
However, insnlink:fence.tso[] is encoded within the standard insnlink:fence[] encoding such that
...
As software can always assume without any penalty that insnlink:fence.tso[] is being
...
insnlink:fence[] instructions, _rs1_ and _rd_, are reserved
...
insnlink:fence[] instructions (with _fm_=`0000`), and standard software shall use only
...
conservative insnlink:fence[] on all operations.
====
...
[#norm:ecall_op1]#The insnlink:ecall[] instruction is used to make a service request to the execution environment.#
...
[#norm:ebreak_op1]#The insnlink:ebreak[] instruction is used to return control to a debugging environment.#

[NOTE]
====
insnlink:ecall[] and insnlink:ebreak[] were previously named SCALL and SBREAK. The
...
[NOTE]
====
insnlink:ebreak[] was primarily designed to be used by a debugger to cause
execution to stop and fall back into the debugger. insnlink:ebreak[] is also used
by the standard GCC compiler to mark code paths that should not be
executed.

Another use of insnlink:ebreak[] is to support "`semihosting`", where the execution
environment includes a debugger that can provide services over an
alternate system call interface built around the insnlink:ebreak[] instruction.
Because the RISC-V base ISAs do not provide more than one insnlink:ebreak[]
instruction, RISC-V semihosting uses a special sequence of instructions
to distinguish a semihosting insnlink:ebreak[] from a debugger inserted insnlink:ebreak[].
...
encoded as an insnlink:ecall[] using an existing ABI, but this would require the
debugger to be able to intercept insnlink:ecall[]s, which is a newer addition to
the debug standard. We intend to move over to using insnlink:ecall[]s with a
...
_rd_=`x0`. The other RV32I HINTs are encoded as insnlink:fence[] instructions with
...
example, insnlink:add[] is a HINT if the destination register is `x0`; the five-bit
_rs1_ and _rs2_ fields encode arguments to the HINT. However, a simple
implementation can simply execute the HINT as an insnlink:add[] of _rs1_ and _rs2_
that writes `x0`, which has no architecturally visible effect.

As another example, a insnlink:fence[] instruction with a zero _pred_ field and a
...
HINT as a insnlink:fence[] that orders the null set of prior memory accesses before
...
|insnlink:lui[] |_rd_=`x0` |2^20^ .8+<.^m|_Designated for future standard use_

|insnlink:auipc[] |_rd_=`x0` |2^20^

|insnlink:addi[] |_rd_=`x0`, and either _rs1_{ne}``x0`` or _imm_{ne}0 |2^17^−1

|insnlink:andi[] |_rd_=`x0` |2^17^

|insnlink:ori[] |_rd_=`x0` |2^17^

|insnlink:xori[] |_rd_=`x0` |2^17^

|insnlink:add[] |_rd_=`x0`, _rs1_{ne}``x0`` |2^10^−32

|insnlink:add[] |_rd_=`x0`, _rs1_=`x0`, _rs2_{ne}``x2``{endash}``x5`` | 28

|insnlink:add[] |_rd_=`x0`, _rs1_=`x0`, _rs2_=`x2`{endash}``x5`` |4|(_rs2_=`x2`) NTL.P1 +
(_rs2_=`x3`) NTL.PALL +
(_rs2_=`x4`) NTL.S1 +
(_rs2_=`x5`) NTL.ALL

|insnlink:slli[] |_rd_=`x0`, _rs1_=`x0`, _shamt_=31 |1|Semihosting entry marker

|insnlink:srai[] |_rd_=`x0`, _rs1_=`x0`, _shamt_=7 |1|Semihosting exit marker

|insnlink:sub[] |_rd_=`x0` |2^10^ .11+<.^m|_Designated for future standard use_

|insnlink:and[] |_rd_=`x0` |2^10^

|insnlink:or[] |_rd_=`x0` |2^10^

|insnlink:xor[] |_rd_=`x0` |2^10^

|insnlink:sll[] |_rd_=`x0` |2^10^

|insnlink:srl[] |_rd_=`x0` |2^10^

|insnlink:sra[] |_rd_=`x0` |2^10^

|insnlink:fence[] |_rd_=`x0`, _rs1_{ne}``x0``, _fm_=0, and either _pred_=0 or _succ_=0| 2^10^−63

|insnlink:fence[] |_rd_{ne}``x0``, _rs1_=`x0`, _fm_=0, and either _pred_=0 or _succ_=0| 2^10^−63

|insnlink:fence[] |_rd_=_rs1_=`x0`, _fm_=0, _pred_=0, _succ_{ne}0 |15

|insnlink:fence[] |_rd_=_rs1_=`x0`, _fm_=0, _pred_{ne}W, _succ_=0 |15

|insnlink:fence[] |_rd_=_rs1_=`x0`, _fm_=0, _pred_=W, _succ_=0 |1 |PAUSE

4+|

|insnlink:slti[] |_rd_=`x0` |2^17^ .7+<.^m|_Designated for custom use_

|insnlink:sltiu[] |_rd_=`x0` |2^17^

|insnlink:slli[] |_rd_=`x0`, and either _rs1_{ne}``x0`` or _shamt_{ne}31 |2^10^−1

|insnlink:srli[] |_rd_=`x0` |2^10^

|insnlink:srai[] |_rd_=`x0`, and either _rs1_{ne}``x0`` or _shamt_{ne}7 |2^10^−1

|insnlink:slt[] |_rd_=`x0` |2^10^

|insnlink:sltu[] |_rd_=`x0` |2^10^
|===

NOTE: insnlink:slli[x0,x0,0x1f] and insnlink:srai[x0,x0,7] were previously designated as
