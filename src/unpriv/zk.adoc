...
[NOTE]
====
[#norm:zknd_shared_instr]#The insnlink:aes64ks1i[] and insnlink:aes64ks2[] instructions
...
[NOTE]
====
[#norm:zkne_shared_instr]#The insnlink:aes64ks1i[] and insnlink:aes64ks2[] instructions
are present in both the extlink:zknd[] and extlink:zkne[] extensions.#
====
...
Mnemonic::
insnlink:aes32dsi[rd,rs1,rs2,bs]
...
Mnemonic::
insnlink:aes32dsmi[rd,rs1,rs2,bs]
...
Mnemonic::
insnlink:aes32esi[rd,rs1,rs2,bs]
...
Mnemonic::
insnlink:aes32esmi[rd,rs1,rs2,bs]
...
Mnemonic::
insnlink:aes64ks1i[rd,rs1,rnum]
...
Description::
insnlink:clmul[] produces the lower half of the 2{times}XLEN carry-less product.
...
Description::
insnlink:clmulh[] produces the upper half of the 2{times}XLEN carry-less product.
...
in conjunction with the insnlink:sha512sig0l[] instruction.
...
Mnemonic::
insnlink:sha512sig0l[rd,rs1,rs2]
...
This instruction is implemented on RV32 only.
Used to compute the Sigma0 transform of the SHA2-512 hash function
in conjunction with the insnlink:sha512sig0h[] instruction.
The transform is a 64-bit to 64-bit function, so the input and output
are each represented by two 32-bit registers.
This instruction must _always_ be implemented such that its execution
latency does not depend on the data being operated on.
...
Mnemonic::
insnlink:sha512sig1h[rd,rs1,rs2]
...
in conjunction with the insnlink:sha512sig1l[] instruction.
The transform is a 64-bit to 64-bit function, so the input and output
are each represented by two 32-bit registers.
This instruction must _always_ be implemented such that its execution
latency does not depend on the data being operated on.
...
Mnemonic::
insnlink:sha512sig1l[rd,rs1,rs2]
...
This instruction is implemented on RV32 only.
Used to compute the Sigma1 transform of the SHA2-512 hash function
in conjunction with the insnlink:sha512sig1h[] instruction.
The transform is a 64-bit to 64-bit function, so the input and output
are each represented by two 32-bit registers.
This instruction must _always_ be implemented such that its execution
latency does not depend on the data being operated on.
...
Mnemonic::
insnlink:sha512sum0r[rd,rs1,rs2]
...
Mnemonic::
insnlink:sha512sum1r[rd,rs1,rs2]
...
It is the inverse of the insnlink:zip[] instruction.
...
It is the inverse of the insnlink:unzip[] instruction.
This instruction is available only on RV32.
...
(insnlink:csrrs[]/insnlink:csrrc[] with _rs1_=`x0` or insnlink:csrrsi[]/insnlink:csrrci[] with _uimm_=0) raise an
...
Software normally uses the instruction insnlink:csrrw[rd,seed,x0] to read the csr:seed[]
CSR.
...
rotation instructions, so insnlink:zip[] and insnlink:unzip[] are not required.
====
...
The insnlink:andn[] instruction is also useful for constant time word-select
...
time, using the insnlink:xperm8[] instruction
...
thousands of cycles under some circumstances. A insnlink:wfi[] or insnlink:pause[]
