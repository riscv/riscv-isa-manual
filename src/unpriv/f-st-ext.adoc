...
insnlink:flw[] and insnlink:fsw[] are only guaranteed to execute atomically if the effective
address is naturally aligned.

[[norm:fsw_flw_bits_maintained]]
insnlink:flw[] and insnlink:fsw[] do not modify the bits being transferred; in particular, the payloads of non-canonical NaNs are preserved.
...
[#norm:fadd-s_fmul-s_op]#insnlink:fadd.s[] and insnlink:fmul.s[] perform single-precision floating-point addition and multiplication respectively, between _rs1_ and _rs2_#.
[#norm:fsub-s_op]#insnlink:fsub.s[] performs the single-precision floating-point subtraction of _rs2_ from _rs1_#.
[#norm:fdiv-s_op]#insnlink:fdiv.s[] performs the single-precision floating-point division of _rs1_ by _rs2_#.
[#norm:fsqrt-s_op]#insnlink:fsqrt.s[] computes the square root of _rs1_#. [#norm:fp_rd]#In each case, the result is written to _rd_.#
...
[#norm:fmin-s_fmax-s_op]#Floating-point minimum-number and maximum-number instructions insnlink:fmin.s[] and insnlink:fmax.s[] write, respectively, the smaller or larger of _rs1_ and _rs2_ to _rd_#.
...
[NOTE]
====
Note that in version 2.2 of the ext:f[] extension, the insnlink:fmin.s[] and
insnlink:fmax.s[] instructions were amended to implement the IEEE 754-2019 cite:[ieee754-2019]
...
[[norm:fmadd-s_op]]
insnlink:fmadd.s[] multiplies the values in _rs1_ and _rs2_, adds the value in
_rs3_, and writes the final result to _rd_. insnlink:fmadd.s[] computes
_(rs1×rs2)+rs3_.

[[norm:fmsub-s_op]]
insnlink:fmsub.s[] multiplies the values in _rs1_ and _rs2_, subtracts the value in _rs3_, and writes the final result to _rd_. insnlink:fmsub.s[] computes
_(rs1×rs2)−rs3_.

[[norm:fnmsub-s_op]]
insnlink:fnmsub.s[] multiplies the values in _rs1_ and _rs2_, negates the product, adds the value in _rs3_, and writes the final result to _rd_. insnlink:fnmsub.s[] computes _−(rs1×rs2)+rs3_.

[[norm:fnmadd-s_op]]
insnlink:fnmadd.s[] multiplies the values in _rs1_ and _rs2_, negates the product, subtracts the value in _rs3_, and writes the final result to _rd_. insnlink:fnmadd.s[] computes _−(rs1×rs2)−rs3_.
...
instructions are encoded in the OP-FP major opcode space. [#norm:fcvt-l-s_fcvt-w-s_op]#insnlink:fcvt.w.s[] or
insnlink:fcvt.l.s[] converts a floating-point number in floating-point register
_rs1_ to a signed 32-bit or 64-bit integer, respectively, in integer
register _rd_#. [#norm:fcvt-s-w_fcvt-s-l_op]#insnlink:fcvt.s.w[] or insnlink:fcvt.s.l[] converts a 32-bit or 64-bit signed
integer, respectively, in integer register _rs1_ into a floating-point
number in floating-point register _rd_#. [#norm:fcvt-wu-s_fcvt-lu-s_fcvt-s-wu_fcvt-s-lu_op]#insnlink:fcvt.wu.s[], insnlink:fcvt.lu.s[], insnlink:fcvt.s.wu[],
and insnlink:fcvt.s.lu[] variants convert to or from unsigned integer values. For
XLEN>32, insnlink:fcvt.w.s[] and insnlink:fcvt.wu.s[] sign-extend the 32-bit result to the destination register width#.
[#norm:fcvt_long_float_rv64_only]#insnlink:fcvt.l.s[], insnlink:fcvt.lu.s[], insnlink:fcvt.s.l[], and insnlink:fcvt.s.lu[] are RV64-only instructions#.
...
for insnlink:fcvt.w.s[], insnlink:fcvt.wu.s[], insnlink:fcvt.l.s[], and insnlink:fcvt.lu.s[] and
...
insnlink:fcvt.s.w[rd,x0], which will never set any exception flags.

[[int_conv]]
.Domains of float-to-integer conversions and behavior for invalid inputs
[%autowidth,float="center",align="center",cols="<,>,>,>,>",options="header",]
|===
| |insnlink:fcvt.w.s[] |insnlink:fcvt.wu.s[] |insnlink:fcvt.l.s[] |insnlink:fcvt.lu.s[]
...
[#norm:fsgnj-s_fsgnjn-s_fsgnjx-s_op]#insnlink:fsgnj.s[], insnlink:fsgnjn.s[], and
insnlink:fsgnjx.s[] are floating-point to floating-point sign-injection instructions
...
Note, insnlink:fsgnj.s[rx,ry,ry] moves _ry_ to _rx_ (assembler pseudoinstruction insn:fmv.s[rx,ry]);
insnlink:fsgnjn.s[rx,ry,ry] moves the negation of _ry_ to _rx_ (assembler pseudoinstruction insn:fneg.s[rx,ry]);
and insnlink:fsgnjx.s[rx,ry,ry] moves the absolute value of _ry_ to _rx_ (assembler pseudoinstruction insn:fabs.s[rx,ry]).
...
floating-point and integer registers. [#norm:fmv-x-w_op]#insnlink:fmv.x.w[] moves
...
[[norm:fmv-w-x_op]]
insnlink:fmv.w.x[] moves the single-precision value encoded in the IEEE 754-2008
...
[NOTE]
====
The insnlink:fmv.w.x[] and insnlink:fmv.x.w[] instructions were previously called insn:fmv.s.x[] and insn:fmv.x.s[]. The use of W is more consistent with their semantics as an instruction that moves 32 bits without interpreting them. This became clearer after defining NaN-boxing. To avoid disturbing existing code, both the W and S versions will be supported by tools.
====
...
Floating-point compare instructions (insnlink:feq.s[], insnlink:flt.s[], insnlink:fle.s[]) perform the
...
[#norm:flt-s_fle-s_signaling]#insnlink:flt.s[] and insnlink:fle.s[] perform what IEEE 754-2008
refers to as _signaling_ comparisons: that is, they set the invalid
operation exception flag if either input is NaN#.
[#norm:feq-s_quiet]#insnlink:feq.s[] performs a _quiet_ comparison: it only sets
...
The insnlink:fclass.s[] instruction examines the value in floating-point register
...
insnlink:fclass.s[] does not set the floating-point exception flags.
