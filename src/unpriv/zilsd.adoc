...
[[norm:Zilsd_sd_x0]]
If using `x0` as `src` of insnlink:sd[], the entire 64-bit operand is zero — i.e., register `x1` is not accessed.

==== Exception Handling

[#norm:Zilsd_RVWMO_exc_misaligned]#For the purposes of RVWMO and exception handling, insnlink:ld[] and insnlink:sd[] instructions are
considered to be misaligned loads and stores#, with one additional constraint:
[#norm:Zilsd_align4_two_4byte]#an insnlink:ld[] or insnlink:sd[] instruction whose effective address is a multiple of 4 gives rise
to two 4-byte memory operations.#

NOTE: This definition permits insnlink:ld[] and insnlink:sd[] instructions giving rise to exactly one
