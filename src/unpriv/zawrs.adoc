...
a insnlink:wrs.nto[] (WRS-with-no-timeout) instruction is provided. Instead of polling
...
[#norm:Zawrs_wrs-nto_stall_exec]#Then a subsequent insnlink:wrs.nto[] instruction would cause the hart to temporarily
...
such use cases a second instruction [#norm:Zawrs_wrs-sto_stall_duration]#insnlink:wrs.sto[] (WRS-with-short-timeout) is
provided that works like insnlink:wrs.nto[] but bounds the stall duration to an
...
The insnlink:wrs.nto[] and insnlink:wrs.sto[] instructions cause the hart to temporarily stall
...
For insnlink:wrs.sto[] the stall
...
[loweralpha]
    ...
    . If insnlink:wrs.sto[], a "`short`" duration since start of stall has not elapsed
...
[#norm:Zawrs_exec_resume_rules]#insnlink:wrs.nto[] and insnlink:wrs.sto[] instructions follow the rules of the insnlink:wfi[] instruction
for resuming execution on a locally enabled pending interrupt.#

[#norm:Zawrs_priv_illegal_instr_excp]#When the csr::[tw] (timeout wait) bit in csr:mstatus[] is set and insnlink:wrs.nto[] is executed
in any privilege mode other than M-mode, and it does not complete within an
implementation-specific bounded time limit, the insnlink:wrs.nto[] instruction will cause
an illegal-instruction exception.#

[#norm:Zawrs_virtual_instr_excp]#When executing in VS- or VU-mode, if the csr::[vtw] bit is set in csr:hstatus[], the
csr::[tw] bit in csr:mstatus[] is clear, and the insnlink:wrs.nto[] does not complete within an
implementation-specific bounded time limit, the insnlink:wrs.nto[] instruction will cause
a virtual-instruction exception.#

[NOTE]
====
Since the insnlink:wrs.sto[] and insnlink:wrs.nto[] instructions can complete execution for
...
The duration of a insnlink:wrs.sto[] instruction's timeout may vary significantly within
...
insnlink:wrs.nto[], unlike insnlink:wfi[], is not specified to cause an illegal-instruction
exception if executed in U-mode when the governing csr::[tw] bit is 0. insnlink:wfi[] is
typically not expected to be used in U-mode and on many systems may promptly
cause an illegal-instruction exception if used at U-mode. Unlike insnlink:wfi[],
insnlink:wrs.nto[] is expected to be used by software in U-mode when waiting on
