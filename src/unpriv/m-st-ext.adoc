...
[#norm:mul_op]#insnlink:mul[] performs an XLEN-bit×XLEN-bit multiplication of
`rs1` by `rs2` and places the lower XLEN bits in the destination
register.# [#norm:mulh_mulhu_mulhsu_op]#insnlink:mulh[], insnlink:mulhu[], and insnlink:mulhsu[] perform the same multiplication but
...
code sequence is insnlink:mulh[rdh,rs1,rs2] followed by insnlink:mul[rdl,rs1,rs2].
insnlink:mulhu[] or insnlink:mulhsu[] can be used instead of insnlink:mulh[] if appropriate.
...
[NOTE]
====
insnlink:mulhsu[] is used in multi-word signed multiplication to multiply the
...
[#norm:mulw_op]#insnlink:mulw[] is an RV64 instruction that multiplies the lower 32 bits of the
source registers, placing the sign extension of the lower 32 bits of the
result into the destination register.#

[NOTE]
====
In RV64, insnlink:mul[] can be used to obtain the upper 32 bits of the 64-bit
...
to shift both arguments left by 32 bits, then use insnlink:mulh[], mulhu[] or mulhsu[].
====
...
(((MUL, DIV))) (((MUL, DIVU)))
[#norm:div_divu_op]#insnlink:div[] and insnlink:divu[] perform an XLEN bits by XLEN bits signed and unsigned
integer division of `rs1` by `rs2`, rounding towards zero.# [#norm:rem_remu_op]#insnlink:rem[] and insnlink:remu[]
provide the remainder of the corresponding division operation.# [#norm:rem_result_sign]#For insnlink:rem[],
...
recommended code sequence is insnlink:div[rdq,rs1,rs2] followed by rem[rdr,rs1,rs2].
insnlink:divu[] and insnlink:remu[] can also be used if appropriate. Note that `rdq` can
...
[#norm:divw_divuw_op]#insnlink:divw[] and insnlink:divuw[] are RV64 instructions that divide the lower 32 bits of
`rs1` by the lower 32 bits of `rs2`, treating them as signed and
unsigned integers, placing the 32-bit quotient in `rd`,
sign-extended to 64 bits.# [#norm:remw_remuw_op]#insnlink:remw[] and insnlink:remuw[] are RV64 instructions that
provide the corresponding signed and unsigned remainder
operations.# [#norm:remw_remuw_result_sign]#Both insnlink:remw[] and insnlink:remuw[] always sign-extend the 32-bit result
...
[[divby0]]
.Semantics for division by zero and division overflow. L is the width of the operation in bits: XLEN for insnlink:div[], insnlink:divu[], insnlink:rem[], and insnlink:remu[], or 32 for insnlink:divw[], insnlink:divuw[], insnlink:remw[], and insnlink:remuw[].
[cols="<2,^,^,^,^,^,^",options="header",]
|===
|Condition |Dividend |Divisor |insnlink:divu[]/insnlink:divuw[] |insnlink:remu[]/insnlink:remuw[] |insnlink:div[]/insnlink:divw[] |insnlink:rem[]/insnlink:remw[]
