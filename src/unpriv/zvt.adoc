[[zvt]]
=== Zvt Family of Vector Matrix Extensions, Version 0.4
...
|Zvtofp8fmm ^| Zvtbase_Zvt8e_Zve32f | OCP FP8 | FP32
|Zvtfp16fmm ^| Zvtbase_Zvt16e_Zve32f | FP16 | FP32
|Zvtbf16fmm ^| Zvtbase_Zvt16e_Zve32f | BF16 | FP32
|Zvtfmm     ^| Zvtbase_Zve32f | FP32 | FP32
|Zvtdmm     ^| Zvtbase_Zvt64e_Zve64d | FP64 | FP64
|===
...
NOTE: The width of the `tm` field constrains TE to be at most 8192.
...
As with the `vset` family of instructions, the matrix configuration
instructions do not raise illegal-instruction exceptions as a consequence
of `vtype`.vill holding the value 1.
...
instructions.
----
Encoding of matrix configuration instructions

31 29    28    27 25 24  23 22 20 19 17 16     15 14 12 11    7 6     0
 100 |   0    | 100 |  00  | 000 |       rs1     | 111 |  rd   |1010111| msettn
 100 |   0    | 100 |  00  | 001 |       rs1     | 111 |  rd   |1010111| msettm
 100 |   0    | 100 |  00  | 010 |       rs1     | 111 |  rd   |1010111| msettk
 100 | altfmt | 100 | vsew | 100 | 000 | mtwiden | 111 | 00000 |1010111| msetmtypei
 100 |   0    | 010 |     rs2    |       rs1     | 111 | 00000 |1010111| msetmtype
----
...
The `msetmtypei` instruction is defined similarly, except the new `mtype`.`mtwiden`,
`vtype`.`vsew`, and `vtype`.`altfmt` fields are supplied by immediates.

----
msetmtype[i] behavior:

vl = 0
if msetmtypei:
  mtype = 0
  mtype.mtwiden = insn.mtwiden
  vtype = 0
  vtype.vsew = insn.vsew
  vtype.altfmt = insn.altfmt
else:
  mtype = rs1
  vtype = rs2

if vtype is invalid per vsetvl semantics:
  vtype.vill = 1
...
NOTE: The recommended assembly syntax for the `msetmtypei` instruction is
`msetmtypei eM[alt], wN`, where `M` gives SEW (8, 16, 32, 64), `alt` sets
altfmt=1 when present, and `N` gives TWIDEN (0, 1, 2, 4), with `mtype[4:2]`
implicitly 0 in this form.
...
----
msettm rd, rs1  // rd = tm = min(rs1, LMUL*EVE, ETE)
----
...
----
msettk rd, rs1  // rd = tk = min(rs1, KMAX)
----

The `msettk` instruction sets `tk` to the lesser of `rs1` and KMAX.

The `msettm` and `msettk` instructions raise an illegal-instruction exception when `mtwiden`=0.
...
accumulator rows.
When loading and storing accumulator columns, additional `msettn`
...
If `mtwiden`=0, these instructions are reserved.
...
but the OCP formats appear to be gaining the most traction.
...
Finally, they are added to the accumulator in IEEE 754 arithmetic,
...
instructions that do not support TEW=8 do not encode `t[0]`, and so
use `rd[4:2]` to encode `t[3:1]`).
