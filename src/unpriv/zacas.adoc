...
[#norm:Zacas_rv32_amocas-w_op]#For RV32, insnlink:amocas.w[] atomically loads a 32-bit data value from address in `rs1`,
...
operation performed by insnlink:amocas.w[] for RV32 is as follows:
...
insnlink:amocas.d[] is similar to insnlink:amocas.w[] but operates on 64-bit data values.

[#norm:Zacas_rv32_amocas-d_op]#For RV32, insnlink:amocas.d[] atomically loads 64-bits of a data value from address in
...
The operation performed by insnlink:amocas.d[] for RV32 is as follows:
...
[#norm:Zacas_rv64_amocas-w_op]#For RV64, insnlink:amocas.w[] atomically loads a 32-bit data value from address in
...
performed by insnlink:amocas.w[] for RV64 is as follows:
...
[#norm:Zacas_rv64_amocas-d_op]#For RV64, insnlink:amocas.d[] atomically loads 64-bits of a data value from address in
...
`rd`.# The operation performed by insnlink:amocas.d[] for RV64 is as follows:
[listing]
    temp = mem[X(rs1)]
    if ( temp == X(rd) )
        mem[X(rs1)] = X(rs2)
...
[#norm:Zacas_rv64_amocas-q_op]#insnlink:amocas.q[] (RV64 only) atomically loads 128-bits of a data value from address in
...
insnlink:amocas.q[] is as follows:
...
A insnlink:fence[] instruction may be used to order the memory read access and, if
