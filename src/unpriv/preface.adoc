...
* Defined insnlink:pause[] hint instruction.
...
of insnlink:fence.i[], counters, and CSR instructions that were in previous base
ISA.
...
* Moved insnlink:fence.i[] out of the mandatory base and into a separate extension,
with ext:zifencei[] ISA name. insnlink:fence.i[] was removed from the Linux user ABI and
...
* Improvements to the description and commentary.
* Modified implicit hinting suggestion on insnlink:jalr[] to support more efficient
macro-op fusion of insnlink:lui[]/insnlink:jalr[] and insnlink:auipc[]/insnlink:jalr[] pairs.
...
* Instructions insn:fmv.s.x[] and insn:fmv.x.s[] were renamed to insnlink:fmv.w.x[] and insnlink:fmv.x.w[]
...
* The insn:scall[] and insn:sbreak[] instructions have been renamed to insnlink:ecall[] and
insnlink:ebreak[], respectively. Their encoding and functionality are unchanged.
...
* The insnlink:fence[] instruction provides finer-grain memory and I/O orderings.
* An AMO for fetch-and-XOR (insn:amoxor[]) has been added, and the encoding for
insn:amoswap[] has been changed to make room.
* The insnlink:auipc[] instruction, which adds a 20-bit upper immediate to the `pc`,
...
* The insnlink:jal[] instruction has now moved to the U-Type format with an
explicit destination register, and the insn:j[] instruction has been dropped
being replaced by insnlink:jal[] with _rd_=`x0`. This removes the only instruction
with an implicit destination register and removes the J-Type instruction
format from the base ISA. There is an accompanying reduction in insnlink:jal[]
reach, but a significant reduction in base ISA complexity.
* The static hints on the insnlink:jalr[] instruction have been dropped. The hints
are redundant with the _rd_ and _rs1_ register specifiers for code
compliant with the standard calling convention.
* The insnlink:jalr[] instruction now clears the lowest bit of the calculated
...
insnlink:fmv.x.d[], respectively. Similarly, insn:mxtf.s[] and insn:mxtf.d[] instructions have
been renamed to insn:fmv.s.x[] and insnlink:fmv.d.x[], respectively.
...
* The insn:fmv.x.s[] and insnlink:fmv.x.d[] instructions now source their operands from
_rs1_, instead of _rs2_. This change simplifies datapath design.
* insnlink:fclass.s[] and insnlink:fclass.d[] floating-point classify instructions have been
added.
