...
[[norm:csrrw_op]]
The insnlink:csrrw[] (Atomic Read/Write CSR) instruction atomically swaps values in
the CSRs and integer registers. insnlink:csrrw[] reads the old value of the CSR,
...
[[norm:csrrs_op]]
The insnlink:csrrs[] (Atomic Read and Set Bits in CSR) instruction reads the value
...
[[norm:csrrc_op]]
The insnlink:csrrc[] (Atomic Read and Clear Bits in CSR) instruction reads the
...
[NOTE]
====
Since insnlink:csrrs[] and insnlink:csrrc[] perform a read-modify-write operation, any bits that read as
...
insnlink:csrrc[] or insnlink:csrrs[] to modify a different bit will cause 0 to be read from
pmpaddr__n__[G-1] and then written back, updating the underlying value to 0.
====

[[norm:csrrs_csrrc_rs1_x0]]
For both insnlink:csrrs[] and insnlink:csrrc[], if _rs1_=`x0`, then the instruction will not
...
exceptions on accesses to read-only CSRs. Both insnlink:csrrs[] and insnlink:csrrc[] always
...
[[norm:csrrw_rs1_x0]]
A insnlink:csrrw[] with _rs1_=`x0` will attempt to write zero to the destination CSR.

[[norm:csrrwi_csrrsi_csrrci_ops]]
The insnlink:csrrwi[], insnlink:csrrsi[], and insnlink:csrrci[] variants are similar to insnlink:csrrw[], insnlink:csrrs[], and
insnlink:csrrc[] respectively, except they update the CSR using an XLEN-bit value
...
[#norm:csrrsi_csrrci_uimm_zero]#For insnlink:csrrsi[] and insnlink:csrrci[], if the uimm[4:0] field is zero, then these
...
insnlink:csrrwi[], if _rd_=`x0`, then the instruction shall not read the CSR and
shall not cause any of the side effects that might occur on a CSR read.#
Both insnlink:csrrsi[] and insnlink:csrrci[] will always read the CSR and cause any read side
...
|insnlink:csrrw[] |Yes |- |No |Yes

|insnlink:csrrw[] |No |- |Yes |Yes

|insnlink:csrrs[]/insnlink:csrrc[] |- |Yes |Yes |No

|insnlink:csrrs[]/insnlink:csrrc[] |- |No |Yes |Yes
...
|insnlink:csrrwi[] |Yes |- |No |Yes

|insnlink:csrrwi[] |No |- |Yes |Yes

|insnlink:csrrsi[]/insnlink:csrrci[] |- |Yes |Yes |No

|insnlink:csrrsi[]/insnlink:csrrci[] |- |No |Yes |Yes
|===
...
when written.  insnlink:csrrw[] and insnlink:csrrwi[] action side effects for all
such fields within the written CSR.  [#norm:csr_rs1_uimm_side_effect]#insnlink:csrrs[], insnlink:csrrsi[], insnlink:csrrc[], and insnlink:csrrci[]
...
encoded as insnlink:csrrs[rd,csr,x0]. The assembler pseudoinstruction to write
a CSR, insn:csrw[csr,rs1], is encoded as insnlink:csrrw[x0,csr,rs1], while
insn:csrwi[csr,uimm], is encoded as insnlink:csrrwi[x0,csr,uimm].
...
other cases, software should execute a insnlink:fence[] instruction between the
relevant accesses. For the purposes of the insnlink:fence[] instruction, CSR read
...
respect to insnlink:fence[] instructions without violating this specification.
====
