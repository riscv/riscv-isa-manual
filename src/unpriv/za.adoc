...
These instructions allow RISC-V to support the _release consistency with
sequentially consistent synchronization operations_ (RCsc) memory consistency
model cite:[Gharachorloo90memoryconsistency].
...
The base RISC-V ISA has a relaxed memory model, <<rvwmo,RVWMO>>, with the
insnlink:fence[] instruction for additional ordering constraints. The address
space is divided by the execution environment into memory and I/O
domains, and the insnlink:fence[] instruction provides options to order accesses to
one or both of these two address domains.

To provide more efficient support for release consistency,
...
insnlink:fence[] instruction should be used to order across both domains.#

[#norm:a_no_aq_rl]#If both bits are clear, no additional ordering constraints
are imposed on the atomic memory operation.#
[#norm:a_acquire_semantics]#If only the _aq_ bit is set, the atomic
...
[#norm:a_release_semantics]#If only the _rl_ bit is set, the
...
[#norm:a_seq_cst_semantics]#If both the _aq_ and _rl_
