...
R-type instruction format. [#norm:amo_op]#These AMO instructions atomically load a data
...
[#norm:amo_ops_supported]#The operations supported are swap, integer add, bitwise AND, bitwise OR,
...
provide release consistency semantics. [#norm:amo_release_consistency]#If the _aq_ bit is set, then no
