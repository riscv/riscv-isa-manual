...
in RV32I (insnlink:add[]) and RV64I (insnlink:addw[])? The insnlink:addw[] opcode could be used for
32-bit adds in RV32I and insn:addd[] for 64-bit adds in RV64I, instead of the
existing design which uses the same opcode insnlink:add[] for 32-bit adds in RV32I
and 64-bit adds in RV64I with a different opcode insnlink:addw[] for 32-bit adds in
RV64I. This would also be more consistent with the use of the same insnlink:lw[]
...
have a insn:*w[] suffix (e.g., insnlink:addw[], but insnlink:and[] not insn:andw[]). In hindsight, this was
...
Contained Trap:::
  ...
  supervisor and user mode on harts, an insnlink:ecall[] by a user-mode hart will
