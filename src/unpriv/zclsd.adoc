...
[#norm:Zilsd_c-ldsp_x0]#For insnlink:c.ldsp[], usage of `x0` as the destination is reserved.#

If using `x0` as `src` of insnlink:c.sdsp[], the entire 64-bit operand is zero, i.e., register `x1` is not accessed.

insnlink:c.ld[] and insnlink:c.sd[] instructions can only use `x8`-`x15`.

==== Exception Handling

For the purposes of RVWMO and exception handling, insnlink:c.ld[], insnlink:c.ldsp[], insnlink:c.sd[], and insnlink:c.sdsp[] instructions are
considered to be misaligned loads and stores, with one additional constraint:
a insnlink:c.ld[], insnlink:c.ldsp[], insnlink:c.sd[], or insnlink:c.sdsp[] instruction whose effective address is a multiple of 4 gives rise
...
|yes
|no
|insnlink:c.ldsp[rd,offset(sp)]
|<<#insns-cldsp>>

|yes
|no
|insnlink:c.sdsp[rs2,offset(sp)]
|<<#insns-csdsp>>

|yes
|no
|insnlink:c.ld[rd',offset(rs1')]
|<<#insns-cld>>

|yes
|no
|insnlink:c.sd[rs2',offset(rs1')]
|<<#insns-csd>>
...
Loads stack-pointer relative 64-bit value into registers `rd'` and `rd'+1`. It computes its effective address by adding the zero-extended offset, scaled by 8, to the stack pointer, `x2`. It expands to insnlink:ld[rd,offset(x2)]. insnlink:c.ldsp[] is only valid when _rd_{ne}x0; the code points with _rd_=x0 are reserved.
...
Stores a stack-pointer relative 64-bit value from registers `rs2'` and `rs2'+1`. It computes an effective address by adding the _zero_-extended offset, scaled by 8, to the stack pointer, `x2`. It expands to insnlink:sd[rs2,offset(x2)].
...
It computes an effective address by adding the zero-extended offset, scaled by 8, to the base address in register rs1'.
It expands to insnlink:sd[rs2',offset(rs1')].
