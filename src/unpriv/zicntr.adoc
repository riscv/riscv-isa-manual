...
divided between the ext:zicntr[] and extlink:zihpm[] extensions.#
...
insnlink:csrrs[rd,counter,x0] canonical form, but the other read-only CSR
instruction forms (based on insnlink:csrrc[]/insnlink:csrrsi[]/insnlink:csrrci[]) are also legal ways to
read these CSRs.
====
...
[NOTE]
====
Instructions that cause synchronous exceptions, including insnlink:ecall[] and
insnlink:ebreak[], are not considered to retire and hence do not increment the
csr:instret[] CSR.
====
