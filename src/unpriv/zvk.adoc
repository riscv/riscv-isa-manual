...
* Regarding side-channel countermeasures:
  ...
  possibility of any timing side-channels. [#norm:zvk_dataindep_timing]#All instructions
...
- [#norm:veccrypto_vl_vstart_rsv]#Instruction encodings that violate the csr:vl[] or csr:vstart[] requirements are vacated.#
...
[#norm:Zvkt_diel_def]#Data-independent execution latency (DIEL) applies to all _data operands_ of an instruction, even those that are not a
...
[#norm:Zvkt_diel_constants_exempt]#Also, DIEL does not apply to constant values specified in the
...
[#norm:Zvkt_diel_control_exempt]#In some cases--which are explicitly specified in the lists below--operands that are used as control rather than data
...
[#norm:vaesz-vs_only]#There is only a insn:*.vs[] form of the instruction.#
...
function clause execute (VSM3ME(vs2, vs1)) = {
  ...
  // Byte swap outputs from little-endian back to big-endian
    let w[16] : bits(32) = rev8(w16);
    let w[17] : bits(32) = rev8(w17);
    let w[18] : bits(32) = rev8(w18);
    let w[19] : bits(32) = rev8(w19);
    let w[20] : bits(32) = rev8(w20);
    let w[21] : bits(32) = rev8(w21);
    let w[22] : bits(32) = rev8(w22);
    let w[23] : bits(32) = rev8(w23);

    // Update the destination register.
    set_velem(vd, 256, i, w[23:16]);
  }
  RETIRE_SUCCESS
  }
}
