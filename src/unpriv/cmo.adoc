...
* The extlink:zicbom[] extension defines a set of cache-block management instructions:
  insnlink:cbo.inval[], insnlink:cbo.clean[], and insnlink:cbo.flush[]
* The extlink:zicboz[] extension defines a cache-block zero instruction: insnlink:cbo.zero[]
* The extlink:zicbop[] extension defines a set of cache-block prefetch instructions:
  insnlink:prefetch.r[], insnlink:prefetch.w[], and insnlink:prefetch.i[]
...
addresses) for the purposes of predecessor and successor sets in insnlink:fence[]
instructions. These operations are _not_ ordered by other instructions that
order stores, e.g. insnlink:fence.i[] and insnlink:sfence.vma[].
...
independently. For example a 64-byte insnlink:cbo.zero[] that spans two 32-byte PMP
...
[[norm:cbo-inval]]
A insnlink:cbo.inval[] instruction executes or raises either an illegal-instruction
...
[NOTE]
====
Until a modified cache block has updated memory, a insnlink:cbo.inval[] instruction may
...
higher privileged level software may program the CSRs so that insnlink:cbo.inval[]
either traps or performs a flush operation in a lower privileged level.
====

[[norm:cbo-clean_cbo-flush]]
A insnlink:cbo.clean[] or insnlink:cbo.flush[] instruction executes or raises an illegal-instruction
...
Finally, a insnlink:cbo.zero[] instruction executes or raises an illegal-instruction or
...
|{check}
|{check}
|insnlink:cbo.clean[base]
|<<#insns-cbo_clean>>

|{check}
|{check}
|insnlink:cbo.flush[base]
|<<#insns-cbo_flush>>

|{check}
|{check}
|insnlink:cbo.inval[base]
|<<#insns-cbo_inval>>
...
|{check}
|{check}
|insnlink:prefetch.i[offset(base)]
|<<#insns-prefetch_i>>

|{check}
|{check}
|insnlink:prefetch.r[offset(base)]
|<<#insns-prefetch_r>>

|{check}
|{check}
|insnlink:prefetch.w[offset(base)]
|<<#insns-prefetch_w>>
...
Mnemonic::
insnlink:cbo.clean[offset(base)]
...
[#norm:cbo-clean_op]#A insnlink:cbo.clean[] instruction performs a clean operation on the cache block whose
...
[NOTE]
====
When executing a insnlink:cbo.clean[] instruction, an implementation may instead perform
...
Mnemonic::
insnlink:cbo.flush[offset(base)]
...
[#norm:cbo-flush_op]#A insnlink:cbo.flush[] instruction performs a flush operation on the cache block whose
...
Mnemonic::
insnlink:cbo.inval[offset(base)]
...
[#norm:cbo-inval_op]#A insnlink:cbo.inval[] instruction performs an invalidate operation on the cache block
...
[NOTE]
====
When executing a insnlink:cbo.inval[] instruction, an implementation may instead perform
a flush operation, since the result of that operation is indistinguishable from
...
[#norm:cbo-zero_op]#A insnlink:cbo.zero[] instruction performs stores of zeros to the full set of bytes
...
[[norm:prefetch-i_op]]
A insnlink:prefetch.i[] instruction indicates to hardware that the cache block whose
...
[[norm:prefetch-r_op]]
A insnlink:prefetch.r[] instruction indicates to hardware that the cache block whose
effective address is the sum of the base address specified in _rs1_ and the
sign-extended offset encoded in _imm[11:0]_, where _imm[4:0]_ equals `0b00000`,
...
[[norm:prefetch-w_op]]
A insnlink:prefetch.w[] instruction indicates to hardware that the cache block whose
effective address is the sum of the base address specified in _rs1_ and the
sign-extended offset encoded in _imm[11:0]_, where _imm[4:0]_ equals `0b00000`,
