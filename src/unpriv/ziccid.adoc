[[ext:ziccid]]
== ext:ziccid[] Extension for Instruction/Data Coherence and Consistency, Version 1.0

The ext:ziccid[] extension mandates more stringent requirements for consistency
between instruction fetches and other memory accesses than those imposed by
the base ISA.
As described in detail in the next section, ext:ziccid[] guarantees that a hart's
...
[NOTE]
====
Under ext:ziccid[], stores _eventually_ become visible to instruction fetches, even
without executing a insn:fence.i[] instruction.
...
As is the case without the ext:ziccid[] extension, software can make a store from
the current hart visible to the hart's instruction fetches immediately by
executing a insn:fence.i[] instruction.
Software can make a store from the current hart visible to another hart's
instruction fetches by executing a insn:fence[w,o] instruction, then sending an
interprocessor interrupt to the remote hart, requesting that it execute
a insn:fence.i[] instruction.
Alternatively, the current hart can execute a insn:fence[w,w] instruction, then
write a flag in memory; upon observing the flag write, the remote hart
executes a insn:fence.i[] instruction.
In neither case is a data insn:fence[] required on the remote hart.
====

The ext:ziccid[] extension depends on the extlink:ziccif[] extension.
...
may be incoherently cached; this cache is flushed upon execution of a insn:fence.i[]
instruction.
...
[NOTE]
====
Absent the execution of a insn:fence.i[] instruction, there is no requirement that
...
NOTE: Therefore, absent the execution of a insn:fence.i[] instruction,
...
NOTE: Executing a insn:fence[w,w] instruction between two store instructions, for
...
The insn:fence.i[] instruction performs the following operations sequentially:
...
[NOTE]
====
The insn:fence.i[] instruction in ext:ziccid[] is specified such that software written for
non-ext:ziccid[] machines is compatible with ext:ziccid[] machines.
For cacheable and coherent memory regions, the combination of making older
stores globally visible and flushing the instruction buffer on insn:fence.i[]
suffices to implement insn:fence.i[]'s pre-ext:ziccid[] semantics for those regions.
====
