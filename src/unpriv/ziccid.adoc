=== ext:ziccid[] Extension for Instruction/Data Coherence and Consistency

The ext:ziccid[] extension mandates more stringent requirements for
consistency between instruction fetches and other memory accesses than those
imposed by the base ISA.
As described in detail in the next section, ext:ziccid[] guarantees that
a hart's instruction fetches appear to occur in program order with respect to
each other, and that stores eventually become visible to all harts'
instruction fetches.

[NOTE]
====
The primary intent of the ext:ziccid[] extension is to accelerate JIT
...
The ext:ziccid[] extension can be viewed as a means to codify the concept of
...
[NOTE]
====
Under ext:ziccid[], stores _eventually_ become visible to instruction fetches,
even without executing a FENCE.I instruction.
...
As is the case without the ext:ziccid[] extension, software can make a store
from the current hart visible to the hart's instruction fetches immediately by
...
[#norm:Ziccid_dep_ziccif]#The ext:ziccid[] extension depends on the extlink:ziccif[] extension.#
...
==== Operational Model of the ext:ziccid[] Extension

An implementation with the ext:ziccid[] extension behaves in a manner consistent
with the following operational model.
[#norm:Ziccid_model_scope]#This model only applies to instruction fetches to memory regions with the
cacheability and coherence PMAs; instruction fetches to other memory regions
proceed as though the ext:ziccid[] extension were not implemented.#
...
[#norm:Ziccid_fetch_order]#Harts have two ordered operational stages: instruction fetch and execution.
...
[#norm:Ziccid_fetch_global_order]#An instruction fetch is a memory read operation that appears in the global
memory order, in program order with respect to other instruction fetches
from the same hart.#
[#norm:Ziccid_fetch_value]#For each instruction byte fetched from memory, memory
...
[#norm:Ziccid_fetch_before_access]#Instruction fetches appear in the global memory order before any explicit memory
...
[#norm:Ziccid_dequeue_order]#After some amount of time, instructions are dequeued in program order from the
instruction buffer and executed.#

NOTE: Therefore, absent the execution of a FENCE.I instruction,
stores do not necessarily become visible immediately to subsequent instruction fetches,
though they do _eventually_ become visible to subsequent instruction fetches.

[#norm:Ziccid_exec_order]#Instructions are executed in program order, but instructions' explicit memory
accesses may be reordered in any manner consistent with RVWMO.#

NOTE: Executing a insn:fence[w,w] instruction between two store instructions,
for example, suffices to guarantee that the second store will not become
visible to any instruction fetch before the first store does.

The FENCE.I instruction performs the following operations sequentially:

[[norm:Ziccid_fence_i_op]]
- All of the hart's older explicit memory accesses become globally visible.
- The instruction buffer is flushed.

[NOTE]
====
The FENCE.I instruction in ext:ziccid[] is specified such that software
written for non-ext:ziccid[] machines is compatible with ext:ziccid[]
machines.
...
suffices to implement FENCE.I's pre-ext:ziccid[] semantics for those regions.
====
