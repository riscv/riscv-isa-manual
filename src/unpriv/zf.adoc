...
[#norm:FLEN_def]#FLEN can be 32, 64, or 128 depending on which of the ext:f[], ext:d[], and
ext:q[] extensions are supported.#
Several additional extensions with the ext:zf[] prefix provide
additional formats and computational instructions.
...
instead operate on floating-point numbers in the `x` registers. [#norm:Zfinx_Zdinx_incompatible_F_D]#These extensions,
