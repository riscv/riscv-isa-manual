...
Side Channel is defined as a channel in a computing system that
...
using side-channel techniques such as Flush+Reload cite:[flush-reload] or Prime+Probe cite:[osvik-ctrsa2006], which measure
...
* Domain-bypass attacks bypass hardware access controls such as privilege
  separation or page tables. Example: Rogue Data Cache Load (Meltdown cite:[meltdown]).
...
* Cross-domain attacks require a disclosure gadget in the victim's domain, which
  ...
  Target Injection (Spectre Variant 2 cite:[spectre]).
...
[listing]
 ...
 9. // the load at line 18. The TEB at line 12 prevents
10. // execution of the loads at line 15 and 18 until the
11. // branch resolves.
12.     teb
