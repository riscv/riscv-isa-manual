...
The insnlink:pause[] instruction hints to a hart that it should temporarily reduce its
rate of execution.
...
it would not supplant insnlink:pause[].
insnlink:pause[] is more appropriate when polling for non-memory events, when polling for
