...
IEEE Compliance: While BF16 (also known as BFloat16) is not an IEEE 754
_standard_ format, it is a valid floating-point format as defined by IEEE
754-2008.
...
In practice, the default IEEE rounding mode (round to nearest, ties to even) is
generally used for arithmetic.

====== Handling exceptions
RISC-V supports _default exception handling_ defined in IEEE 754-2008. BF16 is
no exception.

Default exception handling, as defined in IEEE 754-2008, is a simple and
effective approach to producing results in exceptional cases. For the coder to
be able to see what has happened, and take further action if needed, BF16
instructions set floating-point exception flags the same way as all other
floating-point instructions in RISC-V.

======= Underflow

The underflow exception defined in IEEE 754-2008 requires that a result be
inexact and tiny, where tininess can be detected before or after rounding. In
RISC-V, tininess is detected after rounding.
...
As is defined in IEEE 754-2008, under default exception handling, underflow is
only signalled when the result is tiny and inexact. In such a case, both the
underflow and inexact flags are raised.
