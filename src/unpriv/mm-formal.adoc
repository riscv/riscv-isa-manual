...
[disc]
* <<sat_by_forwarding, Satisfy memory load operation by forwarding from unpropagated stores>>: This partially or entirely satisfies a single memory load operation by forwarding, from program-order-previous memory store operations.
...
ordering extension ext:ztso[]. It should be trivial to adapt the model to
RV32IA and to the ext:g[], ext:q[] and ext:c[] extensions, but we have never tried it.
