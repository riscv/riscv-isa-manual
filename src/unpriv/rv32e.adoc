...
This section describes the RV32E and RV64E base integer
...
chapter only outlines the differences between RV32E and RV64E and
RV32I and RV64I, and so should be read after <<rv32>> and <<rv64>>.
...
registers, (`x0`{endash}``x15``), where `x0` is a dedicated zero register.
...
[[norm:diff_rv32e_rv32i]]
RV32E and RV64E use the same instruction-set encoding as RV32I and RV64I
respectively, except that only registers `x0`{endash}``x15`` are provided. All
encodings specifying the other registers `x16`{endash}``x31`` are reserved.

[NOTE]
====
The previous draft of this chapter made all encodings using the
`x16`{endash}``x31`` registers available as custom. This version takes a more
