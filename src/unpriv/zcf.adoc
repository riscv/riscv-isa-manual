...
[#norm:c-flwsp_op]#insnlink:c.flwsp[] is an RV32FC-only instruction that loads a single-precision
...
insnlink:flw[rd, offset(x2)].
insnlink:c.flwsp[] uses the <<rvc-form,CI format>>.#

[#norm:c-fswsp_op]#insnlink:c.fswsp[] is an RV32FC-only instruction that stores a single-precision
...
[#norm:c-flw_op]#insnlink:c.flw[] is an RV32FC-only instruction that loads a single-precision
...
insnlink:flw[rd′,offset(rs1′)].#

[#norm:c-fsw_op]#insnlink:c.fsw[] is an RV32FC-only instruction that stores a single-precision
