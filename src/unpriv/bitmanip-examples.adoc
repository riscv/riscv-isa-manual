[[app:bitmanip-examples]]
=== Bit Manipulation Assembly Code Examples

The following examples provide software optimization guidance.

==== `strlen`

The insnlink:orc.b[] instruction allows for the efficient detection of `NUL` bytes in an XLEN-sized chunk of data:

 * the result of insnlink:orc.b[] on a chunk that does not contain any `NUL` bytes will be all-ones, and
 * after a bitwise-negation of the result of insnlink:orc.b[], the number of data bytes before the  first `NUL` byte (if any) can be detected by insnlink:ctz[]/insnlink:clz[] (depending on the endianness of data).
