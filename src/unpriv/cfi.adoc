...
(csr::[elp]) state that is updated by an indirect call or indirect jump to
...
landing pad instruction, the hart maintains an expected landing pad (csr::[elp]) state
to determine if a landing pad instruction is required at the target of an
indirect call or an indirect jump. The csr::[elp] state can be one of:
...
with `__x__tval` set to "`landing pad fault (code=2)`" is raised else the csr::[elp] is
updated to `NO_LP_EXPECTED`.

[NOTE]
====
The tracking of csr::[elp] and the requirement for a landing pad instruction
...
a pattern of `0x?017` (for example, the immediate fields of a insn:lui[], insn:auipc[], or
