...
instructions (e.g., insnlink:jalr[], insnlink:c.jr[], insnlink:c.jalr[]), from their original target
...
susceptible to control-flow subversion. Unconditional direct jumps using insnlink:jal[]
transfer control to a target that is in a {pm}qty:1[MiB] range from the current `pc`.
Unconditional indirect jumps using the insnlink:jalr[] obtain their branch target by
...
returning from a procedure. A insnlink:jal[] or insnlink:jalr[] may be used to perform a procedure
call and insnlink:jalr[] to return from a procedure. The RISC-V ABI however defines the
convention that a insnlink:jal[]/insnlink:jalr[] where `rd` (i.e. the link register) is `x1` or
`x5` is a procedure call, and a insnlink:jalr[] where `rs1` is the conventional
...
jump and conditional branch instructions. The insnlink:c.j[] and insnlink:c.jal[] instructions
encode an offset in the immediate field of the instruction and thus are not
susceptible to control-flow subversion. The insnlink:c.jr[] and insnlink:c.jalr[] instructions
perform an unconditional control transfer to the address in register `rs1`. The
insnlink:c.jalr[] additionally writes the address of the instruction following the jump
(`pc+2`) to the link register `x1` and is a procedure call. The insnlink:c.jr[] is a
return from procedure if `rs1` is a conventional link register (i.e. `x1` or
`x5`); else it is an indirect jump.

[[norm:cfi_call_term]]
The term _call_ is used to refer to a insnlink:jal[], insnlink:jalr[] or
insnlink:c.jal[] instruction with a link register as destination, i.e., _rd_≠`x0`.
Conventionally, the link register is `x1` or `x5`. A _call_ using insnlink:jal[] or
insnlink:c.jal[] is termed a _direct call_. A insnlink:c.jalr[] expands to
insnlink:jalr[x1,0(rs1)] and thus it is a call. A call using insnlink:jalr[] or
insnlink:c.jalr[] is termed an _indirect call_.

[[norm:cfi_return_term]]
The term _return_ is used to refer to a insnlink:jalr[] instruction with _rd_=`x0` and
with _rs1_=`x1` or _rs1_=`x5`. A insnlink:c.jr[] instruction expands to
insnlink:jalr[x0,0(rs1)] and is a return if _rs1_=`x1` or _rs1_=`x5`.

[[norm:cfi_indirect-jump_term]]
The term _indirect jump_ is used to refer to a insnlink:jalr[] instruction with _rd_=`x0`
and where the _rs1_ is not `x1` or `x5` (i.e., not a return). A insnlink:c.jr[]
...
a landing pad (insnlink:lpad[]) instruction. The insnlink:lpad[] instruction must be placed at the
program locations that are valid targets of indirect jumps or calls.
[#norm:zicflip_lpad_enc]#The insnlink:lpad[] instruction (See <<LP_INST>>) is
encoded using the insnlink:auipc[] major opcode with _rd_=`x0`.#
...
The insnlink:lpad[] instruction uses the code points defined as HINTs for the insnlink:auipc[]
...
that is built with insnlink:lpad[] instructions can thus continue to operate correctly,
...
An indirect branch using insnlink:jalr[], insnlink:c.jalr[], or insnlink:c.jr[] with `rs1` as `x7` is
termed a _software-guarded branch_. Such branches do not need to land on a
insnlink:lpad[] instruction and thus do not set `ELP` to `LP_EXPECTED`.
...
insnlink:lui[ra,imm20] or a insnlink:auipc[ra,imm20] instruction followed by a
insnlink:jalr[ra,imm12(ra)] instruction where `ra` is the link register (either `x1` or
...
also be expanded to a two instruction sequence composed of a insnlink:lui[x7,imm20] or
insnlink:auipc[x7,imm20] followed by a insnlink:jalr[x0,x7]. Since the address of the
...
the insnlink:lui[] instruction) in the bits 31:12 of the `x7` register.#
[#norm:zicflip_lpad_imm_enc]#The insnlink:lpad[] instruction is encoded with a
...
insnlink:lpad[] instruction does not perform the label check and in programs
...
stream is not 4-byte aligned, or is not insnlink:lpad[], or if the landing pad label
encoded in insnlink:lpad[] is not zero and does not match the expected landing pad label
...
The insnlink:lpad[] requires a 4-byte alignment to address the concatenation of two
...
a pattern of `0x?017` (for example, the immediate fields of a insnlink:lui[], insnlink:auipc[], or
a insnlink:jal[] instruction), followed by a 16-bit or a 32-bit instruction. When
...
boundary to force the unintended insnlink:lpad[] pattern to become misaligned, and thus
...
[#norm:zicflip_lpad_enabled_instr_allowed]#When ext:zicfilp[] is enabled, insnlink:lpad[] is the only instruction allowed to execute when
the `ELP` state is `LP_EXPECTED`. If ext:zicfilp[] is not enabled then the instruction
is a no-op.# [#norm:zicflip_lpad_enabled_exception]#If ext:zicfilp[] is enabled, the insnlink:lpad[] instruction causes a
...
The operation of the insnlink:lpad[] instruction is as follows:

.insnlink:lpad[] operation
[listing]
----
...
[#norm:zicfiss_enc]#The ext:zicfiss[] instructions, except insnlink:ssamoswap.w[] and insnlink:ssamoswap.d[], are encoded using a subset of
...
** [#norm:zicfiss_sspush_enc]#insn:sspush[x1] and insn:sspush[x5] - encoded using insnlink:mop.rr.7[]#
** [#norm:zicfiss_c-sspush_enc]#insn:c.sspush[x1] - encoded using insnlink:c.mop.1[]#

* Pop from the shadow stack (See <<SS_POP>>)
** [#norm:zicfiss_sspopchk_enc]#insn:sspopchk[x1] and insn:sspopchk[x5] - encoded using insnlink:mop.r.28[]#
** [#norm:zicfiss_c-sspopchk_enc]#insn:c.sspopchk[x5] - encoded using insnlink:c.mop.5[]#

* Read the value of csr:ssp[] into a register (See <<SSP_READ>>)
** [#norm:zicfiss_ssrdp_enc]#insnlink:ssrdp[] - encoded using insnlink:mop.r.28[]#

* [#norm:zicfiss_atomic_swap]#Perform an atomic swap from a shadow stack location (See <<SSAMOSWAP>>)#
** insnlink:ssamoswap.w[] and insnlink:ssamoswap.d[]

ext:zicfiss[] does not use all encodings of insnlink:mop.rr.7[] or insnlink:mop.r.28[]. When a
insnlink:mop.rr.7[] or insnlink:mop.r.28[] encoding is not used by the ext:zicfiss[] extension, the
...
the ext:zcmop[] defined insnlink:c.mop.1[] encoding. The insn:c.sspush[x1] expands
to insn:sspush[x1].
...
the shadow copy stored on the shadow stack. The function then uses insnlink:c.jr[x5] to
...
[[norm:ssrdp_read]]
The insnlink:ssrdp[] instruction is provided to move the contents of csr:ssp[] to a destination
register.
...
Encoding _rd_ as `x0` is not supported for insnlink:ssrdp[].

The operation of the insnlink:ssrdp[] instructions is as follows:

.insnlink:ssrdp[] operation
[listing]
----
if (xSSE == 1)
    X(dst) = ssp
...
For RV32, insnlink:ssamoswap.w[] atomically loads a 32-bit data value from address of a
...
insnlink:ssamoswap.d[] (RV64 only) is similar to insnlink:ssamoswap.w[] but operates on 64-bit
data values.

.insnlink:ssamoswap.w[] for RV32 and insnlink:ssamoswap.d[] (RV64 only) operation
[listing]
----
...
For RV64, insnlink:ssamoswap.w[] atomically loads a 32-bit data value from address of a
...
Just as for AMOs in the ext:a[] extension, insnlink:ssamoswap.w[] or insnlink:ssamoswap.d[] requires that the address
held in `rs1` be naturally aligned to the size of the operand (i.e., 8-byte
aligned for doublewords, and 4-byte aligned for words). The same
exception options apply if the address is not naturally aligned.

Just as for AMOs in the ext:zaamo[] extension, insnlink:ssamoswap.w[] or insnlink:ssamoswap.d[] optionally provides release
consistency semantics, using the `aq` and `rl` bits, to help implement
multiprocessor synchronization. An insnlink:ssamoswap.w[] or insnlink:ssamoswap.d[] operation has acquire
...
insnlink:ssamoswap.w[] or insnlink:ssamoswap.d[] instruction can be used to store this checkpoint. When the old
value at the memory location operated on by insnlink:ssamoswap.w[] or insnlink:ssamoswap.d[] is not required,
`rd` can be set to `x0`.
====
