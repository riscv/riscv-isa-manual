...
To enforce forward-edge control-flow integrity, the ext:zicfilp[] extension introduces
...
`XLEN` low order bits of the shadow stack pointer (`ssp`).# [#norm:ssp_addr]#The CSR address is
...
The `ssp` is decremented by insn:sspush[] and insn:c.sspush[] only if the store to the
...
The `ssp` is incremented by insn:sspopchk[] and insn:c.sspopchk[] only if the load from
...
[[norm:ssrdp_read]]
The insn:ssrdp[] instruction is provided to move the contents of `ssp` to a destination
register.
...
Just as for AMOs in the ext:a[] extension, insn:ssamoswap.w[] or insn:ssamoswap.d[] optionally provides release
