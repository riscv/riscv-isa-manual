...
Section xref:priv-rationale.adoc#smepmp_rationale["Smepmp" Extension for PMP Enhancements for memory access and execution prevention in Machine mode].
...
and their associated rules. See xref:machine.adoc#norm:mseccfg_enc_img[Machine security configuration (`mseccfg`) register] for the detailed
