...
include::partial$bytefield/mnscratch.edn[]
...
include::partial$bytefield/mnepc.edn[]
...
include::partial$bytefield/mncause.edn[]
...
include::partial$bytefield/mnstatus.edn[]
...
is __y__, MNRET sets `ELP` to the logical AND of __y__LPE (see xref:priv-cfi.adoc#FCFIACT[Landing-Pad-Enabled (LPE) State]) and `mnstatus`.MNPELP.
