...
include::unpriv:partial$bytefield/mnscratch.edn[]
...
include::unpriv:partial$bytefield/mnepc.edn[]
...
include::unpriv:partial$bytefield/mncause.edn[]
...
include::unpriv:partial$bytefield/mnstatus.edn[]
...
is __y__, MNRET sets `ELP` to the logical AND of __y__LPE (see xref:priv-cfi.adoc#FCFIACT[Landing-Pad-Enabled (LPE) State
]) and `mnstatus`.MNPELP.
