[[sec:zfinx]]
== "Zfinx", "Zdinx", "Zhinx", "Zhinxmin" "Zqinx" Extensions for Floating-Point in Integer Registers, Version 1.0
...
The Zqinx extension provides analogous quad-precision floating-point
instructions. The Zqinx extension depends upon the Zdinx extension.

The Zqinx extension adds all of the instructions that the Q extension
adds, _except_ for the transfer instructions FLQ, FSQ, FMV.Q.X, FMV.X.Q.

The Zqinx variants of these Q-extension instructions have the same
semantics, except that whenever such an instruction would have accessed an 
`f` register, it instead accesses the aligned `x`-register pairs with the same
number on rv64, or the aligned `x`-register groups with the same number on rv32.

=== Processing of Wider Values

Quad-precision operands in RV64Zqinx are held in aligned `x`-register
pairs, and in RV32Zqinx are held in aligned `x`-register groups,
i.e., register numbers must be even. Use of misaligned
(odd-numbered) registers for quad-width floating-point operands is
_reserved_. 

Regardless of endianness, the lower-numbered register holds the
low-order bits, and the higher-numbered register holds the high-order
bits: e.g., bits 63:0 of a quad-precision operand in RV64Zqinx might
be held in register `x14`, with bits 127:64 of that operand held in
`x15`.

When a quad-width floating-point result is written to `x0`, the entire
write takes no effect: e.g., for RV64Zqinx, writing a quad-precision
result to `x0` does not cause `x1` to be written.

When `x0` is used as a quad-width floating-point operand, the entire
operand is zero—i.e., `x1` is not accessed.
