...
in RV64I, indicated by a _W_ suffix to the opcode. These _W_
...
AUIPC (add upper immediate to _pc_) uses the same opcode as RV32I. AUIPC
is used to build _pc_-relative addresses and uses the U-type format.
...
|SUB |_rd_=_x0_ |latexmath:[$2^{10}$] .16+.^| _Reserved for future standard use_
