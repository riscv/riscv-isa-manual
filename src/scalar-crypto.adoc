...
Please refer to xref:b-st-ext.adoc#zbkb[Bit-manipulation for Cryptography] for more information.

[[zbkc-sc,Zbkc]]
==== `Zbkc` - Carry-less multiply instructions

Constant time carry-less multiply for Galois/Counter Mode.
These are separated from the xref:b-st-ext.adoc#zbkb[Bit-manipulation for Cryptography] because they
have a considerable implementation overhead which cannot be amortised
across other instructions.

Please refer to xref:b-st-ext.adoc#zbkc[Carry-less multiplication for Cryptography].
...
Please refer to xref:b-st-ext.adoc#zbkx[Crossbar permutation instructions].
...
|Zbb (xref:b-st-ext.adoc#zbb[Zbb])
|1.0.0
|Ratified
...
|Zbc (xref:b-st-ext.adoc#zbc[Zbc])
|1.0.0
|Ratified
...
the SM3 hash function cite:[gbt:sm3] cite:[iso:sm3].
...
It implements the _P0_ transform of the SM3 hash function cite:[gbt:sm3] cite:[iso:sm3].
This instruction must _always_ be implemented such that its execution
latency does not depend on the data being operated on.
...
Description::
This instruction is supported for the RV32 and RV64 base architectures.
It implements the _P1_ transform of the SM3 hash function cite:[gbt:sm3] cite:[iso:sm3].
This instruction must _always_ be implemented such that its execution
latency does not depend on the data being operated on.
...
|Zbkx (xref:b-st-ext.adoc#zbkx[Zbkx])
|v1.0
|Ratified
|===
...
| {check} | {check} | c.mul       | xref:zc.adoc#insns-c_mul[c.mul]
| {check} | {check} | c.not       | xref:zc.adoc#insns-c_not[c.not]
| {check} | {check} | c.zext.b    | xref:zc.adoc#insns-c_zext_b[c.zext.b]
|===
...
RISC-V Bitmanip extensions (see xref:b-st-ext.adoc#bits[Bitmanip]).
...
See xref:b-st-ext.adoc#zbkb[zbkb] for details of these instructions.
...
See xref:b-st-ext.adoc#zbkc[zbkc] for details of these instructions.
...
See xref:b-st-ext.adoc#zbkx[zbkx] for a complete description of these instructions.
...
ones being FIPS 140-3 cite:[NI19] cite:[NICC21] required for U.S. Federal use,
...
in BSI AIS-31 cite:[KiSc01] cite:[KiSc11] (<<crypto_scalar_es_req_ptg2>>)
...
cite:[nist:fips:202] cite:[nist:fips:180:4]
...
for their behavior cite:[HaLe98] cite:[HaLiLe99] cite:[BaLuMi:11], and (3) ample
...
cite:[nist:fips:202] cite:[nist:fips:180:4]), and the AES / CBC-MAC
