...
This chapter specifies the RVB23 profile family. RVB23 is the first major
release of the RVB series of RISC-V Application Processor Profile.
...
`RV64I` is the mandatory base ISA for `RVB23U64` and is little-endian.
...
The following extensions are localized options in both `RVA23U64` and
`RVB23U64`:

- <<zvkng,`Zvkng`>> Vector crypto NIST Algorithms with GCM.
- <<zvksg,`Zvksg`>> Vector crypto ShangMi Algorithms with GCM.

The following extensions options are localized options in `RVB23U64` but are not
present in `RVA23U64`:
...
also allow these carryless multiply options (`Zvknc` and `Zvksc`) to implement
GCM efficiently, with GHASH available as a separate option.
...
The following extensions are expansion options in both `RVA23U64` and
`RVB23U64`:
...
The following are expansion options for `RVB23U64` as they are not intended to
be made mandatory in future RVB profiles, but are listed as `RVA23U64`
development options as they are intended to become mandatory in future RVA
profiles.
...
NOTE: `Svnapot` is very low cost to provide, so is made mandatory even in RVB.
...
The following are privileged expansion options in `RVB23S64`, but are mandatory
in `RVA23S64`:
...
execution environment). `Ssstrict` (and all the other RVA23 mandates and options)
do not apply to any guest VMs run by a hypervisor. An RVA23 hypervisor can
provide guest VMs that are also RVA23-compatible but with an expanded set of
