...
RVA22S64 has four unprivileged options (`Zfh`, `V`, `Zkn`, `Zks`) from RVA22U64,
and eight privileged options (`Sv48`, `Sv57`, `Svnapot`, `Ssu64xl`, `Sstc`,
`Sscofpmf`, `Zkr`, `H`).
...
and `Shgatpa` are new extension names.  `Sha` was introduced after RVA22S64 was
ratified, but the ratified text required all of `Sha`'s constituent extensions
if the optional `H` extension was included.  Hence, offering the `Sha` option is
equivalent.
...
as there are no RVA22S64 supported options with relevant state to control in the
