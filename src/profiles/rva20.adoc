...
- <<ziccrse,**Ziccrse**>> Main memory regions with both the cacheability and coherence PMAs must
  support RsrvEventual.

NOTE: Ziccrse is a new extension name capturing this feature.

- <<ziccamoa,**Ziccamoa**>> Main memory regions with both the cacheability and coherence PMAs must
...
- <<sstvecd,**Sstvecd**>> `stvec.MODE` must be capable of holding the value 0 (Direct).  When
  `stvec.MODE=Direct`, `stvec.BASE` must be capable of holding any
  valid four-byte-aligned address.

NOTE: This is a new extension name for this feature.

- <<sstvala,**Sstvala**>> `stval` must be written with the faulting virtual address for load,
