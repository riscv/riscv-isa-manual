...
RVA20U64 has one optional extension (ext:zihpm[]).
...
- extlink:zicsr[]  CSR instructions.  These are implied by presence of ext:zicntr[] or ext:f[].
...
NOTE: ext:zifencei[] is not classed as a supported option in the user-mode
...
likely relies on the ext:zifencei[] extension.
...
RVA20S64 has one unprivileged option (ext:zihpm[]) and one privileged option
(Sv48).
