...
To enforce ordering for subsequent S-mode and U-mode accesses, software must
execute an `SFENCE.VMA` instruction with `rs1=x0` and `rs2=x0`, which
synchronizes subsequent memory accesses with all preceding SPMP CSR writes.
To enforce ordering for subsequent VS-mode and VU-mode accesses, an
`HFENCE.GVMA` instruction with `rs1=x0` and `rs2=x0` is required.
