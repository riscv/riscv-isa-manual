...
// g = o = log2(n)
//
// A[i] and B[i] are IEEE 754-encoded binary floating point numbers on (e+p) bits
// (MSB is sign, next e bits are biased exponent, last m bits are the mantissa)
// exponent bias is prodOpBias
// p = m + 1
// the output is an IEEE 754-encoded binary floating-point number on (f+q) bits
...
Rounding to odd (RTO) is not part of the IEEE 754-2008 standard (at least not until and including revision 2019).
