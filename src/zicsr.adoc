...
[#norm:Zicsr:csrrsi_csrrci_uimm_zero]#For CSRRSI and CSRRCI, if the uimm[4:0] field is zero, then these
...
illegal-instruction exceptions on accesses to read-only CSRs.# [#norm:Zicsr:csrrwi_rd_x0]#For
...
such fields within the written CSR.  [#norm:Zicsr:rs1_uimm_side_effect]#The CSRRS[I] an CSRRC[I] instructions
...
implicit CSR accesses, as performed in program order. [#norm:Zicsr:csr_access_order]#In particular,
...
modified by the CSR state.# Furthermore, [#norm:Zicsr:csrr_order]#an explicit CSR read returns the
CSR state before the execution of the instruction#, while [#norm:Zicsr:csrw_order]#an explicit CSR
