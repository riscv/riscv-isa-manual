...
have been moved out of xref:rv32.adoc[RV32I Base Integer Instruction Set, Version 2.1] into this separate
chapter.
====
...
[NOTE]
====
Since CSRRS and CSRRC perform a read-modify-write operation, any bits that read as
...
such fields within the written CSR.  [#norm:csr_rs1_uimm_side_effect]#The CSRRS[I] and CSRRC[I] instructions
...
[#norm:csr_weakly_ordered]#For the RVWMO memory consistency model (xref:rvwmo.adoc[RVWMO Memory Consistency Model]), CSR accesses are weakly
