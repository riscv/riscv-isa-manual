...
[#norm:fpr_regs]#The F extension adds 32 floating-point registers, `f0-f31`, each 32
...
[#norm:accrued_fflags]#The accrued exception flags indicate the exception conditions that have
...
use the R-type format with the OP-FP major opcode. [#norm:fadd-s_fmul-s]#FADD.S and FMUL.S
perform single-precision floating-point addition and multiplication
respectively, between _rs1_ and _rs2_#. [#norm:fsub-s]#FSUB.S performs the
single-precision floating-point subtraction of _rs2_ from _rs1_#. [#norm:fdiv-s]#FDIV.S performs the single-precision floating-point division of _rs1_ by _rs2_#. [#norm:fsqrt-s]#FSQRT.S computes the square root of _rs1_#. In each case, the result is written to _rd_.
...
[#norm:fmin-s_fmax-s]#Floating-point minimum-number and maximum-number instructions FMIN.S and FMAX.S write, respectively, the smaller or larger of _rs1_ and _rs2_ to _rd_#. [#norm:fmin-s_fmax-s_zero_compare]#For the purposes of these instructions only, the value
...
instructions are encoded in the OP-FP major opcode space. [#norm:fcvt_int_float]#FCVT.W.S or
...
register _rd_#. [#norm:fcvt_float_int]#FCVT.S.W or FCVT.S.L converts a 32-bit or 64-bit signed
integer, respectively, in integer register _rs1_ into a floating-point
number in floating-point register _rd_#. [#norm:fcvt_unsigned]#FCVT.WU.S, FCVT.LU.S, FCVT.S.WU,
...
[#norm:float_sign_inject]#Floating-point to floating-point sign-injection instructions, FSGNJ.S,
...
comparison: it only sets the invalid operation exception flag if either input is a signaling NaN#. [#norm:float_comp_NaN_input]#For all three instructions, the result is 0 if either operand is NaN#.
