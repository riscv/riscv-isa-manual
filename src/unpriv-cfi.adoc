...
jump and conditional branch instructions. [#norm:c-j_c-jal_enc#]#The `C.J` and `C.JAL` instructions
encode an offset in the immediate field of the instruction and thus are not
susceptible to control-flow subversion.# [#norm:c_jr_jalr#]#The `C.JR` and `C.JALR` RVC instructions
perform an unconditional control transfer to the address in register `rs1`.# [#norm:c_jalr#]#The
`C.JALR` additionally writes the address of the instruction following the jump
(`pc+2`) to the link register `x1` and is a procedure call.# [#norm:c_jr#]#The `C.JR` is a
...
with _rs1_=`x1` or _rs1_=`x5`. [#norm:c_jr_expand#]#A `C.JR` instruction expands to
...
program locations that are valid targets of indirect jumps or calls. [#norm:lpad_encode#]#The `LPAD`
...
opcode. [#norm:zicflip_inactive#]#When Zicfilp is not active at a privilege level or when the extension
...
for use as the landing pad label register. [#norm:zicflip_landing_pad_x7#]#To support labeled landing pads, the
indirect call/jump sites establish an expected landing pad label (e.g., using
the `LUI` instruction) in the bits 31:12 of the `x7` register.# [#norm:zicflip_lp_encoding#]#The `LPAD`
...
is a no-op. [#norm:zicflip_lp_enabled_exception#]#If Zicfilp is enabled, the `LPAD` instruction causes a
software-check exception with `__x__tval` set to "landing pad fault (code=2)"# if
any of the following conditions are true:

* [#norm:zicflip_lp_enabled_misaligned#]#The `pc` is not 4-byte aligned and `ELP` is `LP_EXPECTED`.#
* [#norm:zicflip_lp_enabled_x7_bits#]#The `ELP` is `LP_EXPECTED` and the `LPL` is not zero and the `LPL` does not
...
** [#norm:zicfiss_c-sspush#]#`C.SSPUSH x1` - encoded using `C.MOP.1`#

* Pop from the shadow stack (See <<SS_POP>>)
** [#norm:zicfiss_sspopchk_x1#]#`SSPOPCHK x1` and `SSPOPCHK x5` - encoded using `MOP.R.28`#
** [#norm:zicfiss_sspopchk_x5#]#`C.SSPOPCHK x5` - encoded using `C.MOP.5`#

* Read the value of `ssp` into a register (See <<SSP_READ>>)
** [#norm:zicfiss_ssrdp#]#`SSRDP` - encoded using `MOP.R.28`#
...
0x011. [#norm:ssp_width#]#There is no high CSR defined as the `ssp` is always as wide as the `XLEN`
of the current privilege mode.# [#norm:ssp_read_only_bits#]#The bits 1:0 of `ssp` are read-only zero. If the
...
[#norm:zicfiss_sspush_x1_x5#]#Only `x1` and `x5` registers are supported as `rs2` for `SSPUSH`.# [#norm:zicfiss_16_bit_push#]#Zicfiss
...
a link register on the shadow stack. [#norm:sspush_c-sspush#]#The `SSPUSH` and `C.SSPUSH` instructions
...
[#norm:zicfiss_sspop_x1_x5#]#Only `x1` and `x5` registers are supported as `rs1` for `SSPOPCHK`.# [#norm:zicfiss_16_bit_sspop#]#Zicfiss
