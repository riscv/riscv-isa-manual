...
indirect jump must land on a landing pad, as specified in <<IND_CALL_JMP>>. [#norm:is_lp_expected#]#If
...
the `ELP` state is `LP_EXPECTED`. [#norm:zicflip_disabled#]#If Zicfilp is not enabled then the instruction
...
[[norm:zicflip_disabled_conditions]]
...
higher addresses to lower addresses. [#norm:shadow_stack_xlen#]#Each entry on the shadow stack is `XLEN`
wide and holds the link register value.# [#norm:shadow_stack_ssp]#The `ssp` points to the top of the
...
Only `x1` and `x5` registers are supported as `rs2` for `SSPUSH`. [#norm:zicfiss_16_bit_push#]#Zicfiss
...
a link register on the shadow [#norm:sspush_c-sspush#]#stack. The `SSPUSH` and `C.SSPUSH` instructions
...
multiprocessor synchronization. [#norm:ssamoswap-w-d_semantics#]#An `SSAMOSWAP.W/D` operation has acquire
