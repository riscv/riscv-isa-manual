...
program locations that are valid targets of indirect jumps or calls. [#norm:lpad_enc]#The `LPAD`
...
[#norm:zicflip_lpad_expected]#  is_lp_expected = ( (JALR || C.JR || C.JALR) &&
...
the `LUI` instruction) in the bits 31:12 of the `x7` register. [#norm:lpad_imm_enc]#The `LPAD`
...
** [#norm:sspush_enc]#`SSPUSH x1` and `SSPUSH x5` - encoded using `MOP.RR.7`#
** [#norm:c-sspush_enc]#`C.SSPUSH x1` - encoded using `C.MOP.1`#

* Pop from the shadow stack (See <<SS_POP>>)
** [#norm:sspopchk_enc]#`SSPOPCHK x1` and `SSPOPCHK x5` - encoded using `MOP.R.28`#
** [#norm:c-sspopchk_enc]#`C.SSPOPCHK x5` - encoded using `C.MOP.5`#

* Read the value of `ssp` into a register (See <<SSP_READ>>)
** [#norm:ssrdp_enc]#`SSRDP` - encoded using `MOP.R.28`#

* [#norm:atomic_swap]#Perform an atomic swap from a shadow stack location (See <<SSAMOSWAP>>)#
...
0x011.# [#norm:ssp_sz]#There is no high CSR defined as the `ssp` is always as wide as the `XLEN`
of the current privilege mode.# [#norm:ssp-field_1_0]#The bits 1:0 of `ssp` are read-only zero. If the
...
a link register on the shadow stack. [#norm:sspush_c-sspush_instr]#The `SSPUSH` and `C.SSPUSH` instructions
...
[listing]
----
[#norm:rv32_ssamoswap-w_rv64_ssamoswap-d_exceptions]#
...
[listing]
----
[#norm:rv64_ssamoswap-w_exceptions]#
  if privilege_mode != M && menvcfg.SSE == 0
      raise illegal-instruction exception
  else if S-mode not implemented
      raise illegal-instruction exception
