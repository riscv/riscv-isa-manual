...
and vice versa.  SFENCE.VMA is also used to invalidate entries in the
...
An implicit read may return any translation for an address that was valid at
...
valid at any time since the most recent SFENCE.VMA that subsumes that address.
...
\begin{commentary}
  The address-translation cache abstraction used less explicitly in earlier
  revisions of this specification, but it was made more explicit in order to
  faciliate defining the behavior of NAPOT PTEs in Chapter~\ref{napotpages}.
  In particular, an implicit load that hits on an address-translation cache
  entry previously fetched due to a separate NAPOT PTE aliasing the 4KiB region
  originally targeted is not easily described in terms of access ordering
  alone.  In such a case, the PTE value being loaded was originally written to
  an address that differs from the address of the PTE targeted by the
  address-translation algorithm, and this complicates the memory ordering
  analysis.  NAPOT PTEs notwithstanding, the inclusion of more explicit
  language regarding the address-translation cache is not intended to imply any
  actual change in behavior from prior versions of this specification.
...
C & V & Description \\
\hline
0 & 0 & The PTE is invalid; all bits except C are don't cares and can be used freely by software \\
0 & 1 & The PTE is valid and uses standard address-translation rules \\
1 & 0 & {\em Reserved for standard use} \\
1 & 1 & {\em Designated for custom use} \\
\hline
\end{tabular}
\end{center}
\caption{Meaning of PTE C and V bits in Sv39.}
\label{tab:pte_cv_bits}
\end{table}

The C bit is used to indicate that a PTE uses a custom implementation-specific
encoding, as shown in Table~\ref{tab:pte_cv_bits}.

The N bit indicates that the page represents a naturally-aligned power-of-two
range of contiguous translations, as defined in the Zsn extension in
Chapter~\ref{napotpages}.

Bits 61--54 are reserved for future standard use.
...
\chapter{``Zsn'' Standard Extension for NAPOT Translation Contiguity, Version 0.1}
\label{napotpages}

The N bit being set in an Sv39 or Sv48 PTE indicates that the PTE represents a
...
\begin{commentary}
  ...
  within the pre-existing Sv39 and Sv48 PTE formats so as not to disrupt
...
\multicolumn{4}{l}{{\tt z}: reserved for future standard use; must be set to zero} \\
\end{tabular}
\end{center}
...
\begin{commentary}
  ...
  \hline
  0 & {\tt y~yyyy~yyyy} & 0                 &  Non-NAPOT 4KiB PTE      \\
  1 & {\tt y~yyyy~yy1z} & 2                 &  16KiB contiguous region \\
  1 & {\tt y~yyyy~1z0z} & 4                 &  64KiB contiguous region \\
  1 & {\tt y~yy1z~0z0z} & 6                 &  256KiB contiguous region \\
  1 & {\em other}       & $-$               &  {\em Reserved for standard use} \\
  \hline
  \multicolumn{4}{l}{{\tt y}: subset of PPN used for address translation} \\
  ...
  This allows the $z$ bits to potentially be used for other purposes instead,
  and it simplifies the design space in the meantime.
\end{commentary}
...
\begin{itemize}
  \item Instead of returning the original value of $pte$, implicit reads of a
...
\begin{commentary}
  ...
  generally should first mark all of the PTEs invalid, then issue an
  SFENCE.VMA, then update the PTE(s), as described in
  Section~\ref{sec:sfence.vma}, unless any inconsistencies are known to be
  benign.  If any inconsistencies do exist, then the effect is the same as when
  SFENCE.VMA is used incorrectly: one of the translations will be chosen, but
