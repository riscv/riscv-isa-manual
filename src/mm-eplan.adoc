...
<<litmus-sample, Table 1>>, the final value of `a0`
...
<<litmus-key, Table 2>>. Of the listed relations, rf edges between
...
For example, in <<litmus-sample, Table 1>>, `a0=1`
...
Consider the <<litms_sb_forward, Table 3>>. When running this program on an implementation with
...
a! image::unpriv:graphviz/litmus_sb_fwd.png[]
!===
|===
...
<<litmus_ppoca, Table 4>>. In this example, (d) is
...
Consider the litmus test of <<litmus-rsw, Table 6>>.
...
Executions of the test in <<litmus-rsw, Table 6>> in
...
Consider <<spinlock_atomics, Listing 1>>.
...
Now, consider the alternative in <<spinlock_fences, Listing 2>>. In
...
FENCE RW,W in <<spinlock_fences, Listing 2>>, not by
the _rl_ in <<spinlock_atomics, Listing 1>>.
...
The terms in xref:rvwmo.adoc#mem-depencies#mem-dependencies[Syntactic Dependencies] work as follows. Instructions
...
<<fflags, Listing 3>>, (c) has a syntactic dependency on both (a) and (b).
...
Consider <<control1, Listing 5>> the
...
Likewise, consider <<control2, Listing 6>>.
...
<<litmus_lb_lrsc, Table 7>>. Suppose a
...
data for that store are known. Consider <<litmus_datarfi, Table 8>> (f) cannot be
...
as in <<litmus:addrdatarfi_no, Table 10>>,
...
[[litmus:addrdatarfi_no, Table 10]]
...
Consider <<litmus:addrdatarfi_no, Table 10>> (f) cannot be
...
[[memory_porting]]
[#memory_porting, Memory Porting]
== Code Porting and Mapping Guidelines

[[tsomappings, Table 11]]
...
<<tsomappings, Table 11>> provides a mapping from TSO memory
...
Alternatives to <<tsomappings, Table 11>> are also possible. A TSO
...
<<powermappings, Table 12>> provides a mapping from Power memory
...
<<armmappings, Table 13>> provides a mapping from ARM memory
...
between the store-release and load-acquire; <<armmappings, Table 13>>
...
With regards to <<linuxmappings, Table 14>>, other
...
<<linuxmappings, Table 14>> provides a mapping of Linux memory
...
<<c11mappings_hypothetical, Table 16>> will suffice. Note however that
the two mappings only interoperate correctly if
`atomic_<op>(memory_order_seq_cst)` is mapped using an LR that has both
_aq_ and _rl_ set.
Even more importantly, a <<c11mappings, Table 15>> sequentially consistent store,
followed by a <<c11mappings_hypothetical, Table 16>> sequentially consistent load
can be reordered unless the <<c11mappings, Table 15>> mapping of stores is
strengthened by either adding a second fence or mapping the store
to `amoswap.rl` instead.

[[c11mappings, Table 15]]
...
[[c11mappings_hypothetical, Table 16]]
...
2+!`*` must be `lr.{w|d}.aqrl` in order to interoperate with code mapped per <<c11mappings, Table 15>>
|===
