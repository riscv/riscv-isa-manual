...
xref:rvwmo.adoc[RVWMO Memory Consistency Model], using more informal
...
provided in xref:rvwmo.adoc[RVWMO Memory Consistency Model] and in the rest of
...
the _herd_ models presented in xref:mm-formal.adoc#sec:herd[Formal Axiomatic Specification in Herd]. This notation is explained in
...
[IMPORTANT]
====
xref:rvwmo.adoc#ax-load[Load Value Axiom]: Each byte of each load _i_ returns the value written
...
* (b) precedes (d): by rule xref:rvwmo.adoc#overlapping-ordering[4]
...
* (f) precedes (h): by rule xref:rvwmo.adoc#overlapping-ordering[4]
...
[IMPORTANT]
====
xref:rvwmo.adoc#ax-atom[Atomicity Axiom] (for Aligned Atomics): If r and w are paired load and
...
[IMPORTANT]
====
xref:rvwmo.adoc#ax-prog[Progress Axiom]: No memory operation may be preceded in the global
...
[[mm-overlap]]
==== Overlapping-Address Orderings (xref:rvwmo.adoc#overlapping-ordering[Rules 1-3])

[NOTE]
====
xref:rvwmo.adoc#overlapping-ordering[Rule 1]: b is a store, and a and b access overlapping memory addresses

xref:rvwmo.adoc#overlapping-ordering[Rule 2]: a and b are loads, x is a byte read by both a and b, there is no
store to x between a and b in program order, and a and b return values
for x written by different memory operations

xref:rvwmo.adoc#overlapping-ordering[Rule 3]: a is generated by an AMO or SC instruction, b is a load, and b
...
(g). Therefore, xref:rvwmo.adoc#overlapping-ordering[rule 2] forbids this CoRR violation
from occurring. As such, xref:rvwmo.adoc#overlapping-ordering[rule 2] strikes a careful
...
There is one more overlapping-address rule: xref:rvwmo.adoc#overlapping-ordering[rule 3] simply states that a value cannot
...
notably, xref:rvwmo.adoc#overlapping-ordering[rule 3] states that hardware
...
[[mm-fence]]
==== Fences (xref:rvwmo.adoc#overlapping-ordering[Rule 4])

[IMPORTANT]
====
Rule xref:rvwmo.adoc#overlapping-ordering[4]: There is a FENCE instruction that orders a before b
====
...
sets and hence are no-ops. Of the ten non-trivial options, six are
...
FENCE instructions using other combinations of PR, PW, SR, and SW are not
normally used in the Linux or C++ memory models but are otherwise well
defined.
...
[[sec:memory:acqrel]]
==== Explicit Synchronization (xref:rvwmo.adoc#overlapping-ordering[Rules 5-8])

[IMPORTANT]
====
xref:rvwmo.adoc#overlapping-ordering[Rule 5]: a has an acquire annotation

xref:rvwmo.adoc#overlapping-ordering[Rule 6]: b has a release annotation

xref:rvwmo.adoc#overlapping-ordering[Rule 7]: a and b both have RCsc annotations

xref:rvwmo.adoc#overlapping-ordering[Rule 8]: a is paired with b
====
...
==== Syntactic Dependencies (xref:rvwmo.adoc#overlapping-ordering[Rules 9-11])

[[ppo-addr]]
[IMPORTANT]
====
xref:rvwmo.adoc#overlapping-ordering[Rule 9]: b has a syntactic address dependency on a

xref:rvwmo.adoc#overlapping-ordering[Rule 10]: b has a syntactic data dependency on a

xref:rvwmo.adoc#overlapping-ordering[Rule 11]: b is a store, and b has a syntactic control dependency on a
====
...
The terms in xref:rvwmo.adoc#mem-dependencies[Syntactic Dependencies] work as follows. Instructions
...
Notably, PPO rules xref:rvwmo.adoc#overlapping-ordering[9-11] are also
...
rule xref:rvwmo.adoc#ppo[11] in turn implies that any subsequent store
...
xref:rvwmo.adoc#source-dest-regs[Source and Destination Register Listings] does _not_ imply
...
[[memory-ppopipeline]]
==== Pipeline Dependencies (xref:rvwmo.adoc#overlapping-ordering[Rules 12-13])

[[addrdatarfi]]
[IMPORTANT]
====
xref:rvwmo.adoc#overlapping-ordering[Rule 12]: b is a load, and there exists some store m between a and b in
program order such that m has an address or data dependency on a,
and b returns a value written by m

xref:rvwmo.adoc#overlapping-ordering[Rule 13]: b is a store, and there exists some instruction m between a and
b in program order such that m has an address dependency on a
====

[[litmus_datarfi]]
.Because of PPO xref:rvwmo.adoc#overlapping-ordering[rule 12] and the data dependency from (d) to (e), (d) must also precede (f) in the global memory order (outcome forbidden)
[float="center",align="center",cols=".^1a,.^1a",frame="none",grid="none",options="noheader"]
|===
|
[%autowidth,float="center",align="center",cols="^,<,^,<",options="header",]
...
PPO rules xref:rvwmo.adoc#overlapping-ordering[12] and xref:rvwmo.adoc#overlapping-ordering[13] reflect behaviors of almost all real processor
pipeline implementations. Rule xref:rvwmo.adoc#overlapping-ordering[12]
...
Rule xref:rvwmo.adoc#overlapping-ordering[13] makes a similar observation to the
...
of rule xref:rvwmo.adoc#overlapping-ordering[11].
...
FENCE.I is described in xref:zifencei.adoc["Zifencei" Extension for Instruction-Fetch Fence], the
...
defined in xref:rvwmo.adoc[RVWMO Memory Consistency Model], with the exception
...
Rule xref:rvwmo.adoc#overlapping-ordering[3] forbids forwarding of values from
...
* programmers will use PPO rules xref:rvwmo.adoc#overlapping-ordering[1] and xref:rvwmo.adoc#overlapping-ordering[4-8] regularly and actively.
* expert programmers will use PPO rules xref:rvwmo.adoc#overlapping-ordering[9-11] to speed up critical paths
of important data structures.
* even expert programmers will rarely if ever use PPO rules xref:rvwmo.adoc#overlapping-ordering[2-3] and
xref:rvwmo.adoc#overlapping-ordering[12-13] directly.
These are included to facilitate common microarchitectural optimizations
(rule xref:rvwmo.adoc#overlapping-ordering[2]) and the operational formal modeling approach (rules xref:rvwmo.adoc#overlapping-ordering[3] and
xref:rvwmo.adoc#overlapping-ordering[12-13]) described
in xref:mm-formal.adoc#operational[An Operational Memory Model]. They also facilitate the
...
* PPO rules xref:rvwmo.adoc#overlapping-ordering[1] and xref:rvwmo.adoc#overlapping-ordering[3-6] reflect
well-understood rules that should pose few surprises to architects.
* PPO rule xref:rvwmo.adoc#overlapping-ordering[2] reflects a natural and common hardware
optimization, but one that is very subtle and hence is worth double
checking carefully.
* PPO rule xref:rvwmo.adoc#overlapping-ordering[7] may not be immediately obvious to
architects, but it is a standard memory model requirement
* The load value axiom, the atomicity axiom, and PPO rules
xref:rvwmo.adoc#overlapping-ordering[8-13] reflect rules that most
...
also orders I/O)
* Ignore all PPO rules except for rules xref:rvwmo.adoc#overlapping-ordering[4] through xref:rvwmo.adoc#overlapping-ordering[7], since the rest
