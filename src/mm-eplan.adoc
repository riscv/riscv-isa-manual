...
<<memorymodel, Chapter 17>>, using more informal
...
discrepancies are listed in <<discrepancies, Section A.7>>. Any
...
model. <<litmus-sample, Figure 1>> shows an example
...
[[litmus-sample]]
.Sample litmus test
graphviz::images/graphviz/litmus_sample.txt[align="center",width=180]
...
<<litmus-sample, Figure 1>>, the final value of `a0`
...
<<sec:herd, Appendix B>>. This notation is explained in
<<litmus-key, Table 44>>. Of the listed relations, rf edges between
...
For example, in <<litmus-sample, Figure 1>>, `a0=1`
...
[NOTE]
====
_The litmus tests repository also provides instructions on how to run the
litmus tests on RISC-V hardware and how to compare the results with the
operational and axiomatic models._

_In the future, we expect to adapt these memory model litmus tests for
use as part of the RISC-V compliance test suite as well._
====
...
[IMPORTANT]
====
_Load Value Axiom: Each byte of each load _i_ returns the value written
to that byte by the store that is the latest in global memory order among
the following stores:_

. _Stores that write that byte and that precede i in the global memory
order_
. _Stores that write that byte and that precede i in program order_
====
...
Consider the <<litms_sb_forward, Figure 2>>. When running this program on an implementation with
...
graphviz::images/graphviz/litmus_sb_fwd.txt[align="center",width=80]

[[litms_sb_forward]]
_A store buffer forwarding litmus test (outcome permitted)_
[cols="1^,2<,1^,2<",options="header",align="center"]
|===
|Hart 0 | |Hart 1 |
| |li t1, 1 | |li t1, 1
...
* (b) precedes (d): by rule ppo:fence[[ppo:fence]]
...
* (f) precedes (h): by rule ppo:fence[[ppo:fence]]
...
graphviz::images/graphviz/litmus_ppoca.txt[align="center"width=130]
...
<<litmus_ppoca, Table 45>>. In this example, (d) is
...
[IMPORTANT]
====
_Atomicity Axiom (for Aligned Atomics): If r and w are paired load and
...
example, the SC instructions in <<litmus_lrsdsc, Figure 4>> may (but are not
...
of <<litmus_lrsdsc, Figure 4>> as identical) will not
...
[IMPORTANT]
====
_<<mm-progress, Progress Axiom>>: No memory operation may be preceded in the global
memory order by an infinite sequence of other memory operations._
====
...
[NOTE]
====
_Rule 1: b is a store, and a and b access overlapping memory addresses_

_Rule 2: a and b are loads, x is a byte read by both a and b, there is no
store to x between a and b in program order, and a and b return values
for x written by different memory operations_

_Rule 3: a is generated by an AMO or SC instruction, b is a load, and b
returns a value written by a_
====
...
<<loadvalueaxiom, Section A.3.2>>, this reflects the observable
...
graphviz::images/graphviz/litmus_lb_lrsc.txt[align="center",width=130]
...
Consider the litmus test of <<frirfi, Figure 4>>, which is one particular
...
Consider the litmus test of <<litmus-rsw, Figure 5>>.
...
Executions of the test in <<litmus-rsw, Figure 5>> in
...
(g). Therefore, PPO rule 2 forbids this CoRR violation
from occurring. As such, PPO rule 2 strikes a careful
...
notably, PPO rule 3 states that hardware
...
[IMPORTANT]
====
_Rule 4: There is a FENCE instruction that orders a before b_
====
...
[[sec:memory:acqrel]]
==== Explicit Synchronization (Rules 5–8)

[IMPORTANT]
====
_Rule 5: a has an acquire annotation_

_Rule 6: b has a release annotation_

_Rule 7: a and b both have RCsc annotations_

_Rule 8: a is paired with b_
====
...
==== Syntactic Dependencies (Rules ppo:addr[[ppo:addr]]–11)

[[ppo-addr]]
[IMPORTANT]
====
_Rule 9: b has a syntactic address dependency on a_

_Rule 10: b has a syntactic data dependency on a_

_Rule 11: b is a store, and b has a syntactic control dependency on a_
====
...
The terms in <<mem-dependencies, Section 17.1.2>> work as follows. Instructions
...
<<fflags, Example 3>>, (c) has a syntactic dependency on both (a) and (b).
...
<<address, Example 4>>, even though `a1` XOR
...
Consider <<control1, Example 5>> the
...
Likewise, consider <<control2, Example 6>>.
...
Notably, PPO rules <<ppo-addr>>–11 are also
...
<<litmus_lb_lrsc, Figure 6>>. Suppose a
...
<<source-dest-regs, Section 17.3>> does _not_ imply
...
[IMPORTANT]
====
_Rule 12: b is a load, and there exists some store m between a and b in
program order such that m has an address or data dependency on a,
and b returns a value written by m_

_Rule 13: b is a store, and there exists some instruction m between a and
b in program order such that m has an address dependency on a_
====

[[litmus_datarfi]]
.Because of PPO rule 12 and the data dependency from (d) to (e), (d) must also precede (f) in the global memory order (outcome forbidden)
graphviz::images/graphviz/litmus_datarfi.txt[align="center",width=130]
...
data for that store are known. Consider <<addrdatarfi, Figure 7>> (f) cannot be
...
graphviz::images/graphviz/litmus_datacoirfi.txt[align="center",width=85]
...
as in <<litmus:addrdatarfi_no, Figure 9>>,
...
Rule 13 makes a similar observation to the
...
Consider <<addrpo, Figure 9>> (f) cannot be
...
FENCE.I is described in <<zifencei, Chapter 3>>, the
...
defined in <<memorymodel, Chapter 17>>, with the exception
...
<<tsomappings, Table 46>> provides a mapping from TSO memory
...
Alternatives to <<tsomappings, Table 46>> are also possible. A TSO
...
<<powermappings, Table 47>> provides a mapping from Power memory
...
<<armmappings, Table 48>> provides a mapping from ARM memory
...
With regards to <<linuxmappings, Table 49>>, other
...
<<linuxmappings, Table 49>> provides a mapping of Linux memory
...
<<lkmm_ll, Example 7>>. If that will indeed be
...
<<c11mappings, Table 50>> provides a mapping of C11/C++11 atomic
...
<<c11mappings_hypothetical, Table 51>> will suffice. Note however that
...
!`^&#2731;` must be `lr.{w|d}.aqrl` in order to interoperate with code mapped per <<c11mappings, Table 50>> !
|===
...
12-13) described
in <<operational, Section B.3>>. They also facilitate the
...
* emulate _aq_ and _rl_ as described in <<porting>>
...
particularly in cases such as RSW <<overlap>>
...
<<discrepancy-rsw1>>-<<discrepancy-rsw3>>.
