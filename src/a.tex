...
value in {\em rd}, and establishes a {\em reservation} for the invoking hart
over a set of bytes that contains at minimum all the bytes in the addressed
word and at most its entire {\em reservation set}---a set of bytes that
subsumes the bytes in the addressed word.
SC.W conditionally writes a word in {\em rs2} to the address in {\em rs1}.
The SC.W succeeds if the reservation is still valid and covers all the bytes
being written.  If the SC.W succeeds, the instruction writes the addressed
bytes and returns zero to {\em rd}.  If the SC.W fails, the instruction
does not write the addressed bytes and returns a non-zero value to {\em rd}.
Regardless of success or failure, the SC.W invalidates any reservation
held by this hart.
...
Reservation sets are implementation defined arbitrarily large sets of bytes.
Reservations are wholly contained within a reservation set.
In systems with memory translation, it is implementation defined whether
two virtual addresses mapping to the same physical address refer to the
same reservation or reservation set.  It is also implementation defined
whether the same virtual address remapped to a different physical address
between the LR and SC refers to the same reservation or reservation set.
...
operations; otherwise, executing an LR instruction generates an access
exception, and executing an SC instruction either generates an access
exception or fails.  The execution environment is responsible for
communicating which regions are reservable.

\begin{commentary}
For compatibility with certain existing bus standards, SC is not defined
as always generating an exception when accessing non-reservable addresses.
However, an exception is expected and highly recommended where possible.
\end{commentary}

An SC succeeds if all of the following conditions are true when the SC
is executed:
\vspace{-0.2in}
\begin{itemize}
\parskip 0pt
...
\item The reservation includes all of the bytes of the word or doubleword
  addressed by the SC.
\end{itemize}
\vspace{-0.2in}
If any of the above conditions are false when the SC is executed, the SC fails.

The reservation on hart {\em H} is invalidated as follows:
\vspace{-0.2in}
\begin{itemize}
\parskip 0pt
\itemsep 1pt
\item Always, if {\em H} executes an SC.
\item Always, if {\em H} executes an LR to a different set of bytes than
  the existing reservation.  In this case, a new reservation is established.
\item Always, if any memory write performed by any hart or device to a byte
  included in the reservation is observed between the LR and the SC executed
  by {\em H}.  A memory write may be, for example, an unconditional store,
  an AMO, a successful SC, a device write, or any other effective or implied
  write such as a PTE update caused by MMU table walk.
\item At the implementation's discretion, for such a memory write to bytes
  not included in the reservation but at least one of which is included
  in its containing reservation set.
\item At the implementation's discretion, occasionally for any reason,
  conditional upon never impeding eventual success of constrained LR/SC
  sequences by some hart for each reservation set.
\end{itemize}
...
the reservation within a non-shared cache.
...
\item The LR instruction's reservation is invalidated by one of the possible
  causes of invalidation listed earlier other than {\em H} executing LR or SC.
...
environment are executing constrained LR/SC loops, and no harts or
devices in the execution environment write to the reservation set
(as described in causes of reservation invalidation),
then at least one hart will eventually exit its constrained LR/SC loop.
...
For example, capacity misses caused by another hart sharing the cache cannot
