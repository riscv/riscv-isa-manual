...
Reservation sets provide implementation leeway to manage reservations
at a granularity matching hardware design.  A common implementation
is to map reservation sets in cacheable space to cache lines,
harmonizing reservation invalidation with the cache coherence protocol.

It is software's joint responsibility to ensure forward progress, for
example by avoiding the allocation of data subject to progress-impeding
writes in the same reservation set(s) as the LR/SC word.

Where this is not a concern, an implementation might define a
single reservation set covering the entire address space.
At the other extreme, a suitably constrained system might define
byte-sized reservation sets, relieving software of the need for
special allocation of LR/SC data for purposes of forward progress
(not necessarily relieving it of a similar need of careful
allocation to avoid false-sharing for performance purposes).
\end{commentary}
