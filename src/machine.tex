...
If an instruction raises multiple synchronous exceptions, the decreasing
priority order of either Table~\ref{exception-priority} or
Table~\ref{exception-priority2} indicates which exception is taken and reported
in {\tt mcause}.  We allow two different priority schemes -- address misaligned
exceptions can either have higher priority than page faults or vice versa.  An
implementation is free to choose either priority scheme, but it cannot be
changed dynamically. The priority of any custom synchronous exceptions is
...
\end{tabular}
\end{center}
\caption{Synchronous exception priority in decreasing priority order (first option).}
...
\end{tabular}
\end{center}
\caption{Synchronous exception priority in decreasing priority order (second option).}
\label{exception-priority2}
\end{table*}

\begin{commentary}
For implementations which raise an exception on every misaligned access, the
address misaligned exception can be detected earlier than a page fault. But for
implementations which do not raise address misaligned exceptions for devices
that support misaligned accesses, the virtual address has to first be
translated to a physical address to determine which device to access, only
after which one can determine if the device supports misaligned accesses.
\end{commentary}
