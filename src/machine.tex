...
The base width is given by $\mbox{XLEN}=2^{\mbox{MXL+4}}$.
\end{commentary}
...
that control the value of XLEN for S-mode and U-mode, respectively.
The encoding of these fields is the same as the MXL field of {\tt
  misa}, shown in Table~\ref{misabase}.  RV32 systems do not support
SXL and UXL and always use XLEN=32 for S-mode and U-mode.

The SXL field is only implemented if S-mode is implemented, and is
otherwise hard-wired to zero.  The allowed values for SXL may be a
function of {\tt misa} and so changing {\tt misa} may cause SXL to
change as a side effect.  An implementation must enforce that
SXL$\leq$MXL.

The UXL field is only implemented if U-mode is implemented, and is
otherwise hard-wired to zero.  UXL may be a function of the value in
{\tt misa}, and changing the value in {\tt misa} can change the value
of UXL as a side effect. If S-mode is supported, the allowed values
for UXL may additionally be a function of SXL, so changing SXL may
cause UXL to change as a side effect.  An implementation must enforce
that UXL$\leq$SXL$\leq$MXL.  When an instruction explicitly writes
both SXL and UXL together (such as a CSR instruction that writes {\tt
  mstatus}), the effect is as though SXL is written first, any side
effects from changing SXL occur, and then UXL is written.  An
implementation can implement UXL as a read-only copy of SXL.

\begin{commentary}
The values in SXL and UXL can potentially depend on more than just the
MXL field in {\tt misa}.  For example, if the S bit in {\tt misa} is
writeable, then this can alter whether the SXL field affects the value
of UXL. Future extensions to {\tt misa} might also cause such
dependencies.
\end{commentary}
