...
|RASEMU |[#norm:mctrctl-rasemu_op]#Enables RAS (Return Address Stack) Emulation Mode.#  See xref:ras-emulation-mode[RAS (Return Address Stack) Emulation Mode].

|MTE |[#norm:mctrctl-mte_op]#Enables recording of traps to M-mode when M=0.#  See xref:external-traps[External Traps].

|STE |[#norm:mctrctl-ste_op]#Enables recording of traps to S-mode when S=0.#  See xref:external-traps[External Traps].

|BPFRZ |[#norm:mctrctl-bpfrz_op]#Set `sctrstatus`.FROZEN on a breakpoint exception that traps to M-mode or S-mode.# See xref:freeze[Freeze].

|LCOFIFRZ |[#norm:mctrctl-lcofifrz_op]#Set `sctrstatus`.FROZEN on local-counter-overflow interrupt (LCOFI) that traps to M-mode or S-mode.# See xref:freeze[Freeze].

|EXCINH |[#norm:mctrctl-excinh_op]#Inhibit recording of exceptions.#  See xref:transfer-type-filtering[Transfer Type Filtering].

|INTRINH |[#norm:mctrctl-intrinh_op]#Inhibit recording of interrupts.#  See xref:transfer-type-filtering[Transfer Type Filtering].

|TRETINH |[#norm:mctrctl-tretinh_op]#Inhibit recording of trap returns.#  See xref:transfer-type-filtering[Transfer Type Filtering].

|NTBREN |[#norm:mctrctl-ntbren_op]#Enable recording of not-taken branches.#  See xref:transfer-type-filtering[Transfer Type Filtering].

|TKBRINH |[#norm:mctrctl-tkbrinh_op]#Inhibit recording of taken branches.#  See xref:transfer-type-filtering[Transfer Type Filtering].

|INDCALLINH |[#norm:mctrctl-indcallinh_op]#Inhibit recording of indirect calls.#  See xref:transfer-type-filtering[Transfer Type Filtering].

|DIRCALLINH |[#norm:mctrctl-dircallinh_op]#Inhibit recording of direct calls.#  See xref:transfer-type-filtering[Transfer Type Filtering].

|INDJMPINH |[#norm:mctrctl-indjmpinh_op]#Inhibit recording of indirect jumps (without linkage).#  See xref:transfer-type-filtering[Transfer Type Filtering].

|DIRJMPINH |[#norm:mctrctl-dirjmpinh_op]#Inhibit recording of direct jumps (without linkage).#  See xref:transfer-type-filtering[Transfer Type Filtering].

|CORSWAPINH |[#norm:mctrctl-corswapinh_op]#Inhibit recording of co-routine swaps.#  See xref:transfer-type-filtering[Transfer Type Filtering].

|RETINH |[#norm:mctrctl-retinh_op]#Inhibit recording of function returns.#  See xref:transfer-type-filtering[Transfer Type Filtering].

|INDLJMPINH |[#norm:mctrctl-indljmpinh_op]#Inhibit recording of other indirect jumps (with linkage).#  See xref:transfer-type-filtering[Transfer Type Filtering].

|DIRLJMPINH |[#norm:mctrctl-dirljmpinh_op]#Inhibit recording of other direct jumps (with linkage).#  See xref:transfer-type-filtering[Transfer Type Filtering].
|Custom[3:0] | [#norm:mctrctl-custom_op]#WARL bits designated for custom use.#  The value 0 must correspond to standard behavior.  See xref:custom-extensions[Custom Extensions].
|===
...
|STE |[#norm:vsctrctl-ste_op]#Enables recording of traps to VS-mode when S=0.#  See xref:external-traps[External Traps]..
|BPFRZ |[#norm:vsctrctl-bpfrz_op]#Set `sctrstatus`.FROZEN on a breakpoint exception that traps to VS-mode.# See xref:freeze[Freeze].
|LCOFIFRZ |[#norm:vsctrctl-lcofifrz_op]#Set `sctrstatus`.FROZEN on local-counter-overflow interrupt (LCOFI) that traps to VS-mode.# See xref:freeze[Freeze].
...
[#norm:sctrdepth-depth_op1]#The depth of the CTR buffer dictates the number of entries to which the hardware records transfers. For a depth of N, the hardware records transfers to entries 0..N-1. All xref:entry-registers[Entry Registers] read as '0' and are read-only when the selected entry is in the range N to 255.  When the depth is increased, the newly accessible entries contain unspecified but legal values.#

[#norm:sctrdepth-depth_param]#It is implementation-specific which DEPTH value(s) are supported.#
|===

[#norm:sctrdepth_mode]#Attempts to access `sctrdepth` from VS-mode or VU-mode raise a virtual-instruction exception, unless CTR state enable access restrictions apply.#  See xref:state-enable-access-control[State Enable Access Control].
...
|===
|Field |Description
|WRPTR |[#norm:sctrstatus-wrptr]#WARL field that indicates the physical CTR buffer entry to be written next.  It is incremented after new transfers are recorded (see xref:behavior[Behavior]), though there are exceptions when `__x__ctrctl`.RASEMU=1, see xref:ras-emulation-mode[RAS (Return Address Stack) Emulation Mode].  For a given CTR depth (where depth = 2^(DEPTH+4)^), WRPTR wraps to 0 on an increment when the value matches depth-1, and to depth-1 on a decrement when the value is 0.  Bits above those needed to represent depth-1 (e.g., bits 7:4 for a depth of 16) are read-only 0. On depth changes, WRPTR holds an unspecified but legal value.#
|FROZEN |[#norm:sctrstatus-frozen_op]#Inhibit transfer recording.# See xref:freeze[Freeze].
|===

[#norm:Ssctr_sctrstatus_acc]#Undefined bits in `sctrstatus` are WPRI. Status fields may be added by future extensions,
and software should ignore but preserve any fields that it does not recognize.  Undefined  bits must be implemented as read-only 0, unless a custom extension is implemented and enabled# (see xref:custom-extensions[Custom Extensions]).
...
See xref:state-enable-access-control[State Enable Access Control] for cases where CTR accesses from S-mode and VS-mode may be restricted.
...
|CCV |[#norm:ctrdata-ccv]#Cycle Count Valid.# See xref:cycle-counting[Cycle Counting]. |WARL
...
xref:cycle-counting[Cycle Counting]. |WARL
|===

[#norm:ctrdata_undef]#Undefined bits in `ctrdata` are WPRI. Undefined bits must be implemented as read-only 0, unless a xref:custom-extension[Custom extension] is implemented and enabled.#

[NOTE]
[%unbreakable]
====
_Like the xref:transfer-type-filtering[Transfer Type Filter] bits in `mctrctl`, the `ctrdata`.TYPE bits leverage the E-trace itype encodings._
====
...
* [#norm:sctrclr_op1]#Zeroes all  CTR xref:entry-registers[Entry Registers], for all DEPTH values#
* [#norm:sctrclr_op2]#Zeroes the CTR cycle counter and CCV# (see xref:cycle-counting[Cycle Counting])
...
[#norm:sctrclr_exceptions]#SCTRCLR raises an illegal-instruction exception in U-mode, and a virtual-instruction exception in VU-mode, unless CTR state enable access restrictions apply.#  See xref:state-enable-access-control[State Enable Access Control].

[[state-enable-access-control]]
=== State Enable Access Control

When Smstateen is implemented, the `mstateen0`.CTR bit controls access to CTR register state from privilege modes less privileged than M-mode.  [#norm:mstateen_ctr1]#When `mstateen0`.CTR=1, accesses to CTR register state behave as described in xref:CSRs[CSRs] and xref:entry-registers[Entry Registers] above, while SCTRCLR behaves as described in xref:supervisor-ctr-clear-instruction[Supervisor CTR Clear Instruction].#  [#norm:mstateen_ctr0]#When `mstateen0`.CTR=0 and the privilege mode is less privileged than M-mode, the following operations raise an illegal-instruction exception:#
...
[#norm:hstateen_vs]#When `mstateen0`.CTR=1 and `hstateen0`.CTR=1, VS-mode accesses to supervisor CTR state behave as described in xref:CSRs[CSRs] and xref:entry-registers[Entry Registers] above, while SCTRCLR behaves as described in xref:supervisor-ctr-clear-instruction[Supervisor CTR Clear Instruction].  When `mstateen0`.CTR=1 and `hstateen0`.CTR=0, both VS-mode accesses to supervisor CTR state and VS-mode execution of SCTRCLR raise a virtual-instruction exception.#
...
[%unbreakable]
====
_See xref:indirect-csr.adoc[Indirect CSR] for how bit 60 in `mstateen0` and `hstateen0` can also restrict access to `sireg*`/`siselect` and `vsireg*`/`vsiselect` from privilege modes less privileged than M-mode._
====
...
|*Enabled* |*Disabled*
.2+|*Trap* |*Enabled* |[#norm:trap_ee]#Recorded#. | [#norm:trap_ed]#External trap.  Not recorded by default,# but see xref:external-traps[External Traps]..
...
* `mctrctl`.MTE, `sctrctl`.STE, and `vsctrctl`.STE are used to determine whether an external trap is recorded (see xref:external-traps[External Traps].);
* `sctrctl`.LCOFIFRZ and `sctrctl`.BPFRZ determine whether CTR becomes frozen (see xref:freeze[Freeze])
* For all other `__x__ctrctl` fields, the value in `vsctrctl` is used.

[NOTE]
====
_Consider an exception that traps from VU-mode to HS-mode, with `vsctrctl`.U=1 and `sctrctl`.S=1.  Because both the source mode and target mode are enabled for recording, whether the trap is recorded then depends on the CTR configuration (e.g., the xref:transfer-type-filtering[Transfer Type Filter] bits) in `vsctrctl`, not in `sctrctl`._
====
...
[%unbreakable]
====
_As described in xref:cycle-counting[Cycle Counting],
...
remaining call entry was recorded (see xref:ras-emulation-mode[RAS (Return Address Stack) Emulation Mode]). The implication is that returns that
