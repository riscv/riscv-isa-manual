...
|RASEMU |Enables RAS (Return Address Stack) Emulation Mode.  See xref:ras-emulation-mode[RAS (Return Address Stack) Emulation Mode].

|MTE |Enables recording of traps to M-mode when M=0.  See xref:external-traps[External traps].

|STE |Enables recording of traps to S-mode when S=0.  See xref:external-traps[External traps].

|BPFRZ |Set `sctrstatus`.FROZEN on a breakpoint exception that traps to M-mode or S-mode. See xref:freeze[Freeze].

|LCOFIFRZ |Set `sctrstatus`.FROZEN on local-counter-overflow interrupt (LCOFI) that traps to M-mode or S-mode. See xref:freeze[Freeze].

|EXCINH |Inhibit recording of exceptions.  See xref:transfer-type-filtering[Transfer Type Filtering].

|INTRINH |Inhibit recording of interrupts.  See xref:transfer-type-filtering[Transfer Type Filtering].

|TRETINH |Inhibit recording of trap returns.  See xref:transfer-type-filtering[Transfer Type Filtering].

|NTBREN |Enable recording of not-taken branches.  See xref:transfer-type-filtering[Transfer Type Filtering].

|TKBRINH |Inhibit recording of taken branches.  See xref:transfer-type-filtering[Transfer Type Filtering].

|INDCALLINH |Inhibit recording of indirect calls.  See xref:transfer-type-filtering[Transfer Type Filtering].

|DIRCALLINH |Inhibit recording of direct calls.  See xref:transfer-type-filtering[Transfer Type Filtering].

|INDJMPINH |Inhibit recording of indirect jumps (without linkage).  See xref:transfer-type-filtering[Transfer Type Filtering].

|DIRJMPINH |Inhibit recording of direct jumps (without linkage).  See xref:transfer-type-filtering[Transfer Type Filtering].

|CORSWAPINH |Inhibit recording of co-routine swaps.  See xref:transfer-type-filtering[Transfer Type Filtering].

|RETINH |Inhibit recording of function returns.  See xref:transfer-type-filtering[Transfer Type Filtering].

|INDLJMPINH |Inhibit recording of other indirect jumps (with linkage).  See xref:transfer-type-filtering[Transfer Type Filtering].

|DIRLJMPINH |Inhibit recording of other direct jumps (with linkage).  See xref:transfer-type-filtering[Transfer Type Filtering].
|Custom[3:0] | WARL bits designated for custom use.  The value 0 must correspond to standard behavior.  See xref:custom-extensions[Custom Extensions].
|===
...
|STE |Enables recording of traps to VS-mode when S=0.  See xref:external-traps[External traps].
|BPFRZ |Set `sctrstatus`.FROZEN on a breakpoint exception that traps to VS-mode. See xref:freeze[Freeze].
|LCOFIFRZ |Set `sctrstatus`.FROZEN on local-counter-overflow interrupt (LCOFI) that traps to VS-mode. See xref:freeze[Freeze].
...
The depth of the CTR buffer dictates the number of entries to which the hardware records transfers. For a depth of N, the hardware records transfers to entries 0..N-1. All xref:entry-registers[Entry Registers] read as '0' and are read-only when the selected entry is in the range N to 255.  When the depth is increased, the newly accessible entries contain unspecified but legal values.

It is implementation-specific which DEPTH value(s) are supported.
|===

Attempts to access `sctrdepth` from VS-mode or VU-mode raise a virtual-instruction exception, unless CTR state enable access restrictions apply.  See xref:stable-enable-access-control[State Enable Access Control].
...
See xref:stable-enable-access-control[State Enable Access Control] for cases where CTR accesses from S-mode and VS-mode may be restricted.
...
|CCV |Cycle Count Valid. See xref:cycle-counting[Cycle Counting]. |WARL
...
Undefined bits in `ctrdata` are WPRI. Undefined bits must be implemented as read-only 0, unless a xref:custom-extensions[Custom Extensions] is implemented and enabled.

[NOTE]
[%unbreakable]
====
_Like the xref:transfer-type-filtering[Transfer Type Filtering] bits in `mctrctl`, the `ctrdata`.TYPE bits leverage the E-trace itype encodings._
====
...
* Zeroes all  CTR xref:entry-registers[Entry Registers], for all DEPTH values
* Zeroes the CTR cycle counter and CCV (see xref:cycle-counting[Cycle Counting])
...
SCTRCLR raises an illegal-instruction exception in U-mode, and a virtual-instruction exception in VU-mode, unless CTR state enable access restrictions apply.  See xref:stable-enable-access-control[State Enable Access Control].

[[state-enabl-access-control]]
== State Enable Access Control

When Smstateen is implemented, the `mstateen0`.CTR bit controls access to CTR register state from privilege modes less privileged than M-mode.  When `mstateen0`.CTR=1, accesses to CTR register state behave as described in xref:CSRs[CSRs] and xref:entry-registers[Entry Registers] above, while SCTRCLR behaves as described in xref:supervisor-ctr-clear-instruction[Supervisor CTR Clear Instruction].  When `mstateen0`.CTR=0 and the privilege mode is less privileged than M-mode, the following operations raise an illegal-instruction exception:
...
[%unbreakable]
====
_See xref:indirect-csr.adoc[Chapter 5] for how bit 60 in `mstateen0` and `hstateen0` can also restrict access to `sireg*`/`siselect` and `vsireg*`/`vsiselect` from privilege modes less privileged than M-mode._
====
...
|*Enabled* |*Disabled*
.2+|*Trap* |*Enabled* |Recorded. | External trap.  Not recorded by default, but see xref:external-traps[External traps].
...
[NOTE]
====
_Consider an exception that traps from VU-mode to HS-mode, with `vsctrctl`.U=1 and `sctrctl`.S=1.  Because both the source mode and target mode are enabled for recording, whether the trap is recorded then depends on the CTR configuration (e.g., the xref:transfer-type-filtering[Transfer Type Filtering] bits) in `vsctrctl`, not in `sctrctl`._
====
