...
lower-privilege modes, __w__ latexmath:[$<$] __x__, are always globally
...
__y__ latexmath:[$>$] __x__, are always globally enabled regardless of the
...
__y__latexmath:[$\neq$]M, __x__RET also sets MPRV=0.

[NOTE]
====
Setting __x__PP to the least-privileged supported mode on an __x__RET helps
identify software bugs in the management of the two-level privilege-mode
stack.
====

__x__PP fields are *WARL* fields that can hold only privilege mode _x_ and any
implemented privilege mode lower than _x_. If privilege mode _x_ is not
implemented, then __x__PP must be read-only 0.
