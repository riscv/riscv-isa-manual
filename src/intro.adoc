...
xref:rv32.adoc[RV32I Base Integer Instruction Set] and xref:rv64.adoc[RV64I Base Integer Instruction Set], which provide 32-bit
...
xref:rv32e.adoc[RV32E and RV64E Base Integer Instruction Sets] describes the RV32E and RV64E subset variants of the
...
xref:rv128.adoc[RV128I Base Integer Instruction Set] sketches a future RV128I variant of the
...
base ISA. xref:extending.adoc[Extending the RISC-V ISA] extending describes various ways
...
in detail in xref:naming.adoc[ISA Extension Naming Conventions].
...
instruction defined in xref:zifencei.adoc["Zifencei" Extension for Instruction-Fetch Fence]).
(((memory access, implicit and explicit)))
...
xref:rvwmo.adoc["RVWMO Memory Consistency Model] and in appendices. Optionally,
an implementation may adopt the stronger model of Total Store Ordering,
as defined in xref:ztso-st-ext.adoc["Ztso" Extension for Total Store Ordering]. The execution environment
...
described in xref:c-st-ext.adoc["C" Extension for Compressed Instructions] reduces code size by
...
As described xref:extending.adoc[Extending the RISC-V Instruction Set Architecture], an implementation that does not require support
