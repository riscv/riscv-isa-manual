...
RISC-V (pronounced “risk-five”) is a completely open instruction-set architecture (ISA) that not only supports computer architecture research and education, but also is becoming a standard free and open architecture for industry implementations. It is suitable for direct, native hardware  implementation and not just simulations or a binary translation. RISC-V avoids “over-architecting” for a specific microarchitecture style or implementation technology, but instead, allows efficient implementation in many different styles. 

The RISC-V ISA is separated into a small base integer ISA, which is usable by itself as a base for customized accelerators or for educational purposes. In addition, optional standard extensions support general purpose software development. This fully virtualizable ISA eases hypervisor development and simplifies your experiments with new, privileged architecture design.

The RISC-V ISA supports extensive ISA extensions and specialized variants, including the following examples.

* The revised 2008 IEEE-754 floating-point standard [7].
* Both 32-bit and 64-bit address space variants for applications, operating system kernels, and hardware implementations.
* Highly-parallel multicore or manycore implementations, including heterogeneous multiprocessors.
* Optional variable-length instructions to both expand available instruction encoding space and to support an optional dense instruction encoding for improved performance, static code size, and energy efficiency. 

[TIP]
====
Commentary for design decisions is formatted as non-normative notes simpliar to this paragraph. You can skip these notes if you are interested only in the specification itself.
====

The name RISC-V was chosen to represent the fifth major RISC ISA design from UC Berkeley (RISC-I cite:[riscI-isca1981], RISC-II cite:[Katevenis:1983], SOAR cite:[Ungar:1984], and SPUR cite:[spur-jsscc1989] were the first four). It also puns on the use of the Roman numeral "V" to signify "variations" and "vectors", as support for a range of architecture research, including various data-parallel accelerators, is an explicit goal of the ISA design.  

(((ISA, definition)))
The RISC-V ISA is designed to avoid implementation details, although commentary is included on implementation-driven decisions, and can be read as the software-visible interface to a variety of implementations rather than as the design of a specific hardware artifact. The RISC-V manual is structured in two volumes. This volume covers the design of the base unprivileged instructions, including optional unprivileged ISA extensions. Unprivileged instructions can be used generally for all privilege modes in all privileged architectures, though behavior might vary depending on what privilege mode and privilege architecture that you use. The second volume provides the design of the first (“classic”) privileged architecture. These manuals use IEC 80000-13:2008 conventions, with a byte of 8 bits.

[TIP]
====
The unprivileged ISA design avoids dependency for any specific microarchitectural features, such as cache line size, or on privileged architecture details, such as page translation. This design allows for both simplicity and maximum flexibility for alternative microarchitectures or privileged architectures.
====

=== RISC-V Hardware Platform Terminology

A RISC-V hardware platform can contain one or more RISC-V-compatible processing cores, along with other non-RISC-V-compatible cores, fixed-function accelerators, various physical memory structures, I/O devices, and an interconnect structure to allow the components to communicate.
(((core, component)))

A component is termed a _core_ only if it contains an independent instruction fetch unit. A RISC-V-compatible core can support multiple RISC-V-compatible hardware threads, or _harts_, through multithreading.
(((core, extensions, coprocessor)))

A RISC-V core can include additional specialized instruction-set extensions or an added _coprocessor_. The term _coprocessor_  refers to a unit that is attached to a RISC-V core and is mostly sequenced by a RISC-V instruction stream. The unit also contains additional architectural state and instruction-set extensions, and possibly some limited autonomy relative to the primary RISC-V instruction stream.

An _accelerator_ refers to either a non-programmable fixed-function unit or a core that can operate autonomously, but is specialized for certain tasks. In RISC-V systems, many programmable accelerators are expected to be RISC-V-based cores with specialized instruction-set extensions, customized coprocessors, or both. An important class of RISC-V accelerators are I/O accelerators, which offload I/O processing tasks from the main application cores.
(((core, accelerator)))

The system-level organization of a RISC-V hardware platform can range from a single-core microcontroller to a many-thousand-node cluster of shared-memory manycore server nodes. Even small systems-on-a-chip can be structured as a hierarchy of multicomputers and multiprocessors to modularize development effort or to provide secure isolation between subsystems.
(((core, cluster, multiprocessors)))

=== RISC-V Software Execution Environments and Harts

The behavior of a RISC-V program depends on the execution environment in which it runs. A RISC-V execution environment interface (EEI) defines the following attributes:
...
* The behavior of all legal instructions that are executed on each hart (for example, the ISA is one component of the EEI).
* The handling of any interrupts or exceptions raised during execution, including environment calls. 

Examples of EEIs include the Linux application binary interface (ABI) or the RISC-V supervisor binary interface (SBI). The implementation of a RISC-V execution environment can be pure hardware, pure software, or a combination of both hardware and software. For example, opcode traps and software emulation can be used to implement functionality that is not provided in the hardware. 

Execution environment implementations for RISC-V include the following examples:
...
* RISC-V operating systems that provide multiple user-level execution environments, by multiplexing user-level harts onto available physical processor threads and by controlling access to memory by using virtual memory.
* RISC-V hypervisors that provide multiple supervisor-level execution environments for guest operating systems.
* RISC-V emulators, such as Spike, QEMU or rv8, which emulate RISC-V harts on an underlying x86 system. These emulators can provide either a user-level or a supervisor-level execution environment.
...
From the perspective of software running in a given execution environment, a hart is a resource that autonomously fetches and executes RISC-V instructions within that execution environment. In this respect, a hart behaves similar to a hardware thread resource even if the hart is time-multiplexed onto real hardware by the execution environment. Some EEIs support the creation and destruction of additional harts, for example, through environment calls to fork new harts.

The execution environment is responsible for ensuring the eventual forward progress of each of its harts. For an individual hart, that responsibility is suspended while the hart is exercising a mechanism that explicitly waits for an event, such as the wait-for-interrupt instruction (defined in Volume II of this specification). If the hart is terminated, that responsibility ends. 

The following events constitute forward progress of a hart:
...
[TIP]
====
The term hart was introduced in the work on Lithe cite:[lithe-pan-hotpar09] and cite:[lithe-pan-pldi10] to provide a term to represent an abstract execution resource as opposed to a software thread programming abstraction.

The important distinction between a hardware thread (hart) and a software thread context is that the software that is running inside of an execution environment is not responsible for progressing each of its harts; that is the responsibility of the outer execution environment. The environment's harts operate similar to hardware threads from the perspective of the software that is inside the execution environment.

An execution environment implementation might time-multiplex a set of guest harts onto fewer host harts that are provided by its own execution environment. However, the guest harts must operate similar to independent hardware threads. If there are more guest harts than host harts, then the execution environment must be able to preempt the guest harts and not wait indefinitely for guest software on a guest hart to "yield" control of that guest hart.
====

=== RISC-V ISA Overview

A RISC-V ISA is defined as a base integer ISA that must be present in any implementation, plus optional extensions to the base ISA. The base integer ISAs are very similar to that of the early RISC processors. The base integer ISAs so not include branch delay slots. They do include support for optional variable-length instruction encodings. A base is carefully restricted to a minimal set of instructions sufficient to provide a reasonable target for compilers, assemblers, linkers, and operating systems (with additional privileged operations), and so provides a convenient ISA and software toolchain "skeleton" around which you can build a more customized processor ISA.

Although it is convenient to speak of _the_ RISC-V ISA, RISC-V is actually a family of related ISAs, of which there are currently four base ISAs. Each base integer instruction set is characterized by the width of the integer registers and the corresponding size of the address space and by the number of integer registers. 

There are two primary base integer variants, RV32I and RV64I, described in <<rv32>> and <<rv64>>, which provide 32-bit or 64-bit address spaces respectively. Note that the term XLEN refers to the width of an integer register in bits (either 32 or 64). 

<<rv32e, Chapter 6>> describes the RV32E and RV64E subset variants of the RV32I or RV64I base instruction sets respectively, which were added to support small microcontrollers, and include half the number of integer registers as the previous two variants. 

<<rv128, Chapter 8>> sketches a future RV128I variant of the base integer instruction set that supports a flat, 128-bit address space (XLEN=128). The base integer instruction sets use a two's-complement representation for signed integer values.
