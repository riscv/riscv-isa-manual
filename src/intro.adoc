...
xref:rv32.adoc[] and xref:rv64.adoc[], which provide 32-bit
...
xref:rv32e.adoc[] describes the RV32E and RV64E subset variants of the
...
xref:rv128.adoc[] sketches a future RV128I variant of the
...
base ISA. xref:extending.adoc[] describes various ways
...
instruction defined in xref:zifencei.adoc[]).
(((memory access, implicit and explicit)))
...
As described in xref:extending.adoc[], an implementation that does not require support
