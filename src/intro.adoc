...
supervisor binary interface (SBI). [#norm:execution_environment_implementation_flexibility]#The implementation of a RISC-V
...
xref:rv32e.adoc[RV32E and RV64E Base Integer Instruction Sets] and xref:rv64.adoc[RV64I Base Integer Instruction Set], which provide 32-bit
...
base ISA. xref:unpriv:extending.adoc[Extending the RISC-V ISA] describes various ways
...
As described in xref:extending.adoc["Extending RISC-V"], an implementation that does not require support
