...
This volume outlines a standard architecture for debug support on
...
Version 0.13 of the Debug specification was ratified by the RISC-V Foundation’s
...
This volume contains two parts. The main part is the specification,
which is given in the numbered chapters. The second part is a set of
appendices. The information in the appendices is intended to clarify and
provide examples, but is not part of the actual specification.

==== ISA vs. non-ISA

This specification contains both ISA and non-ISA parts. The ISA parts
define self-contained ISA extensions. The other chapters of this volume
...
All register definitions in this volume follow the format shown below.
...
include::{debug-registers}sample_registers.adoc[]
...
This volume does not suggest a strategy or implementation for hardware
