...
reservation set. For example, the Unix platform is expected to require
of main memory that the reservation set be of fixed size, contiguous,
naturally aligned, and no greater than the virtual memory page size.
...
<<cas>>. If inlined, compare-and-swap functionality need
...
<<critical>>. Note the first AMO is marked _aq_ to
