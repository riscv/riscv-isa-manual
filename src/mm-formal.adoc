...
* <<early_sc_fail, Early sc fail>>: This causes the `sc` to fail, either a spontaneous fail or because
...
<<commit_store:prev_addrs>> is satisfied
the conditions
<<commit_store:prev_stores>> and
<<commit_store:prev_loads>> are also
