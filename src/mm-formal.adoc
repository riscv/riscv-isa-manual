...
provided in xref:rvwmo.adoc#memorymodel[RVWMO Memory Model] and in the rest of
the main body of the ISA specification.
All currently known discrepancies are listed in xref:mm-eplan.adoc#discrepancies[Known Issues].
...
how Alloy can be used to model check some of the mappings in xref:mm-eplan.adoc#memory_porting[Code Porting and Mapping Guidelines].
...
(see xref:mm-eplan.adoc#litmustests[Litmus Tests]) and small ELF binaries
...
* for any other instruction, latexmath:[i.\textit{program_loc}+4].
...
[NOTE]
====
In xref:rvwmo.adoc#rvwmo-primitives[Memory Model Primitives] it is said that
...
latexmath:[mlo] in latexmath:[i.\textit{mem_loads}] that has
...
. update latexmath:[i.\textit{mem_loads}] to indicate that
...
latexmath:[i.\textit{mem_loads}] that has unsatisfied slices, can be
...
latexmath:[i.\textit{mem_loads}] are entirely satisfied (i.e. there
...
latexmath:[\textit{address}\ldots\textit{address}+\textit{size}-1].
. set latexmath:[i.\textit{mem_stores}] to latexmath:[msos]; and
...
latexmath:[i.\textit{mem_stores}]. Action:

. split _mem_value_ between the memory store operations
latexmath:[i.\textit{mem_stores}]; and
. update the state of latexmath:[i] to
...
latexmath:[i.\textit{mem_stores}], latexmath:[mso] can be
propagated if:
...
. update latexmath:[i.\textit{mem_stores}] to indicate that
...
. clear latexmath:[i.\textit{mem_stores}]; and
. update the state of latexmath:[i] to
...
operations in latexmath:[i.\textit{mem_stores}] have been propagated,
...
. add _reg_name_ to latexmath:[i.\textit{reg_reads}] with
...
. add _reg_name_ to latexmath:[i.\textit{reg_writes}] with
latexmath:[deps] and _reg_value_; and
. update the state of latexmath:[i] to Plain(_next_state_).

where latexmath:[deps] is a pair of the set of all _read_sources_ from
latexmath:[i.\textit{reg_reads}], and a flag that is true iff
