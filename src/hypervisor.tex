...
zero, as enumerated in Table~\ref{table:hedeleg-bits}.
...
For other traps, {\tt htval} is set to zero, but a future standard may
redefine {\tt htval}'s setting for other traps.
...
However, external interrupts directed to HS-level continue to be
...
{\tt mstatush} (RV32 only), {\tt mideleg}, {\tt mip}, and {\tt mie}, and
...
written with either zero or the guest physical address that faulted,
shifted right by 2~bits.
For other traps, {\tt mtval2} is set to zero, but a future standard may
redefine {\tt mtval2}'s setting for other traps.
...
faulting virtual address as usual, and {\tt mtval2} or {\tt htval} is
...
A nonzero guest physical address written to {\tt mtval2}/{\tt htval}
shall always correspond to the exact virtual address written to
{\tt mtval}/{\tt stval}.
...
\end{tabular}
\end{center}
\caption{Supervisor and machine cause register ({\tt scause} and {\tt mcause}) values when the hypervisor extension is implemented.}
\label{hcauses}
\end{table*}
...
whereas those from HS-mode or S-mode use cause 9.
...
A future standard may define additional cases, thus allowing values that
...
Table~\ref{table:tinst-values} shows the values that may be automatically
written to the trap instruction register for each standard exception
cause.
For exceptions that prevent the fetching of an instruction, only zero may
be written.
A custom value may be automatically written only if the instruction that
traps is nonstandard.
A future standard may permit other values to be written, chosen from the
set of allowed values established earlier.
...
{\tt htinst} must be taken from Table~\ref{table:pseudoinsts};
zero is not allowed.
...
Table~\ref{table:pseudoinsts-basis}.
...
psuedoinstructions of Table~\ref{table:pseudoinsts}.}
\label{table:pseudoinsts-basis}
\end{table*}
