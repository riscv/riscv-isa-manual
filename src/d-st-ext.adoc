...
[#norm:D_ext_flen_64]#The D extension widens the 32 floating-point registers, `f0-f31`, to
...
and comprise floating-point loads and stores (FL__n__/FS__n__) and floating-point move instructions (FMV._n_.X/FMV.X._n_). [#norm:fpr_reg_narrow_transfer_in]#A narrower _n_-bit transfer, _n_<FLEN, into the `f` registers will create a valid NaN-boxed value#. [#norm:fpr_reg_narrow_transfer_out]#A narrower
_n_-bit transfer out of the floating-point registers will
transfer the lower _n_ bits of the register ignoring the
upper FLEN-_n_ bits#.

[[norm:improper_nan-boxed_input]]
...
[#norm:fld]#The FLD instruction loads a double-precision floating-point value from
memory into floating-point register _rd_#. [#norm:fsd]#FSD stores a double-precision
...
[[norm:double_computational_instrs]]
...
instructions are encoded in the OP-FP major opcode space. [#norm:fcvt_int_double]#FCVT.W.D or
...
respectively, in integer register _rd_#. [#norm:fcvt_double_int]#FCVT.D.W or FCVT.D.L converts a
...
_rd_#. [#norm:fcvt_double_unsigned]#FCVT.WU.D, FCVT.LU.D, FCVT.D.WU, and FCVT.D.LU variants convert to
or from unsigned integer values#. [#norm:fcvt_word_double_sign_ext]#For RV64, FCVT.W[U].D sign-extends the
32-bit result. [#norm:fcvt_long_double_rv64_only]#FCVT.L[U].D and FCVT.D.L[U] are RV64-only instructions#.
[#norm:fcvt_valid_input]#The range of valid inputs for FCVT._int_.D and the behavior for invalid
inputs are the same as for FCVT._int_.S#.
(((floating point, convert and move)))

[[norm:fcvt_rounding_mode]]
...
[#norm:fcvt_float_double]#The double-precision to single-precision and single-precision to
...
destination#. [#norm:fcvt_float_double_rounding]#FCVT.S.D rounds according to the RM field; FCVT.D.S will
never round#.
...
[[norm:double_sign_injection_instrs]]
...
patterns between the floating-point and integer registers. [#norm:fmv-x-d]#FMV.X.D moves
...
_rd_#. [#norm:fmv-d-x]#FMV.D.X moves the double-precision value encoded in IEEE 754-2008
