...
This document describes the `Zvkgs`  Vector Cryptography extension, which adds supoprt
 for vector-scalar Galois Multiply and Accumulate-Multiply instructions.
...
This section introduces all of the extensions in the Additional Vector Cryptography 
...
therefore must be a multiple of `EGS=4`. Otherwise the behavior is reserved. +
Likewise, `vstart` must be a multiple of `EGS=4`. Otherwise the behavior is reserved.
