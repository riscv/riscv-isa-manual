...
In addition to the unprivileged state described in <<sec:csrinsts>>,
an implementation may contain additional CSRs, accessible by
some subset of the privilege levels using the CSR instructions described
in <<csrinsts>>. In this chapter, we map out the CSR address space. The
