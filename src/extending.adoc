...
xref:rv32-64g.adoc#opcodemap[RISC-V base opcode map], each of which represents a 25-bit
...
gray in xref:rv32-64g.adoc#opcodemap[RISC-V base opcode map]).
...
encoded as described in xref:introduction#instlengthcode[RISC-V instruction length encoding].
...
xref:introduction#instlengthcode[RISC-V instruction length encoding] to encode parallel
