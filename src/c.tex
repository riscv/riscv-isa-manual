...
added to any of the base ISAs (RV32, RV64), and we use the
...
and RV64C, but as shown in Table~\ref{rvcopcodemap}, a few
opcodes are used for different purposes depending on base ISA width.
For example, the wider address-space RV64C variants require
...
of single-precision floating-point values.  If the C
...
(RV32I/E or RV64I) or the F and D standard extensions where
...
C.LDSP is an RV64C-only instruction that loads a 64-bit value from memory into
...
C.SDSP is an RV64C-only instruction that stores a 64-bit value in register
...
C.LD is an RV64C-only instruction that loads a 64-bit value from memory into
...
encoding quadrant where all other immediates are sign-extended.
\end{commentary}
...
retaining the guarantee of eventual success.
